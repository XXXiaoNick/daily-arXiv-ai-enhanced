...
TL;DR: 该研究比较了微调的传统CNN、零样本预训练多模态LLM和微调多模态LLM在图像中人工文本叠加检测任务上的表现，发现LLM在少量数据（少于1000张图像）上微调后性能提升36%，达到或超过需要大量数据的CNN基线。


<details>
  <summary>Details</summary>
Motivation: 探索如何将语言引导模型适应于精确的视觉理解任务，特别是在有限监督的情况下，以弥合视觉和语言之间的差距，研究多模态LLM在专业视觉任务中的潜力。

Method: 对传统CNN、零样本预训练多模态LLM和微调多模态LLM在人工文本叠加检测任务上进行全面比较，重点研究LLM在少量数据上的微调效果。

Result: LLM在少于1000张图像上微调后，准确率提升高达36%，性能匹配或超过需要大量数据的CNN基线模型，展示了LLM方法的数据效率和适应性。

Conclusion: LLM为基础的方法在真实世界物体检测任务中具有出色的适应性和数据效率，为在低资源视觉环境中应用多模态变换器提供了实用指导。
...
TL;DR: 该研究评估了使用焦点损失和基础数据增强策略的U-Net在脑肿瘤MRI分割中的性能，建立了可复现的基线模型。
...
Method: 在公开MRI数据集上使用U-Net模型，采用焦点损失函数并进行参数调优，评估水平翻转、旋转和缩放三种数据增强技术的影响。

Result: 使用焦点损失的U-Net模型达到了90%的精确度，与最先进方法结果相当。

Conclusion: 通过公开所有代码和结果，本研究为脑肿瘤分割中的数据增强策略和损失函数设计提供了透明可复现的基线参考。
...
Method: 提出两种缓解策略：集体或单独调整初始噪声，寻找和使用能鼓励更早吸引盆逃逸的初始样本。

Result: 这些方法显著减少了记忆化，同时保持了图像-文本对齐。

Conclusion: 初始噪声样本在决定逃逸时间中起关键作用，通过适当调整初始噪声可以有效缓解扩散模型的记忆化问题。
...
TL;DR: 研究测试了AI生成图像中的代表性偏见，发现DALL-E 3和Ideogram都强化了传统性别刻板印象，特别是在职业图像中。


<details>
  <summary>Details</summary>
Motivation: 当前AI生成可视化研究主要关注创建过程和质量，忽视了代表性偏见问题。随着AI图像工具日益普及，解决有害的性别偏见对确保媒体和专业环境中的多样性至关重要。

Method: 通过生成750多张AI职业图像，使用主题分析方法评估DALL-E 3和Ideogram两个AI图像生成工具在性别代表性方面的表现。

Result: 研究发现两个AI工具都强化了传统性别刻板印象，尽管程度不同。AI可视化工具存在强化狭隘代表性的风险。

Conclusion: 研究强调需要解决AI生成图像中的性别偏见问题，并为从业者、个人和研究人员提出了增加代表性、生成可见性别图像的建议。
...
TL;DR: 在视觉自回归模型(VAR)中引入动态专家混合路由机制，通过尺度感知阈值策略在无需额外训练的情况下减少计算冗余，实现20% FLOPs减少和11%推理加速，同时保持图像质量。


<details>
  <summary>Details</summary>
Motivation: VAR模型在递增分辨率下重复调用Transformer导致计算冗余，需要提高计算效率而不牺牲生成质量。

Method: 在VAR架构中集成动态专家混合路由器，采用尺度感知阈值策略，根据token复杂度和分辨率平衡专家选择。

Result: 相比密集基线，减少20% FLOPs，推理速度提升11%，图像质量保持相同水平。

Conclusion: 动态专家混合路由机制有效解决了VAR模型的计算冗余问题，实现了计算效率与生成质量的良好平衡。
...
<details>
  <summary>Details</summary>
Motivation: 现有无监督OOD检测方法通常依赖预测熵，但这种方法混淆了认知不确定性和随机不确定性，导致将分布内模糊区域误分类为OOD。需要一种能够准确分离这两种不确定性的方法。

Method: 使用层次贝叶斯建模高斯混合模型参数在深度神经网络特征空间中的认知不确定性，无需辅助数据或额外训练阶段。

Result: 在SemanticKITTI数据集上，相比现有基于预测熵的方法，AUROC提升18%，AUPRC提升22%，FPR95从76%降低到40%（减少36%）。

Conclusion: 该方法通过准确分离认知不确定性，显著提升了无监督OOD检测性能，无需额外数据或训练，为LiDAR点云语义分割中的未知物体检测提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 在人类活动识别中，现实生活中的活动范围与标注传感器数据集中可捕获的活动之间存在巨大差距，无法正确处理未见活动会严重削弱分类器的可靠性。此外，并非所有类别都同等不同，有些类别显著重叠或包含其他子活动。

Method: 将活动类别组织成结构化层次结构，提出Hi-OSCAR分层开放集分类器，能够识别已知活动并同时拒绝未知活动，允许将未知类别定位到最近的内部节点。

Result: 开发了新的数据集NFI_FARED，包含多个受试者执行19种活动的数据，涵盖日常生活、通勤和快速运动等不同情境，该数据集完全公开可供下载。

Conclusion: Hi-OSCAR不仅实现了开放集分类，还提供了超越二元"已知/未知"分类的洞察力，为开放集HAR研究奠定了基础。
...
TL;DR: 提出了一种基于无监督聚类的高频振荡检测方法，在80-500Hz频段内区分HFOs与尖峰、背景活动和伪迹，在控制数据集上达到97.67%的敏感性和98.57%的精确度。
...
Method: 使用时频域S变换提取事件，采用无监督聚类技术对事件进行分类，区分HFOs与尖峰、背景活动和伪迹。

Result: 在控制数据集上敏感性97.67%、精确度98.57%、F分数97.78%；在癫痫患者中，切除与非切除接触点的HFOs比率达0.73，与手术结果相关性更强。

Conclusion: HFOs是癫痫患者癫痫发生性的有前景生物标志物，移除HFOs（特别是快速波纹）可导致无癫痫发作，而残留HFOs则导致癫痫复发。
...
TL;DR: 本文分析了DINOv2视觉模型的表示机制，提出了Minkowski表示假说，认为token是由原型凸组合形成的，超越了传统的线性稀疏表示。


<details>
  <summary>Details</summary>
Motivation: 理解DINOv2模型感知内容的本质，探索视觉变换器表示的组织结构，超越传统的线性表示假说。

Method: 采用稀疏自编码器构建32,000个单元的字典，分析下游任务的概念使用模式，研究表示几何和统计特性，提出Minkowski表示假说。

Result: 发现分类任务利用"Elsewhere"概念实现学习否定，分割依赖边界检测器，深度估计使用三种单目深度线索；表示部分密集而非严格稀疏，具有低维局部连接结构。

Conclusion: 提出了Minkowski表示假说，认为token是由原型凸组合形成的，这种结构基于概念空间理论，为解释视觉变换器表示提供了新视角。
...
Result: 在三个基准数据集上的实验表明，PhyDAE在四种修复任务中均取得最优性能，同时显著减少参数量和计算复杂度，实现性能与效率的最佳平衡。
...
TL;DR: Hulu-Med是一个透明的医学视觉语言模型，统一处理医学文本、2D/3D图像和视频，在30个基准测试中达到最先进性能，超越开源模型并与专有系统竞争。


<details>
  <summary>Details</summary>
Motivation: 解决临床决策中多模态数据整合的挑战，克服现有医学视觉语言模型的不透明流程、数据稀缺和架构不灵活问题。

Method: 基于统一补丁视觉编码器和LLM解码器构建，渐进式训练1670万个样本，从2D扩展到3D和视频理解，采用医学感知令牌减少实现高效训练。

Result: 在视觉问答、医学报告生成和多语言罕见疾病场景的复杂推理任务中表现优异，7B到32B参数变体仅需4000到40000 GPU小时训练。
...
TL;DR: Puffin是一个统一的相机中心多模态模型，通过将相机视为语言来扩展空间感知，能够从任意视角理解和生成场景。
...
Method: 引入"相机即语言"的新范式，结合语言回归和基于扩散的生成方法，使用包含400万视觉-语言-相机三元组的大规模数据集Puffin-4M进行训练，整合全局相机参数和像素级相机映射。
...
Method: 提出SOR框架，冻结内部网络结构（如卷积滤波器），同时使用组套索和L1惩罚的组合，以最少额外参数定制模型。

Result: 在三个少样本医学图像分类任务中，使用DenseNet121和EfficientNetB4基础网络取得了与现有基准相当的竞争性结果。
...
TL;DR: BEAR是一个全面的多模态大语言模型具身能力评估基准，包含4,469个跨14个领域的图像-视频-文本条目，涵盖从低层次感知到高层次规划的6个类别。评估发现现有模型在具身能力方面存在持续局限，提出的BEAR-Agent方法能显著提升模型性能。
...
Method: 提出BEAR基准，包含4,469个跨14个领域的图像-视频-文本条目，涵盖6个类别。同时提出BEAR-Agent方法，集成预训练视觉模型来增强多模态大语言模型的感知、3D理解和规划能力。

Result: 评估20个代表性多模态大语言模型发现它们在所有具身能力领域都存在持续局限。BEAR-Agent方法在BEAR基准上实现了9.12%的绝对增益和17.5%的相对改进。

Conclusion: 提升多模态大语言模型的具身能力有益于模拟环境中的具身任务，BEAR基准为系统性评估提供了重要工具。
...
TL;DR: 提出一种受生物视觉机制启发的对抗防御框架，通过模拟人眼的注视点-外周处理、扫视眼动和皮层填充机制，无需重新训练分类器即可有效缓解对抗性噪声。


<details>
  <summary>Details</summary>
Motivation: 传统防御方法依赖计算密集的优化，而人类视觉系统通过进化机制天然具有对抗扰动的鲁棒性。研究假设注意力引导的非均匀稀疏采样和预测编码是这种鲁棒性的关键。

Method: 采用强化学习引导的扫视来选择性捕获多个注视点-外周瞥视，将这些瞥视整合成重建图像后再进行分类。该方法包含三个关键生物机制：注视点-外周处理、扫视眼动和皮层填充。

Result: 在ImageNet数据集上的实验表明，该方法能提高系统在不同分类器和攻击类型下的鲁棒性，同时相比生物和非生物启发的防御技术显著减少训练开销。

Conclusion: 生物启发的预处理能有效缓解对抗性噪声，保持语义完整性，且无需重新训练或微调下游分类器，可与现有系统无缝集成。
...
TL;DR: 基于预训练深度学习模型的实时泄漏检测系统，结合RGB和热成像技术，在多种环境下实现泄漏与非泄漏场景分类，热成像模型在推理速度、准确性和模型大小方面表现更优。


<details>
  <summary>Details</summary>
Motivation: 开发能够在不同环境条件下实时、准确检测泄漏的安全关键系统，解决传统方法在复杂光照条件下的性能限制问题。

Method: 使用平衡的二元数据集（4000张图像），采用预训练的深度学习模型（如VGG19、NasNetMobile）处理RGB和热成像数据，在消费级硬件（RTX 4080）上实现实时检测。

Result: 达到100%准确率，热成像模型推理时间低至44毫秒，模型大小小于350MB，在真实机器人测试中VGG19热成像模型表现最佳。

Conclusion: 热成像技术在泄漏检测中具有显著优势，系统在消费级硬件上实现了高效、实时的部署能力，适用于安全关键应用场景。
...
TL;DR: LinearSR是一个基于线性注意力的图像超分辨率框架，首次系统性地解决了线性注意力在超分辨率中的训练不稳定、感知-失真权衡等关键挑战，实现了高效且高质量的图像超分辨率。


<details>
  <summary>Details</summary>
Motivation: 生成式图像超分辨率模型依赖自注意力的二次复杂度(O(N^2))，造成计算瓶颈。线性注意力提供O(N)解决方案，但历史上因训练不稳定等问题未能充分发挥潜力。

Method: 提出ESGF策略解决训练不稳定问题；使用SNR-based MoE架构缓解感知-失真权衡；建立轻量级TAG指导范式；构建完整的LinearSR框架。

Result: LinearSR同时实现最先进的感知质量和卓越效率，核心扩散前向传播(1-NFE)达到SOTA级速度，整体多步推理时间保持高度竞争力。

Conclusion: 该工作首次为线性注意力在逼真超分辨率领域提供了稳健方法，为高效生成式超分辨率的未来研究建立了基础范式。
...
TL;DR: 提出了一种基于计算机视觉的无监督关键帧提取管道，用于新西兰濒危鹦鹉kākā的个体识别，替代传统侵入性标记方法。


<details>
  <summary>Details</summary>
Motivation: 传统野生动物监测方法（如鸟类腿环标记）耗时且具有侵入性，需要开发基于AI的非侵入式高效替代方案。

Method: 结合YOLO和Grounding DINO进行目标检测，使用光流模糊检测、DINOv2图像编码和聚类方法，从喂食器视频中提取代表性关键帧。

Result: 提出的关键帧选择方法在kākā重识别任务中实现了高准确率，为在更具挑战性环境中使用媒体数据奠定了基础。

Conclusion: 这种基于AI和计算机视觉的非侵入式方法为野生动物监测提供了有价值的替代方案，有助于生态学和保护生物学的新方法开发。
...
TL;DR: 提出了Q-Router，一个基于多层级模型路由的智能视频质量评估框架，通过视觉语言模型动态选择专家模型，解决了现有VQA模型泛化性差、可解释性有限和扩展性不足的问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于直接分数监督的VQA模型存在三个主要问题：跨不同内容和任务的泛化能力差（包括用户生成内容、短视频和AI生成内容）、可解释性有限，以及缺乏对新用例或内容类型的扩展性。

Method: Q-Router采用多层级模型路由系统，集成多种专家模型，使用视觉语言模型作为实时路由器，根据输入视频语义动态推理并集成最合适的专家模型。系统基于计算预算构建多层级路由，最重的层级包含特定的时空伪影定位以实现可解释性。

Result: 大量实验表明，Q-Router在各种基准测试中匹配或超越了最先进的VQA模型，同时显著提高了泛化性和可解释性。在基于质量的问题回答基准Q-Bench-Video上表现优异，并能有效定位时空伪影。

Conclusion: Q-Router展示了作为下一代VQA系统基础的潜力，能够结合专业专家的互补优势，在异构视频源和任务中提供灵活且鲁棒的性能，并有望作为视频生成模型后训练中的奖励函数。
...
TL;DR: 提出一个解决医学视觉问答挑战的统一框架，包括多级模态对齐、硬负样本挖掘和门控交叉注意力模块，在多个Med-VQA数据集上达到最优性能。
...
Method: 1) 多级模态对齐的统一解决方案；2) 使用软标签的硬负样本挖掘方法；3) 集成答案词汇表作为先验知识的门控交叉注意力模块。
...
Result: 在标准SR基准测试中，该方法在720p视频上比现有模型实现了高达60%的端到端延迟提升，且没有可感知的质量损失。

Conclusion: SkipSR通过选择性跳过低细节区域的计算，在保持感知质量的同时显著加速了视频超分辨率处理，为更高分辨率和更长视频的处理提供了可行的解决方案。
...
TL;DR: 提出D-CoDe框架，通过动态压缩和问题分解解决图像预训练视觉语言模型在视频理解中的感知瓶颈和token过载问题，无需训练即可提升视频理解能力。


<details>
  <summary>Details</summary>
Motivation: 将图像预训练的视觉语言模型(VLMs)扩展到视频领域面临挑战，主要问题是感知瓶颈和token过载，因为视频包含密集且时间延展的视觉输入，超出了图像模型的处理能力。

Method: D-CoDe框架包含两个核心组件：1) 动态压缩 - 自适应选择代表性帧并进行内容感知的空间token聚合，减少冗余同时保留信息内容；2) 问题分解 - 将原始查询重新表述为子问题，引导模型关注视频的不同方面。

Result: 实验表明D-CoDe在各种基准测试中有效提升了视频理解能力，在具有挑战性的长视频基准上表现优异，显示出处理复杂视频语言任务的潜力。

Conclusion: D-CoDe是一个无需训练的适配框架，通过动态压缩和问题分解成功解决了视频大语言模型中的关键挑战，为图像预训练模型向视频领域的扩展提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法将3D实例映射到2D RGB-D图像并使用视觉语言模型分类，但会引入2D遮挡噪声且计算内存成本高，推理速度慢

Method: 设计教师模型提取高质量实例嵌入，通过标签引导蒸馏算法将开放词汇知识蒸馏到3D学生模型中，学生模型可直接从点云分类实例

Result: 在ScanNet200数据集上达到35.7 AP50的SOTA性能，推理速度比之前方法快6.0x到152.2x
...
TL;DR: 提出了一种基于自监督视觉变换器的作物生长监测方法，避免了繁琐的图像标注，能够预测生长趋势并区分不同蔓越莓品种的时间差异。


<details>
  <summary>Details</summary>
Motivation: 蔓越莓种植中的变化监测通常需要人工完成，耗时且效率低。深度学习有潜力但面临高维特征难以解释和需要大量标注的问题。

Method: 使用自监督方法微调视觉变换器，通过时间回归和类别预测的双重前置任务学习植物和果实外观演化的潜在空间表示。

Result: 生成的2D时间轨迹提供了可解释的作物生长时间序列模型，能够预测生长趋势并区分不同品种的时间差异。
...
TL;DR: PHyCLIP提出了一种使用双曲空间ℓ₁乘积度量的方法，同时捕捉概念族内的层次结构和跨概念族的组合语义。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型难以同时表达概念族内的层次结构（如狗⊆哺乳动物⊆动物）和跨概念族的组合语义（如"车里的狗"⊆狗、车），双曲空间能有效表示树状层次但组合性表示能力不足。

Method: 采用双曲空间的ℓ₁乘积度量，在双曲因子的笛卡尔积上构建模型。概念族内层次在单个双曲因子中表示，跨概念族组合通过ℓ₁乘积度量捕获，类似于布尔代数。

Result: 在零样本分类、检索、层次分类和组合理解任务上的实验表明，PHyCLIP优于现有的单空间方法，并在嵌入空间中提供更可解释的结构。

Conclusion: PHyCLIP通过双曲空间的乘积度量成功解决了同时表示层次结构和组合语义的挑战，为多模态表示学习提供了更有效的框架。
...
<details>
  <summary>Details</summary>
Motivation: 现有的分割模型在白盒设置下对对抗样本具有显著脆弱性，但现有对抗攻击方法在不同分割模型间的迁移性较差。分割模型中复杂的上下文依赖关系以及代理模型和目标模型之间的特征分布差距导致迁移成功率不理想。

Method: 提出SegTrans框架，将输入样本划分为多个局部区域，重新映射其语义信息生成增强样本，用这些增强样本替代原始样本进行扰动优化。该方法仅保留原始输入的局部语义信息，而非使用全局语义信息来优化扰动。
...
Conclusion: SegTrans通过局部语义信息重映射有效解决了分割模型对抗攻击的迁移性问题，在保持高攻击成功率的同时显著提升了计算效率。
...
<details>
  <summary>Details</summary>
Motivation: 现有防御方法主要针对图像分类任务，难以扩展到图像修复这类生成任务。图像修复输出连续高维且依赖空间连贯性，微小扰动不足以阻止学生模型学习底层映射。

Method: ASVP使用奇异值分解对教师模型内部特征图进行操作，放大前k个奇异值来注入结构化高频扰动，破坏蒸馏所需的对齐关系。

Result: 在5个图像修复任务上测试，ASVP使学生模型PSNR降低高达4dB，SSIM降低60-75%，而对教师模型性能影响可忽略。相比现有方法提供更强更一致的防御效果。

Conclusion: ASVP为保护开源图像修复模型免受未经授权的知识蒸馏提供了实用解决方案。
...
Method: 构建Ro-Bench基准，包含通过编辑风格、对象、背景及其组合生成的高质量、多样化且时序相关的反事实视频数据。

Result: 评估了8个最新视频MLLMs，发现现有模型在反事实视频内容上性能显著下降；通过反事实数据微调，在Ro-Bench上性能提升21.73%，在MVBench的20个任务上提升12.78%。
...
TL;DR: 提出了一种针对动物健康监测的对象聚焦数据增强框架，通过分割动物和背景并使用变换及扩散合成来增强有限数据场景下的检测性能


<details>
  <summary>Details</summary>
Motivation: 农业监测系统依赖多源数据，但无人机动物健康监测面临数据稀缺问题，传统迁移学习方法因缺乏反映特定农场条件的大数据集而效果有限

Method: 开发对象聚焦数据增强框架，分割动物与背景，通过变换和扩散合成生成真实多样的场景，提升动物检测性能

Result: 初步实验显示增强数据集在动物检测任务上优于基线模型

Conclusion: 该方法通过生成领域特定数据，在数据稀缺场景下实现了实时动物健康监测，弥补了有限数据与实际应用之间的差距
...
TL;DR: 提出了Perception-Time Scaling (PTS)新范式，通过将复杂感知问题分解为中间可处理的子问题，显著提升大视觉语言模型在视觉感知任务中的精度，在DisTANCE基准上高精度表现从8.0%提升至64.7%。


<details>
  <summary>Details</summary>
Motivation: 现有大视觉语言模型在推理时扩展方面取得进展，但在视觉感知任务中效果有限，主要因为当前模型采用"快速感知"范式，将视觉理解视为一次性输出，没有建模底层感知过程。

Method: 提出Perception-Time Scaling (PTS)范式，鼓励token丰富的感知，将复杂感知问题分解为中间可处理的子问题；结合强化学习技术实现感知与推理时扩展的对齐。

Result: PTS显著提升感知精度，在DisTANCE基准上高精度表现从8.0%提升至64.7%；在域外任务中泛化良好；即使使用纯合成数据，与数学推理数据结合也能在推理和真实世界感知基准上获得一致提升。

Conclusion: PTS通过引入更多感知相关token和增加模型对图像token的关注，有效解决了当前大视觉语言模型在视觉感知任务中的局限性，为感知与推理的协同优化提供了新思路。
...
Result: 在13种姿态估计设置下使用超过115,000个信号帧进行评估，mmJoints估计描述符的错误率低于4.2%，关节位置精度提升高达12.5%，活动识别性能提升高达16%。
...
TL;DR: HiMIR是一个高效的图像检索调度框架，通过分层多粒度图像表示和跨层次相似性一致性来提升检索精度和效率。


<details>
  <summary>Details</summary>
Motivation: 传统检索方法精度有限，现有多向量检索方法虽然通过分解查询和图像分割提高了精度，但仍存在对齐不足和冗余计算的问题。

Method: 提出分层范式使用多个中间粒度表示不同图像对象以增强对齐，利用跨层次相似性一致性和层次稀疏性减少冗余匹配计算，并自动为不同数据集配置参数。

Result: HiMIR在精度上显著提升，同时计算量比现有MVR系统减少高达3.5倍。

Conclusion: HiMIR框架通过分层多粒度表示和优化检索调度，在保持高精度的同时大幅提升了图像检索效率。
...
TL;DR: 提出了首个针对生成图像中手部区域的质量评估任务HandEval，基于HandPair数据集训练，利用MLLM和手部关键点先验知识，在T2I模型生成的手部图像质量评估中优于现有方法，并能提升下游任务的性能。


<details>
  <summary>Details</summary>
Motivation: 当前T2I模型在生成复杂局部区域（特别是人手）时存在结构扭曲和纹理不真实的问题，但手部区域质量评估被忽视，限制了人像生成质量优化和AIGC检测等下游任务的性能。

Method: 构建HandPair数据集（48k高质量和低质量手部图像对），开发HandEval模型，结合MLLM的视觉理解能力和手部关键点先验知识，并构建人工标注测试集进行验证。

Result: HandEval在质量评估上与人类判断更一致，优于现有SOTA方法。集成到图像生成和AIGC检测流程中，分别显著提升了生成手部真实感和检测准确率。

Conclusion: HandEval是首个专门针对生成手部图像的质量评估模型，在评估准确性和下游应用有效性方面表现出色，为T2I模型的手部生成质量优化提供了有力工具。
...
TL;DR: 提出首个防御AI非法着色的方法Uncolorable Examples，通过向灰度图像添加不可察觉的扰动来破坏未经授权的着色效果。


<details>
  <summary>Details</summary>
Motivation: AI着色技术存在版权侵权风险，如未经授权对黑白漫画和电影进行着色并转售，但目前缺乏有效的防范方法。

Method: 提出PAChroma方法，使用拉普拉斯滤波器优化不可察觉的扰动以保持感知质量，并在优化过程中应用多样化输入变换以增强跨模型迁移性和对后处理的鲁棒性。
...
Method: 引入next-clean-token预测范式，通过低成本微调使预训练自回归模型能够接受噪声扰动token嵌入并预测下一个干净token；在推理时使用高斯噪声初始化token序列，在嵌入空间进行迭代的next-clean-token预测，采用概率准则并行验证和接受多个token。
...
TL;DR: 本文提出一种通过修改视觉编码器来缓解大型视觉语言模型中物体幻觉问题的方法，通过识别并屏蔽不确定视觉标记来减少幻觉现象。
...
Method: 提出一种简单有效的策略：使用对抗扰动识别不确定视觉标记，并在视觉编码器的中间层自注意力过程中屏蔽这些标记，抑制其对视觉编码的影响。

Result: 大量实验表明，该方法显著减少了大型视觉语言模型中的物体幻觉，并能与其他现有技术协同工作。

Conclusion: 通过修改视觉编码器来识别和屏蔽不确定视觉标记，是缓解物体幻觉问题的有效方法，具有实用性和兼容性。
...
Method: 1) 基于token分布空间熵的动态温度控制，平衡内容多样性、对齐精度和结构一致性；2) 在推测解码中引入熵感知接受规则，实现近无损生成。

Result: 在多个基准测试中验证了方法的有效性，能够在约85%的推理成本下实现传统加速方法的近无损生成。

Conclusion: 该方法在多种自回归图像生成模型中均表现出良好的效果和泛化能力，显著提升了生成质量和采样速度。
...
TL;DR: 该论文提出了DGLSS-NL任务，即噪声标签下的LiDAR语义分割领域泛化，并开发了DuNe双视图框架，通过强-弱分支一致性约束和置信度感知过滤，在多个数据集上实现了最先进的性能。
...
Conclusion: DuNe框架在DGLSS-NL任务中表现出强大的领域泛化能力，为噪声标签下的LiDAR语义分割提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 潜在扩散模型在医学图像转换中效率高，但可能损失对临床诊断至关重要的像素级细节，特别是小病灶区域的准确重建。

Method: 开发了一种后训练框架，在预训练的潜在扩散模型中引入病灶感知的医学像素空间目标，增强病灶轮廓的精确度。
...
TL;DR: 开发了一个基于视觉Transformer的图像异常检测框架，用于监控机器人柔性微电极植入过程，通过渐进粒度特征采样和特征通道选择来提高检测性能。
...
Method: 利用机器人FME植入系统的显微相机，在四个检查点分别检查微针、FME探针、钩挂结果和植入点。使用预训练的视觉Transformer，提出渐进粒度补丁特征采样方法解决不同位置的敏感度-容错度权衡问题，并选择信噪比更高的特征通道。
...
TL;DR: 提出一种通过多模态知识训练单模态3D-CNN的方法，用于动态手势识别，通过跨模态对齐损失提升单模态网络性能


<details>
  <summary>Details</summary>
Motivation: 现有方法通常显式组合多模态信息，但本文希望将多模态知识嵌入到单模态网络中，使每个单模态网络都能获得性能提升
...
TL;DR: MambaH-Fit：基于状态空间建模的超曲面拟合点云法向量估计框架，通过注意力驱动的层次特征融合和补丁状态空间模型，显著提升局部几何细节建模能力


<details>
  <summary>Details</summary>
Motivation: 现有法向量估计方法在建模细粒度几何结构方面不足，限制了法向量预测精度。现有Mamba方法主要关注全局形状结构，对局部几何细节建模不足

Method: 提出注意力驱动的层次特征融合方案自适应融合多尺度点云补丁特征；构建补丁状态空间模型，将点云补丁建模为隐式超曲面进行状态动态建模

Result: 在基准数据集上的广泛实验表明，该方法在准确性、鲁棒性和灵活性方面优于现有方法

Conclusion: 所提出的组件通过消融研究验证了其有效性，能够有效理解细粒度几何结构以进行法向量预测
...
<details>
  <summary>Details</summary>
Motivation: 传统稀疏表示分类方法在特征表示方面存在局限，需要更强大的特征学习能力来提高分类性能。

Method: 构建包含卷积自编码器和全连接层的网络架构，自编码器学习鲁棒深度特征，全连接层负责寻找稀疏表示，最终利用稀疏编码进行分类。

Result: 在三个不同数据集上的实验表明，该方法产生的稀疏表示比最先进的SRC方法获得更好的分类结果。

Conclusion: 提出的深度稀疏表示分类网络能够有效学习鲁棒特征并生成高质量的稀疏表示，显著提升分类性能。
...
TL;DR: 提出GL-DT框架，通过时空特征融合和全局-局部协同检测策略提升无人机多目标跟踪性能，解决小目标检测和轨迹连续性等问题。
...
Conclusion: 该方法为无人机检测和跟踪技术的发展提供了有力支持，有效解决了现有方法在检测精度和轨迹连续性方面的挑战。
...
TL;DR: 该论文提出Dense2MoE方法，将密集的扩散变换器转换为混合专家结构，通过结构化稀疏化减少激活参数60%，同时保持原始性能。
...
Method: 将DiT块中的前馈网络替换为MoE层，提出混合块选择性激活DiT块，采用多步蒸馏流程包括泰勒度量专家初始化、负载平衡知识蒸馏和组特征损失优化。

Result: 将FLUX.1等大型扩散变换器转换为MoE结构，FFN激活参数减少62.5%，总激活参数减少60%，性能保持原始水平且优于剪枝方法。
...
TL;DR: 本文提出了一种新颖的多分支ConvNeXt架构，专门针对医学图像分析的细微挑战，在COVID-19诊断中实现了优异性能，ROC-AUC达到0.9937，验证准确率0.9757，F1分数0.9825。
...
Method: 采用多分支ConvNeXt架构，集成全局平均池化、全局最大池化和新的注意力加权池化三个并行分支。使用端到端流程，包括数据预处理、增强和两阶段训练策略，有效利用迁移学习。

Result: 在包含2,609个CT切片的组合数据集上验证，模型性能优异：ROC-AUC 0.9937，验证准确率0.9757，F1分数0.9825，优于该数据集上所有先前报告的模型。
...
TL;DR: SOS是一个基于对象中心组合策略的简单可扩展数据合成管道，通过将高质量合成对象片段粘贴到新图像中，生成准确多样的掩码、边界框和指代表达式，在检测和视觉定位任务上超越大型真实图像数据集。


<details>
  <summary>Details</summary>
Motivation: 大型标注数据集成本高昂、覆盖范围存在偏差且难以扩展，合成数据缺乏灵活性、准确性和组合多样性。

Method: 使用对象中心组合策略，基于结构化布局先验和生成式重光照，将高质量合成对象片段粘贴到新图像中。

Result: 在10万张SOS合成图像上训练的模型在检测和视觉定位任务上优于在GRIT(2000万)和V3Det(20万)等更大真实图像数据集上训练的模型，在LVIS检测上提升+10.9 AP，在gRefCOCO视觉定位上提升+8.4 N_Acc。

Conclusion: SOS支持可控数据集构建，在低数据和封闭词汇设置下改善泛化能力，与真实数据结合在有限真实数据场景下表现更佳。
...
TL;DR: 提出多模态语义扩散模型MSDM，通过整合细胞形态、颜色特征和元数据来生成逼真的细胞核分割图像-掩码对，解决计算病理学中标注数据稀缺问题，提升分割模型的准确性和泛化能力。
...
Method: 开发多模态语义扩散模型MSDM，通过细胞/细胞核形态图、RGB颜色特征和BERT编码的元数据来条件化生成过程，使用多头交叉注意力整合这些异构模态，实现对生成图像的精细控制。

Result: 定量分析显示合成图像与真实数据高度匹配，在匹配生物条件下生成图像与真实图像的嵌入之间的Wasserstein距离较低。合成样本（如柱状细胞）显著提高了分割模型在柱状细胞上的准确性。
...
TL;DR: 提出了PSepT（极坐标可分离变换），一种可分离的正交变换，通过张量积构造离散余弦变换径向基和傅里叶谐波角向基，克服了极坐标中的不可分离性障碍，显著降低了计算复杂度和内存需求。


<details>
  <summary>Details</summary>
Motivation: 传统的正交矩图像表示方法存在高计算复杂度和数值不稳定性问题，特别是在大阶数时。Zernike和伪Zernike矩需要耦合的径向-角向处理，无法高效分解，导致计算复杂度和条件数随阶数急剧增长。

Method: PSepT采用张量积构造，将离散余弦变换（DCT）径向基和傅里叶谐波角向基结合，实现完全核分解，支持独立的径向和角向处理。这种可分离设计将计算复杂度从O(n³N²)到O(n⁶N²)降低到O(N² log N)。

Result: PSepT将计算复杂度降至O(N² log N)，内存需求降至O(N²)，条件数缩放降至O(√N)，相比多项式方法实现了指数级改进。实验结果显示更好的数值稳定性、计算效率和在结构化数据集上的竞争性分类性能，同时保持精确重建。

Conclusion: PSepT的可分离框架实现了传统方法无法实现的高阶矩分析，为稳健的图像分析应用开辟了新可能性，提供了正交性、完备性、能量守恒和旋转协变性等优良特性。
...
<details>
  <summary>Details</summary>
Motivation: 深度神经网络通常被视为不透明系统，需要可解释性方法来提高信任和问责。现有方法通常将测试时预测归因于输入特征或影响训练样本，但作者认为应该将这两种视角结合起来研究。

Method: 探索训练特征归因，将测试预测与特定训练图像的特定区域联系起来，从而提供对深度模型内部工作的新见解。

Result: 在视觉数据集上的实验表明，训练特征归因能够产生细粒度的、测试特定的解释：它识别出导致错误分类的有害示例，并揭示了传统归因方法未能暴露的虚假相关性（如基于补丁的捷径）。
...
<details>
  <summary>Details</summary>
Motivation: 视频支气管镜检查中，医生需要确定支气管镜在复杂支气管树中的位置，这对医生具有挑战性。现有导航方法依赖CT扫描和额外传感器，需要额外设置和训练。拓扑定位通常足以提供导航辅助。

Method: 开发了基于图像的支气管镜拓扑定位管道，仅使用幻影数据进行训练，无需患者CT扫描，避免了真实数据标注的高成本。

Result: 该方法在结果上超越了现有方法，特别是在真实数据测试序列上表现优异。

Conclusion: 提出的图像支气管镜拓扑定位方法无需CT扫描，训练成本低，泛化能力强，为支气管镜检查提供了有效的导航辅助。
...
TL;DR: 提出了一种无需真实图像即可合成多样化对象实例的方法，用于解决实例级识别(ILR)中标注数据稀缺的问题，显著提升了多个ILR基准测试的性能。


<details>
  <summary>Details</summary>
Motivation: 实例级识别需要识别单个对象而非宽泛类别，但细粒度特性使得大规模标注数据集创建困难，限制了ILR在实际应用中的广泛使用。

Method: 通过合成方法从多个领域生成多样化对象实例，包含不同条件和背景，形成大规模训练集，无需依赖任何真实图像。

Result: 在七个跨越多个领域的ILR基准测试中，基于生成数据微调的基础视觉模型显著提升了检索性能。

Conclusion: 该方法为大规模数据收集和整理提供了高效有效的替代方案，只需目标领域名称作为输入即可实现ILR，开启了广泛实际应用的可能性。
...
TL;DR: TARO是一个新颖的开放世界目标检测框架，不仅识别未知物体，还将其分类到语义层次结构中的粗粒度父类别，相比传统方法仅标记为'Unknown'更有用。


<details>
  <summary>Details</summary>
Motivation: 现代目标检测器局限于'封闭世界'假设，遇到新物体时存在风险。传统开放集检测方法将所有未知物体标记为单一'Unknown'类别，这在安全关键场景中不够实用。

Method: TARO采用独特架构：基于sparsemax的头部建模物体性，层次引导的重标记组件提供辅助监督，分类模块学习层次关系。

Result: 实验显示TARO能将最多29.9%的未知物体分类到有意义的粗粒度类别，显著减少未知与已知类别的混淆，在未知召回率和已知mAP方面都表现优异。

Conclusion: TARO框架有效解决了开放世界检测问题，通过层次化分类为未知物体提供更有意义的描述，在安全关键应用中具有重要价值。
...
<details>
  <summary>Details</summary>
Motivation: 现有的Video Depth Anything (VDA)方法依赖批处理，无法在在线场景下使用，这限制了其在实时系统中的应用。
...
Conclusion: oVDA成功实现了高效的在线视频深度估计，特别适合在边缘设备上部署，将发布代码和编译脚本以便实际应用。
...
TL;DR: 本文首次对板球击球动作分类进行了全面的基准研究，比较了四种研究范式的七种深度学习方法，发现学术文献中的准确率声明与实际实现结果存在显著差距。


<details>
  <summary>Details</summary>
Motivation: 板球击球动作分类在体育视频分析中仍然是一个具有挑战性的问题，需要有效建模空间和时间特征。现有文献中的准确率声明与实际性能存在较大差距。

Method: 系统评估了传统CNN-LSTM架构、注意力模型、视觉变换器、迁移学习方法以及现代EfficientNet-GRU组合，在统一基准上使用PyTorch Lightning实现。

Result: 重新实现先前论文的方法得到显著较低的准确率（46.0%、55.6%、57.7%），而现代SOTA方法（EfficientNet-B0+GRU）达到92.25%的准确率。

Conclusion: 现代架构和系统优化可以带来实质性改进，标准化评估协议在体育视频分析研究中至关重要。
...
TL;DR: 提出了一个可解释的驾驶员意图预测框架VCBM，通过新的多模态数据集DAAD-X和概念瓶颈模型，为自动驾驶系统提供时空一致的解释，无需后处理技术。


<details>
  <summary>Details</summary>
Motivation: 随着自动驾驶系统与人类交互增加，决策过程的可解释性对安全驾驶至关重要。深度学习系统需要理解环境表示和驾驶任务，这是当前的主要挑战。

Method: 构建DAAD-X多模态数据集，包含驾驶员眼动和车辆视角的层次化文本解释；提出Video Concept Bottleneck Model (VCBM)框架，生成时空一致的解释；使用transformer模型和可视化技术。

Result: 实验表明transformer模型比传统CNN模型具有更好的可解释性；提出的多标签t-SNE可视化技术能展示多个解释之间的解缠和因果关联。

Conclusion: VCBM框架能够为驾驶员意图预测提供内在可解释性，transformer架构在可解释性方面优于CNN，为自动驾驶系统的安全交互提供了有效解决方案。
...
TL;DR: Cattle-CLIP是一个基于CLIP的多模态深度学习框架，通过语义线索改进牛只行为识别性能，在监督学习下达到96.1%的准确率，并在少样本场景中表现稳健。


<details>
  <summary>Details</summary>
Motivation: 牛只行为是动物健康和生产力的重要指标，现有视频监控方法在数据稀缺的行为识别任务中存在性能限制，需要开发更有效的识别方法。

Method: 基于CLIP模型构建多模态框架，添加时间整合模块，采用定制化数据增强策略和专用文本提示来弥合领域差距，并在CattleBehaviours6数据集上进行评估。

Result: 在监督设置下达到96.1%的总体准确率，对进食、饮水和站立反刍行为的召回率接近100%，在少样本场景中表现出良好的泛化能力。
...
Method: 设计了时空自注意力编码器和时空交叉注意力解码器，通过多尺度特征处理探索局部和全局相关性，并开发了TRT-LOS和TRT-NLOS两个任务特定实现。

Result: 在合成数据和真实世界数据上的广泛实验表明，该方法显著优于现有方法，并贡献了大规模合成LOS数据集和真实NLOS测量数据。

Conclusion: TRT架构在光子效率成像的3D重建任务中表现出色，为LOS和NLOS成像提供了有效的解决方案。
...
TL;DR: 提出Stable Video Infinity (SVI)方法，能够生成无限长度视频，具有高时间一致性、合理的场景转换和可控的流式故事情节。通过错误回收微调技术，让Diffusion Transformer从自身生成的错误中学习，解决了训练假设与自回归推理之间的差异。
...
Method: SVI采用错误回收微调，通过闭环回收将DiT自生成的错误转化为监督提示，鼓励模型主动识别和纠正自身错误。具体包括：(i)注入历史错误干预干净输入；(ii)用一步双向积分高效近似预测并计算残差误差；(iii)跨离散时间步动态存储错误到重放内存中。

Result: SVI能够将视频从秒级扩展到无限时长，无需额外推理成本，同时保持与多种条件（音频、骨架、文本流）的兼容性。在三个基准测试上验证了其多功能性和最先进性能。
...
TL;DR: 提出了TEMA-LLM框架，通过大语言模型生成语义标签，结合多注意力机制提升跨域序列推荐性能


<details>
  <summary>Details</summary>
Motivation: 跨域序列推荐在电商平台中至关重要，需要准确捕捉域内和跨域行为模式来提供个性化体验

Method: 使用LLM生成领域感知的语义标签，将标签嵌入与项目标识符、文本和视觉特征融合，引入标签增强的多注意力机制建模用户偏好

Result: 在四个大规模电商数据集上的实验表明，TEMA-LLM持续优于最先进的基线方法

Conclusion: 该方法展示了LLM在提升消费者电子领域智能、用户中心服务方面的潜力
...
TL;DR: 本文对图像和视频在恶劣天气条件下的恢复技术进行了全面综述，涵盖了传统方法和现代数据驱动模型，包括CNN、Transformer、扩散模型和视觉语言模型，并讨论了当前研究的局限性和未来方向。


<details>
  <summary>Details</summary>
Motivation: 恶劣天气条件（如雾霾、雨雪）会显著降低图像和视频质量，对依赖视觉输入的智能交通系统（如自动驾驶、交通监控）构成严重挑战，因此需要开发有效的恢复技术来减轻天气引起的视觉损伤。

Method: 将现有方法分类为传统基于先验的方法和现代数据驱动模型（CNN、Transformer、扩散模型、视觉语言模型），并根据处理范围分为单任务模型、多任务/多天气系统和全能框架。

Result: 提供了全面的技术分类和评估，包括日间和夜间恢复挑战、基准数据集和评估协议，为开发天气弹性视觉系统提供了重要参考。

Conclusion: 指出了当前研究的局限性，并展望了未来方向，包括混合/复合退化恢复、实时部署和智能AI框架，同时承诺持续更新最新相关论文和开源实现。
...
Method: 提出HMVDx混合运动视频诊断框架，使用两个MLLM分别完成动作理解和疾病诊断任务，并引入基于医疗决策逻辑的可用性指标。

Result: HMVDx在肩关节损伤诊断中的准确率比直接视频诊断提高了79.6%。

Conclusion: 该研究展示了低成本MLLM在医疗应用中的潜力，为未来MLLM在医学视频理解领域的应用提供了重要技术贡献。
...
TL;DR: 本文建立了图像隐私分类的零样本基准，发现尽管视觉语言模型(VLMs)在鲁棒性方面表现更好，但在隐私预测准确性上仍落后于专门的较小模型。


<details>
  <summary>Details</summary>
Motivation: 当前文献倾向于采用为通用任务设计的大型视觉语言模型，但缺乏系统评估，可能忽略了专用模型的性能上限。

Method: 建立零样本基准，评估前3名开源VLM，使用任务对齐提示，并与现有视觉专用和多模态方法对比性能、效率和鲁棒性。

Result: VLM虽然在参数数量和推理速度上资源密集，但在隐私预测准确性上落后于专门的较小模型，但对图像扰动的鲁棒性更高。

Conclusion: VLM目前在隐私预测准确性上不如专用模型，但在鲁棒性方面有优势。
...
<details>
  <summary>Details</summary>
Motivation: 为了解决黑盒视觉语言模型在放射学图像问答中产生幻觉的问题，需要一种可靠的方法来检测和过滤可能产生错误回答的问题。

Method: 使用GPT-4o和GPT-4.1模型，通过高温采样(温度1.0)生成多个回答，利用双向蕴含检查将意义等效的回答分组，计算语义簇的相对频率来得到DSE值。通过排除高DSE值的问题来提高准确性。

Result: 在706个图像-问题对中，基线准确率为GPT-4o 51.7%，GPT-4.1 54.8%。过滤掉高熵问题(DSE > 0.3)后，GPT-4o准确率提升至76.3%(保留334/706问题)，GPT-4.1提升至63.8%(保留499/706问题)，统计显著性p < .001。
...
TL;DR: 提出统一框架联合优化时序语句定位(TSG)和参考视频对象分割(RefVOS)，通过Moment-Centric采样策略和双向锚点更新传播提升分割性能。
...
Method: 训练时使用[FIND]令牌进行关键时刻识别，推理时采用Moment-Centric采样策略，结合双向锚点更新传播增强跟踪稳定性。

Result: 提出的方法在多个RefVOS基准测试中取得最先进性能。

Conclusion: 该框架有效整合时序定位能力，无需外部时间戳编码，显著提升视频对象分割质量。
...
TL;DR: 本文提出VPPO算法，通过分析token级视觉感知来优化多模态强化学习，重点关注视觉依赖度高的关键token，在多个基准测试中显著提升模型性能。
...
Result: 在8个感知和推理基准测试中，VPPO显著优于领先的开源RL调优模型，在7B和32B模型规模上均验证了有效性。

Conclusion: 本研究不仅为分析多模态RLVR建立了新的token级感知视角，还提出了有效的优化策略来显著增强LVLMs的多模态推理能力。
...
TL;DR: 人类视觉注视轨迹遵循类似动物觅食的Levy行走模式，通过大规模眼动实验和CNN模型证明了视觉信息搜索的统计规律。
...
Method: 进行大规模人类被试实验，40名参与者观看50张多样化图像，使用高速眼动仪记录超过400万个注视点，并训练CNN从图像输入预测注视热图。

Result: 分析显示人类眼睛的注视轨迹遵循类似动物觅食的Levy行走，CNN模型能够准确重现新颖图像中的显著注视区域。
...
TL;DR: 提出了CapGeo框架，通过将几何图形转换为文本描述来提升多模态大语言模型的几何推理能力，并创建了CapGeo-Bench数据集用于评估几何描述模型。


<details>
  <summary>Details</summary>
Motivation: 当前先进的多模态大语言模型在几何推理方面表现不佳，瓶颈在于理解几何图形而非推理本身。由于几何图形可以用简洁文本描述，将视觉内容转换为文本描述是提升几何推理能力的有效途径。

Method: 引入CapGeo框架，通过几何图形描述辅助推理，将视觉内容转换为文本描述；创建CapGeo-Bench数据集（4,641个图形-描述对）和基于关键点的评估指标来系统评估几何描述模型。

Result: 使用描述后模型性能显著提升：Qwen2.5-VL-72B从8.6%提升到59.0%，Claude-Opus-4从44.8%提升到73.0%。提出的关键点评估指标与下游CapGeo性能强相关。

Conclusion: CapGeo框架和基准测试为推进多模态大语言模型的几何推理能力提供了新途径，通过将视觉内容转换为文本描述可以有效解决几何推理瓶颈。
...
<details>
  <summary>Details</summary>
Motivation: 解决基于扩散的无线电地图生成方法存在模型大、迭代去噪慢、推理延迟高等问题，阻碍实际部署。

Method: 提出RadioFlow框架，学习噪声与数据之间的连续传输轨迹，实现单步高效采样，显著加速训练和推理过程。

Result: 实验表明RadioFlow在保持重建精度的同时，参数量减少8倍，推理速度提升4倍以上，达到最先进性能。
...
TL;DR: 提出Hybrid-depth框架，通过整合CLIP和DINO基础模型，在对比语言引导下聚合多粒度特征，提升单目深度估计性能。


<details>
  <summary>Details</summary>
Motivation: 当前自监督单目深度估计方法由于语义-空间知识提取不足而存在性能限制，需要获取更充分的上下文信息。

Method: 采用粗到细渐进学习框架：1) 在对比语言引导下聚合CLIP的全局语义和DINO的局部空间细节特征；2) 集成相机位姿信息和像素级语言对齐来细化深度预测，可作为即插即用模块与现有MDE框架集成。

Result: 在KITTI基准测试中显著优于现有最优方法，所有指标均有提升，且有益于下游任务如BEV感知。

Conclusion: 通过语言引导聚合CLIP语义上下文和DINO空间细节，有效解决了特征粒度不匹配问题，提升了单目深度估计性能。
...
TL;DR: 提出IRCR-Net用于病理图像中的细胞核实例分割，通过实例感知一致性正则化机制解决半监督学习中实例级一致性不足和伪标签噪声问题。
...
Result: 实验结果表明，该方法在多个公开数据集上显著提升了半监督细胞核实例分割性能，在某些场景下甚至超越了全监督方法。
...
TL;DR: 本文提出了一种用于热红外图像增强的渐进式提示融合网络(PPFN)，通过建立基于热成像过程的提示对来调制模型特征，并引入选择性渐进训练机制处理复合退化情况。
...
Method: 提出PPFN网络，基于热成像过程建立提示对，通过融合对应提示对来调制特征；引入选择性渐进训练(SPT)机制逐步优化模型对复合情况的处理。

Result: 该方法在特定退化下提供有前景的视觉结果，在复杂退化场景中性能显著提升8.76%，同时建立了高质量多场景红外基准数据集。

Conclusion: PPFN方法能够有效处理热红外图像的单重和多重退化问题，在保留关键结构细节的同时增强图像对比度，为热红外图像增强提供了有效解决方案。
...
TL;DR: 提出基于视觉语言模型的多模态关键词预测方法，通过零样本、监督微调、Fine-tune-CoT和动态CoT策略提升模型在复杂推理场景下的性能。
...
Method: 使用视觉语言模型，采用零样本、监督微调、Fine-tune-CoT和动态CoT策略，其中动态CoT通过自适应注入CoT数据解决"过度思考"问题。

Result: 在多个数据集上的实验结果表明所提方法的有效性。

Conclusion: 提出的动态CoT策略能够有效提升多模态关键词预测任务的性能，特别是在复杂推理场景下。
...
Method: BLINK-Twice集成了三个核心组件：七种视觉挑战类型、自然对抗图像对以强制依赖视觉内容、带注释的推理链用于细粒度评估推理过程。

Result: 评估了20个领先的MLLMs，包括12个基础模型和8个推理增强模型。BLINK-Twice对当前模型构成重大挑战，现有语言空间的推理策略虽然能提高性能，但往往导致不稳定和冗余的推理。

Conclusion: 重复图像观察能提高模型性能，像o3这样的主动视觉交互模型表明需要新的视觉推理范式，数据集已公开可用。
...
TL;DR: VAD-GS是一个针对城市场景的3D高斯泼溅框架，通过体素可见性推理、多样性视图选择和基于块匹配的多视图立体重建来恢复缺失的几何结构，解决了传统3DGS在点云初始化不完整时的失真问题。


<details>
  <summary>Details</summary>
Motivation: 传统3D高斯泼溅方法严重依赖初始点云质量，在城市等无界动态环境中，由于观察视锥体不重叠，导致点云覆盖不均匀和不完整，训练时会产生失真和伪影。现有方法无法重建缺失结构。

Method: 1. 基于体素的可见性推理识别不可靠几何结构；2. 多样性感知的视图选择选取信息丰富的支持视图；3. 基于块匹配的多视图立体重建恢复缺失结构；4. 在缺乏初始点的区域生成新的高斯基元。

Result: 在Waymo和nuScenes数据集上的大量实验表明，VAD-GS优于最先进的3DGS方法，显著提高了静态和动态对象重建几何的质量。

Conclusion: VAD-GS能够通过可靠的几何先验在缺乏初始点的区域生成新的高斯基元，有效解决了3DGS在城市场景中的几何恢复问题，提升了重建质量。
...
TL;DR: 提出了两种无需重新训练的DETR模型MLP层稀疏化方法：SIBS（基于静态激活模式的启发式方法）和MGS（轻量级门控机制），MGS能实现85-95%的激活稀疏度，在COCO数据集上保持或提升性能的同时显著减少计算量。
...
Method: SIBS：基于固定激活模式预测神经元非活动性的启发式方法；MGS：在预训练DETR基础上训练的轻量级门控机制，使用小型线性层预测动态稀疏性。

Result: MGS实现了85-95%的激活稀疏度，在COCO数据集实验中保持或改进性能的同时显著减少了计算量。
...
Conclusion: Mono4DEditor提供了一个灵活准确的4D场景文本驱动编辑框架，成功解决了复杂动态场景中语义精确编辑的挑战。
...
TL;DR: DWTA-Net是一个两阶段低光照视频增强框架，通过结合短期和长期时序信息来抑制噪声和伪影，提升视觉质量。


<details>
  <summary>Details</summary>
Motivation: 现有学习方法在真实低光照场景中处理严重噪声时存在困难，主要原因是无法有效利用时序信息。

Method: 第一阶段使用Visual State-Space块进行多帧对齐，恢复亮度、颜色和结构；第二阶段引入基于光流引导的动态权重时序聚合循环精炼模块，自适应平衡静态和动态区域。

Result: 在真实低光照视频上的实验表明，DWTA-Net能有效抑制噪声和伪影，相比最先进方法提供更优的视觉质量。

Conclusion: DWTA-Net通过联合利用短期和长期时序线索，成功解决了低光照视频增强中的噪声问题。
...
TL;DR: 提出了SilvaScenes数据集，用于森林环境中的树木实例分割和物种分类，包含1476棵树木的24个物种，在五个生物气候区域收集。基准测试显示树木分割相对容易但物种分类仍具挑战性。


<details>
  <summary>Details</summary>
Motivation: 现有数据集主要关注城市环境或物种数量有限，无法满足森林管理中复杂自然环境下的感知需求，如精确林业、生物多样性监测和林业设备自动化。

Method: 在加拿大魁北克五个生物气候区域收集林冠下图像，由林业专家标注1476棵树木的24个物种，构建SilvaScenes数据集，并基准测试现代深度学习实例分割方法。

Result: 树木分割表现良好（最高mAP 67.65%），但物种分类仍然困难（mAP仅35.69%），表明该任务具有挑战性。

Conclusion: SilvaScenes填补了森林环境感知数据集的空白，为开发复杂自然条件下的树木检测和物种分类系统提供了重要资源。
...
Result: 该方法缓解了测试时提示调优中校准性能的下降，为实际部署场景中视觉语言模型的可靠性提供了简单有效的解决方案。

Conclusion: 维度熵最大化是一种有效的正则化方法，能够提升视觉语言模型在测试时适应中的校准性能，增强模型在实际应用中的可靠性。
...
TL;DR: 提出一种基于DreamBooth的少样本角色生成方法，通过多token策略和LoRA微调，能够从少量参考角色中生成无限数量的新角色，同时保持艺术风格一致性。


<details>
  <summary>Details</summary>
Motivation: 解决动画和游戏行业中从少量参考角色生成大量新角色时保持艺术风格一致性的挑战，突破传统方法的限制。

Method: 基于DreamBooth框架，采用多token策略（为单个角色和集体风格分配独立token），结合LoRA参数高效微调，移除类别特定正则化集，并在生成时引入随机token和嵌入。

Result: 在五个小型专业数据集上的评估显示，该方法能生成高质量、多样化的角色，同时保持参考角色的独特美学特征，人类评估进一步证实了其有效性。

Conclusion: 该方法在少样本条件下成功实现了无限角色生成，同时保持了艺术风格一致性，为动画和游戏行业提供了创新的创作工具。
...
TL;DR: 提出了基于临床实践的方法论来定义交互式医学图像分割的评估任务和指标，并构建了标准化评估框架。通过评估现有算法发现：最小化用户交互信息损失、自适应缩放机制、训练与验证一致性、3D上下文的重要性以及非医学领域模型的局限性。


<details>
  <summary>Details</summary>
Motivation: 当前交互式医学图像分割算法评估存在不一致和临床不现实的问题，阻碍了公平比较和真实性能评估。

Method: 提出临床基础的方法论来定义评估任务和指标，构建标准化评估框架，并在异构复杂任务上评估最先进算法。

Result: 发现关键因素包括：最小化交互信息损失、自适应缩放机制、训练验证一致性、3D上下文对不规则形状目标的重要性，以及非医学领域模型在复杂情况下的性能下降。

Conclusion: 标准化评估框架对公平比较算法性能至关重要，交互式分割算法的设计需要考虑临床现实性、信息保留和上下文建模。
...
TL;DR: PhysToolBench是首个专门评估多模态大语言模型对物理工具理解能力的基准，包含三个难度级别：工具识别、工具理解和工具创造，评估了32个模型并发现工具理解能力存在显著不足。


<details>
  <summary>Details</summary>
Motivation: 虽然多模态大语言模型在具身AI中利用常识知识进行高级规划，但其对物理工具的真正理解程度尚未量化，需要专门的评估基准。

Method: 构建包含1000多个图像-文本对的视觉问答数据集，通过三个难度级别评估模型能力：工具识别、工具理解和工具创造。

Result: 对32个多模态大语言模型的评估显示，在工具理解方面存在显著缺陷，特别是在理解工具操作原理和创造新工具方面。

Conclusion: 当前多模态大语言模型在物理工具理解方面仍有很大改进空间，需要进一步研究来提升这方面的能力。
...
TL;DR: 三星智能手机图像中的对角线伪影会影响PRNU相机源验证，但PRO模式的原始图像可避免此问题。该伪影也可用于减少HDR误检和定位人像模式中的合成虚化区域。


<details>
  <summary>Details</summary>
Motivation: 研究三星智能手机图像中的对角线伪影及其对PRNU相机源验证的影响，探索在无法获取原始图像时的替代解决方案。

Method: 分析多款三星Galaxy系列手机的图像伪影模式，比较普通JPEG图像和PRO模式原始图像的PRNU验证效果，探索伪影在法证应用中的潜在用途。

Result: 发现某些Galaxy S和A系列手机存在共同的伪影模式导致指纹冲突；PRO模式原始图像可可靠进行PRNU验证；伪影可用于减少HDR图像误检和定位人像模式合成虚化区域。

Conclusion: 对角线伪影对PRNU验证构成挑战，但原始图像可规避此问题；在无法获取原始图像时，伪影本身可转化为有用的法证工具。
...
TL;DR: PRNet是一个实时检测框架，通过保留和有效利用原始浅层空间特征来增强小目标表示，在保持计算效率的同时显著提升小目标检测精度。


<details>
  <summary>Details</summary>
Motivation: 航拍图像中的小目标检测由于像素表示有限，在特征提取过程中存在严重的信息退化问题，浅层空间细节难以与语义信息有效对齐，导致频繁漏检和误检。现有FPN方法通过后处理增强来缓解这些损失，但重建的细节往往偏离原始图像信息，阻碍了与语义内容的融合。

Method: 提出PRNet框架，包含两个核心模块：渐进细化颈部（PRN）通过骨干网络重用和迭代细化实现空间-语义对齐；增强切片采样（ESSamp）通过优化的重排和卷积在降采样过程中保留浅层信息。

Result: 在VisDrone、AI-TOD和UAVDT数据集上的大量实验表明，PRNet在可比较的计算约束下优于最先进的方法，实现了更优的精度-效率权衡。

Conclusion: PRNet通过专注于原始浅层空间特征的保留和高效利用，有效解决了航拍图像中小目标检测的信息退化问题，在保持实时性的同时显著提升了检测性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统多层感知机(MLP)作为隐式神经表示(INR)进行变形建模时，需要昂贵的正则化，导致训练不稳定且难以有效对齐特征。

Method: 将变形重新定义为微分向量流的构造，通过将结构流属性直接编码到网络架构中，确保连续性、可逆性和时间一致性。

Result: 在各种应用中（包括人脸和图像变形，以及高斯溅射变形）的广泛实验表明，FLOWING实现了最先进的变形质量，且收敛更快。

Conclusion: FLOWING框架通过流中心方法产生了原则性和稳定的变换，实现了准确且结构保持的2D图像和3D形状变形。
...
TL;DR: TC-LoRA是一种新的可控扩散模型范式，通过动态调节模型权重来实现条件控制，相比传统的静态激活修改方法，能更好地适应生成过程的不同阶段。


<details>
  <summary>Details</summary>
Motivation: 当前可控扩散模型使用静态架构修改中间激活来注入条件指导，这种方法无法适应多阶段去噪过程的动态特性，限制了模型从粗结构到细细节的适应能力。

Method: 提出TC-LoRA框架，使用超网络实时生成LoRA适配器，根据时间和用户条件为冻结的主干网络定制权重修改，实现动态、上下文感知的控制。
...
TL;DR: FSP-DETR是一个统一的检测框架，能够在单一模型中实现稳健的少样本检测、开放集识别和跨生物医学任务的泛化，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 生物医学环境中的目标检测受到标注数据稀缺和新类别频繁出现的根本限制，需要能够处理少样本、开放集和跨任务场景的统一解决方案。

Method: 基于类别无关的DETR骨干网络，通过原始支持图像构建类别原型，使用增强视图和轻量级Transformer解码器学习嵌入空间，联合优化原型匹配损失、对齐分离损失和KL散度正则化。

Result: 在卵细胞、血细胞和疟疾检测任务上的广泛实验表明，FSP-DETR在少样本和开放集场景下显著优于先前的少样本和基于原型的检测器。

Conclusion: FSP-DETR提供了推理时的灵活性，支持未见类别识别、背景拒绝和跨任务适应而无需重新训练，为生物医学目标检测提供了统一有效的解决方案。
...
TL;DR: 对2023-2025年CVPR、ICLR和NeurIPS的26,104篇论文进行透明、可复现的研究趋势测量，识别出三个宏观转变：多模态视觉-语言-大语言模型工作的急剧增长、生成方法的稳步扩展以及3D和视频活动的持续活跃。


<details>
  <summary>Details</summary>
Motivation: 量化计算机视觉和机器学习领域的研究趋势，揭示技术发展的宏观转变，为学术界提供可审计和扩展的分析框架。

Method: 对论文标题和摘要进行标准化、短语保护和匹配，使用手工制作的词典分配最多35个主题标签，挖掘任务、架构、训练机制、目标、数据集和共现模态的细粒度线索。

Result: 发现三个主要趋势：(1)多模态视觉-语言-LLM工作急剧增长，将经典感知重新定义为指令跟随和多步推理；(2)生成方法稳步扩展，扩散研究围绕可控性、蒸馏和速度进行整合；(3)3D和视频活动持续活跃，从NeRF转向高斯溅射，并越来越关注以人和智能体为中心的理解。

Conclusion: 研究揭示了计算机视觉和机器学习领域的重要技术转变，发布了词典和方法论以支持审计和扩展，虽然存在词典召回率和仅基于摘要的局限性，但纵向信号在会议和年份间保持一致。
...
<details>
  <summary>Details</summary>
Motivation: 解决当前空间推理研究对室内3D扫描和人工标注的依赖，以及缺乏有效全尺度场景建模导致过拟合的问题，推进多场景下的全尺度空间推理能力。

Method: 使用任务特定、专家驱动的自动化流水线构建SpaceVista-1M数据集（包含38K视频场景、约1M空间问答对），并开发SpaceVista-7B模型，采用尺度感知专家和渐进奖励机制。

Result: 在5个基准测试（包括SpaceVista-Bench）上表现出色，展示了跨所有尺度和场景的强大泛化能力。

Conclusion: 提出的数据集、模型和基准测试为全尺度空间推理提供了有效解决方案，显著提升了MLLMs的空间智能水平。
...
TL;DR: 提出基于蒸馏的框架，通过从小型动作模型转移知识，为视觉语言模型赋予动作执行能力，大幅降低训练成本并提升性能
...
Method: 两阶段训练策略：1) 轻量级对齐，将VLM隐藏状态映射到小型动作模型的动作空间；2) 选择性微调语言模型、状态编码器和动作模块

Result: 在LIBERO上达到97.3%平均成功率（提升11.8%），在LIBERO-LONG上达到93.5%（提升24.5%），真实世界实验中达到82.0%成功率（提升17%）

Conclusion: 动作蒸馏能有效使视觉语言模型生成精确动作，同时大幅减少训练成本
...
TL;DR: StreamingVLM是一个为实时理解无限视觉输入而设计的视觉语言模型，通过统一的训练和推理框架解决长视频处理中的延迟和内存问题。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在处理无限视频流时面临二次计算成本和性能下降的挑战，滑动窗口方法要么破坏连贯性，要么因冗余重计算导致高延迟。
...
Conclusion: StreamingVLM实现了对无限视觉输入的实时稳定理解，其SFT策略不仅增强了流式推理能力，还提升了通用VQA能力，无需特定任务的微调。
...
TL;DR: 评估GLiNER-BioMed模型在BioASQ生物医学命名实体识别任务上的表现，引入基于词典的后处理策略来改善基因和化学实体分类错误，但该方法在开发集上有效而在盲测集上未能泛化。


<details>
  <summary>Details</summary>
Motivation: 解决生物医学命名实体识别中相似实体类型（如基因和化学物）难以区分的问题，提升信息提取的准确性。

Method: 使用GLiNER-BioMed模型，并引入基于词典的后处理策略来修正常见误分类，同时探索了条件随机场等替代方法。

Result: 后处理策略在开发集上将微平均F1分数从0.79提升到0.83，但在盲测集上反而从0.79下降到0.77，显示过拟合问题。
...
TL;DR: 本文揭示了测试时缩放(TTS)技术的一个新失败模式：当候选响应多样性受限时，TTS更可能产生不安全输出。作者提出了RefDiv攻击方法来验证这一现象，并发现现有安全防护措施对此类攻击防护有限。


<details>
  <summary>Details</summary>
Motivation: TTS技术假设候选响应多样性能够提高可靠性，但作者发现这一假设存在缺陷。当多样性受限时，TTS系统更容易产生不安全输出，这构成了一个未被认识到的安全风险。

Method: 提出RefDiv（参考引导多样性减少）协议作为诊断攻击方法，通过限制候选响应的多样性来压力测试TTS管道。在四个开源模型和两种TTS策略上进行了广泛实验。

Result: 实验表明，限制多样性会显著增加TTS产生不安全结果的概率，这种效应甚至比高对抗性意图的提示更强。现有安全防护分类器无法有效检测RefDiv生成的对抗性输入。

Conclusion: TTS技术存在基于多样性的固有安全漏洞，现有防护措施对此防护有限。需要设计更鲁棒的TTS策略来抵御此类多样性驱动的攻击。
...
TL;DR: 提出HAREN-CTC模型，通过多任务学习和跨层注意力机制整合多层自监督学习特征，用于语音抑郁症检测，在两个数据集上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有语音抑郁症检测方法通常只使用SSL模型的最后一层或单一最佳层，容易过拟合且无法充分利用层次结构特征来捕捉稀疏、异质的抑郁信号。

Method: HAREN-CTC架构包含层次自适应聚类模块和跨模态融合模块，使用CTC损失处理稀疏时序监督，通过跨层注意力建模层间依赖关系。

Result: 在DAIC-WOZ和MODMA数据集上分别达到0.81和0.82的macro F1分数，在标准数据划分和交叉验证设置下均优于现有方法。

Conclusion: HAREN-CTC通过有效整合多层SSL特征和CTC时序建模，显著提升了语音抑郁症检测的准确性和泛化能力。
...
TL;DR: 提出了一种评估数学推理能力的新框架，通过分析GPT-3.5在GSM8K数据集上的推理步骤，识别特定失败点并揭示模型推理模式的认知特征。


<details>
  <summary>Details</summary>
Motivation: 解决人工智能中机器学习模型对数学理解程度的核心问题，超越标准基准测试来诊断具体的推理失败点。

Method: 首先让GPT-3.5在GSM8K数据集上生成结构化推理步骤，然后用GPT-4o-mini分析错误并进行无监督聚类，识别出现的"推理模式"。

Result: 揭示了模型认知特征的显著非人类脆弱性：在顺序计算等程序性模式上准确率接近完美，但在需要组合推理的限制性模式上性能急剧下降。
...
TL;DR: 提出置信度分数(CS)作为无参考指标的替代方案，缓解传统基于流畅性指标对创意文本生成的偏见。
...
Method: 从模型输出概率分布中推导置信度分数(CS)，并与流畅性指标在创意提示上的表现进行对比。

Result: 在99个创意提示上，流畅性指标偏好新颖回复的比例为0%，而CS为19%，差异显著。CS还能有效区分不同难度任务。

Conclusion: 置信度分数在保留传统指标核心评估优势的同时，缓解了对创意性的偏见，为现代LLM提供更平衡的评估。
...
TL;DR: Recover-LoRA是一种轻量级、数据集无关的方法，通过合成数据和logit蒸馏来恢复因量化、剪枝、序列化等优化导致的语言模型性能下降。


<details>
  <summary>Details</summary>
Motivation: 语言模型部署中的推理优化（如量化、剪枝、格式转换等）会导致模型性能下降，现有研究主要关注鲁棒量化技术，而本文关注从任何导致模型权重退化的源中恢复模型精度。

Method: 使用合成数据和logit蒸馏在选择性层上学习LoRA适配器，将退化模型与其全精度模型对齐。

Result: 在多种小型语言模型（包括MHA和GQA注意力架构）上，Recover-LoRA能够恢复5-17%的模型精度。

Conclusion: Recover-LoRA提供了一种有效的方法来恢复因各种优化技术导致的模型性能退化，适用于不同类型的注意力架构。
...
TL;DR: Mnemosyne是一种受人类记忆启发的无监督长期记忆架构，专为边缘设备上的大语言模型设计，通过图结构存储、记忆过滤和概率召回机制，在医疗对话场景中显著提升了长期记忆能力和响应自然度。


<details>
  <summary>Details</summary>
Motivation: 当前LLM记忆系统要么依赖暴力扩展上下文，要么使用静态检索管道，在边缘设备上效果不佳，特别是在纵向医疗助手等需要处理重复但时间上不同的对话场景中表现有限。

Method: 采用图结构存储、模块化内容和冗余过滤器、记忆提交和修剪机制，以及基于人类记忆建模的概率召回（包含时间衰减和刷新过程），并引入从记忆图中高效提取的"核心摘要"来捕捉用户个性等长期细节。

Result: 在纵向医疗对话实验中，Mnemosyne在盲测人类评估中获得65.8%的最高胜率（基线RAG为31.1%），在LoCoMo基准测试中在时间推理和单跳检索方面达到当前最高分，总体平均得分54.6%排名第二，优于Mem0和OpenAI等常用方法。

Conclusion: Mnemosyne证明了通过边缘兼容且易于迁移的无监督记忆架构，可以显著改善事实回忆、增强时间推理能力，并产生更自然的用户响应。
...
TL;DR: 本文提出将AI生成文本检测重新定义为离群检测问题，将人类文本视为分布外样本，机器生成文本视为分布内样本，使用单类学习和基于分数的学习方法构建检测框架。


<details>
  <summary>Details</summary>
Motivation: 现有方法将AI文本检测视为二分类问题，但人类文本并不构成统一分布，导致分类器记忆OOD特征而非学习'非ID'行为本质，限制了泛化能力。

Method: 使用DeepSVDD和HRN等单类学习方法，以及基于能量的方法等基于分数的学习技术，构建OOD检测框架。

Result: 在DeepFake数据集上达到98.3%的AUROC和AUPR，FPR95仅为8.9%。在多语言、受攻击、未见模型和领域文本设置中均表现稳健。

Conclusion: 将AI文本检测重构为OOD检测问题能显著提升检测器的鲁棒性和泛化能力，优于传统的二分类方法。
...
Result: 在YpathR基准上获得98.64%的Recall@5，比基线提升23个百分点；在YpathQA-M上，将通用和医学LLM的准确率平均提升9.0%，最高提升15.6%。
...
TL;DR: 提出了LatentBreak方法，通过替换语义等效词生成低困惑度的自然对抗提示，以规避基于困惑度的防御机制，在多个安全对齐模型上优于现有越狱算法。
...
Method: LatentBreak采用白盒攻击，在潜在空间中最小化对抗提示与无害请求表示的距离，通过替换语义等效词生成低困惑度的自然对抗提示。

Result: LatentBreak生成的提示更短且困惑度更低，在多个安全对齐模型上成功规避基于困惑度的过滤器，性能优于其他越狱算法。

Conclusion: LatentBreak证明了通过语义等效替换生成低困惑度对抗提示的有效性，为开发更强大的越狱攻击提供了新思路，同时也揭示了现有基于困惑度防御的局限性。
...
TL;DR: 本文研究了数字平台上错误信息的对抗性攻击，特别关注语言切换、查询长度膨胀和结构重构等转换方式，提出了一个多语言多智能体大语言模型框架，可作为网络插件部署到在线平台。


<details>
  <summary>Details</summary>
Motivation: 数字平台上错误信息的快速传播威胁公共话语、情感稳定和决策制定，而现有的对抗性攻击研究尚未系统性地研究特定的转换方式。

Method: 开发了一个多语言多智能体大语言模型框架，结合检索增强生成技术，研究语言切换（英语、法语、西班牙语、阿拉伯语、印地语和中文）、查询长度膨胀后摘要化以及重构为多选题等转换方式。

Result: 该框架可作为网络插件部署到在线平台，展示了在现实网络应用中插件化部署的可行性。

Conclusion: AI驱动的错误信息检测对于保护在线事实完整性对抗多样化攻击至关重要，插件化部署为实际应用提供了可行方案。
...
TL;DR: 该论文提出了一个基于情感热点的统一模型，通过检测文本、音频和视频中的情感热点，使用热点门控融合和路由混合对齐器来改善多模态情感识别性能。


<details>
  <summary>Details</summary>
Motivation: 对话中的情感识别(ERC)面临挑战，因为判别性证据稀疏、局部化且跨模态异步。需要专注于情感热点来提升识别效果。

Method: 检测每个话语在文本、音频和视频中的情感热点，通过热点门控融合将热点与全局特征融合，使用路由混合对齐器对齐模态，并构建跨模态图编码对话结构。

Result: 在标准ERC基准测试中相比强基线模型获得了一致的性能提升，消融实验确认了热点门控融合和混合对齐器的贡献。

Conclusion: 基于情感热点的视角可以为未来多模态学习提供信息，为ERC中的模态融合提供了新的视角。
...
<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在西方高资源环境外的多模态理解和推理能力下降，需要专门评估其在亚洲文化背景下的表现。

Method: 构建包含8个亚洲国家、10种语言的27,000个问题的多模态基准数据集，提出五维评估协议，并开发文化意识基础验证模块检测捷径学习。

Result: 该框架能够系统评估模型在跨国家、跨语言、跨模态的一致性以及文化知识泛化能力，并提供可操作的改进见解。

Conclusion: MMA-ASIA为构建文化可靠的多模态大语言模型提供了重要评估工具和方法论指导。
...
TL;DR: GraphGhost是一个统一框架，将神经元激活和信号传播表示为图，揭示LLMs如何从序列输入中捕获结构语义并通过结构一致机制生成输出。
...
Method: 使用图算法（如PageRank）表征LLMs特性，识别激活神经元并通过结构干预评估它们。

Result: 揭示了跨不同数据集的共享和模型特定推理行为，对关键神经元节点的编辑会引发推理崩溃。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在医疗领域的应用日益广泛，但其可能复制或放大性别相关偏见的问题尚未得到充分研究，这威胁到AI支持临床护理的公平性和可靠性。

Method: 使用新英格兰医学杂志挑战赛的案例研究，为多个开源和专有LLM分配性别（女性、男性或未指定），评估它们在LLM性别分配下对诊断和患者性别相关性的判断一致性。

Result: 大多数模型的诊断在不同LLM性别下相对一致，但所有模型在患者性别相关性和必要性判断方面都表现出显著不一致，特别是在相关性判断方面，某些模型甚至显示出系统性的女性-男性差异。

Conclusion: 这些发现揭示了一种未被充分探索的偏见，可能削弱LLM在临床实践中的可靠性，强调需要常规检查与LLM交互时的身份分配一致性，以确保可靠和公平的AI支持临床护理。
...
TL;DR: 提出了一种利用大语言模型自动生成数学应用题中干扰条件的迭代框架，该框架能保持原问题解法的有效性，大幅减少人工标注成本。


<details>
  <summary>Details</summary>
Motivation: 现有数学应用题数据集大多只包含必要信息，而带有干扰条件的问题往往被忽视。现有干扰条件数据集存在难度低、脱离上下文等问题，导致干扰条件容易被检测和忽略，降低了基准测试的可信度。

Method: 设计了一个迭代框架，利用LLM从多个角度和认知层次自动生成干扰条件，并明确引导模型生成不改变原问题解法的干扰条件，从而无需重新生成答案。

Result: 该框架高效易部署，显著减少了生成带干扰条件的数学应用题所需的工作量，同时保持了高质量的数据。

Conclusion: 提出的自动生成干扰条件框架解决了现有数据集的局限性，为更可靠地评估大语言模型的数学推理能力提供了有效工具。
...
TL;DR: 通过重评估模型在基准问题改写版本上的表现，发现LLM在改写问题上的准确率显著下降，表明基准分数可能因记忆测试项目而虚高。


<details>
  <summary>Details</summary>
Motivation: 探究大型语言模型基准分数是否因记忆测试项目或近似重复项而虚高，验证模型是否真正理解而非依赖表面形式捷径。

Method: 使用简单协议，对ARC-Easy和ARC-Challenge基准问题进行改写，在Mistral-7B-Instruct和Qwen2.5-7B-Instruct模型上比较原始问题和改写问题的准确率差距，包含解码控制、多选输出格式强制和改写清理步骤。

Result: 改写导致非平凡的准确率下降（原始vs改写），与先前关于污染和脆弱表面形式捷径的担忧一致。

Conclusion: 基准测试存在污染风险，模型可能过度依赖表面形式而非真正理解，需要更严格的评估方法来确保泛化能力。
...
TL;DR: JAI-1是一个750亿参数的泰语中心语言模型，采用上扩增策略从较小的英语模型扩展参数空间，系统整合泰语知识，在泰语基准测试中表现优于Typhoon2-70B。


<details>
  <summary>Details</summary>
Motivation: 现有泰语模型主要依赖开源模型进行额外训练，但这种方法会侵蚀模型参数空间中已有的知识。需要一种既能保留通用智能又能有效整合泰语知识的方法。

Method: 从高性能英语开源LLM出发，扩展参数空间，利用新增容量系统整合泰语知识。预训练使用1.5万亿token（含3000亿泰语token），后进行监督微调和对齐调优。

Result: 在泰语中心基准测试（IFEval-TH、MT-Bench-TH和JAI-Hall-Bench）中表现优于Typhoon2-70B，验证了上扩增和知识整合框架的有效性。

Conclusion: 上扩增策略成功保留了原始模型的通用智能，同时建立了独特的架构，为未来可扩展增强奠定了基础。
...
TL;DR: 研究销售导向对话代理如何基于用户画像（年龄、性别、职业）调整对话策略，发现职业对对话意图影响最大，提出轻量级职业条件策略提升对话效果。


<details>
  <summary>Details</summary>
Motivation: 随着智能对话模型的快速发展，需要真实的用户模拟研究来优化对话策略，特别是针对销售导向的对话系统。

Method: 引入基于用户职业的条件策略，引导代理优先选择与用户偏好一致的对话意图，实现更短更成功的对话。

Result: 年龄和性别影响整体性能，但职业对对话意图差异影响最显著；职业条件策略能显著提升对话效果。

Conclusion: 丰富的用户画像模拟很重要，简单的基于人物特征的策略能有效提升销售导向对话系统的性能。
...
TL;DR: Text2Stories提出了文本到故事对齐的任务和指标，用于量化需求（用户故事）与访谈记录中表达的实际需求的匹配程度，包括正确性和完整性两个维度。


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型可以自动从自然语言输入生成软件需求，但评估这些需求是否准确反映利益相关者需求仍然主要依赖人工。需要一种可量化的方法来评估需求与原始访谈内容的对齐程度。

Method: 将访谈记录分割成文本块，将对齐问题建模为文本块与用户故事之间的匹配问题。使用基于LLM的匹配器来实现对齐检测，同时探索嵌入模型作为有效的阻塞方法。

Result: 在四个数据集上的实验表明，基于LLM的匹配器在保留注释上达到0.86的宏F1分数，而仅使用嵌入模型效果较差但能实现有效的阻塞。

Conclusion: Text2Stories指标能够比较不同故事集（如人工生成vs自动生成），作为现有用户故事质量标准的可扩展、忠实于源内容的补充评估方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法直接将大语言模型用于提取任务，但将JSON模式视为静态的人类设计契约，导致提取性能不佳、幻觉频繁和代理行为不可靠。
...
Conclusion: PARSE通过将JSON模式视为可解释和改进的自然语言理解契约，显著提升了结构化信息提取的性能和可靠性。
...
TL;DR: 研究发现评估导向的提示会夸大LLM性能，导致更长的推理链但准确率提升有限，建议使用中性措辞和双框架检查来确保基准测试反映实际部署能力。


<details>
  <summary>Details</summary>
Motivation: 调查评估导向的提示是否夸大LLM性能而不带来相应的能力提升，因为真实部署需要简洁、合同约束的回答。

Method: 使用GPT-OSS-20B模型进行六组A/B测试，固定任务内容和解码，改变框架（评估导向vs真实世界）和推理深度，通过确定性验证器计算准确性、合规性等指标。

Result: 评估框架可靠地增加了推理链长度并降低了仅回答合规性，准确率提升有限或不一致；激励措辞会重新加权错误组成；多语言标题重现了这些特征。

Conclusion: 提供可复现的A/B框架和实用指导：中性措辞、双框架检查、合同感知评分等，确保基准测试收益反映可部署能力。
...
TL;DR: PULSE框架使用小型语言模型生成的理由作为直接学习信号，通过联合建模用户行为及其语义驱动因素，在多领域推荐任务中超越了传统ID、协同过滤和LLM模型。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型推理能力强但推理成本高，小型语言模型效率高但推荐推理能力未被充分探索。现有系统仅将自然语言理由作为无监督描述文本，未能充分利用其作为学习信号的潜力。

Method: 提出PULSE框架，创建跨领域的Thought Space，将SLM生成的理由作为直接学习信号，与交互历史联合监督，同时建模用户行为（what）和语义驱动因素（why）。

Result: 在多个基准数据集上，PULSE超越了领先的ID、协同过滤和基于LLM的顺序推荐模型，在跨领域推荐中表现出优越的迁移性，并在推理导向问答等下游任务中表现强劲。

Conclusion: PULSE通过将理由作为一等信号处理，产生了更鲁棒和可泛化的嵌入，证明了SLM在推荐系统中的潜力，为高效且有效的推荐系统提供了新方向。
...
TL;DR: 提出ExPO-HM方法，通过解释-检测策略优化解决仇恨表情包检测问题，在三个基准测试中实现最先进性能，相比基线方法提升15-17% F1分数。


<details>
  <summary>Details</summary>
Motivation: 现有仇恨表情包检测方法多为直接检测，只产生二元预测，缺乏实际审核所需的上下文和解释。最近的解释-检测方法性能甚至不如简单SFT基线。

Method: ExPO-HM结合SFT预热、GRPO与课程学习，以及条件决策熵作为推理质量度量和奖励，灵感来自人类标注员的训练评估过程。

Result: 在三个仇恨表情包基准测试中，ExPO-HM在二元检测、细粒度分类和推理质量方面均达到最先进性能，相比GRPO和DPO基线分别提升15%和17% F1分数。
...
TL;DR: HDLM是一种新的离散扩散语言模型，通过分层词汇表实现从粗粒度到细粒度的语义预测过程


<details>
  <summary>Details</summary>
Motivation: 现有扩散语言模型在语义层次建模方面存在局限，需要一种能够捕捉语言多层次语义结构的模型

Method: 使用分层词汇表，前向过程将token扰动到更高层次的抽象语义，反向过程逐步预测更详细的语义

Result: 在文本生成实验中，HDLM相比基线模型展现出更低的验证困惑度和生成困惑度

Conclusion: HDLM提供了一种通用的时变语义尺度预测框架，能够有效提升语言建模性能
...
TL;DR: UCoT是一种高效推理框架，通过前置思维嵌入实现CoT压缩，使用小模型作为压缩器和大模型作为执行器的协作工作流，在保持强大推理能力的同时显著减少CoT长度。


<details>
  <summary>Details</summary>
Motivation: 长CoT虽然能提升LLMs的推理能力，但由于自回归特性导致计算成本高、延迟大。现有CoT压缩方法要么需要繁琐的手动提示设计，要么构建外部压缩数据集而牺牲关键推理细节。

Method: UCoT采用两阶段方法：第一阶段训练压缩器生成富含推理信息的前置思维嵌入；第二阶段优化执行器利用这些嵌入进行短推理，使用奖励机制确保正确性。

Result: 在GSM8K数据集上，UCoT将Qwen2.5-7B-Instruct模型的token使用量减少50%，性能比SOTA方法高出3.08%。

Conclusion: UCoT框架在保持强大推理能力的同时显著提高了推理效率，为LLMs的高效推理提供了有效解决方案。
...
TL;DR: 提出了一个分析个人叙事风格的形式化框架，结合功能语言学、计算机科学和心理学，通过语言模型自动提取语言特征，应用于梦境叙事分析。
...
TL;DR: 该研究利用LLM分析定性文本数据，与传统评分量表结合创建增强型心理测试，在抑郁症评估中显著提升了测量精度和准确性。
...
Method: 结合LLM评分的文本数据和传统评分量表项目，创建增强型测试框架，基于项目信息计算经验性地选择最有效的LLM评分指令，在真实世界样本(n=693)和合成数据集(n=3,000)上进行验证。

Result: 在保留测试集上，增强型测试在测量精度和准确性方面实现了统计显著提升，LLM项目带来的信息增益相当于在原19项测试基础上增加6.3-16.0个项目。

Conclusion: 该框架为利用日益增长的转录文本数据增强传统心理测量提供了可扩展的方法，在临床健康等领域具有潜在应用价值，代表了绕过传统自动评分瓶颈的概念性转变。
...
Result: 在8×H800 GPU上，HumanEval基准测试达到1100+ tokens/秒，六个基准测试平均超过800 tokens/秒；相比Fast-dLLM实现10倍加速，相比优化后的自回归模型QWen2.5-3B仍有2-3倍速度优势。

Conclusion: dInfer为扩散式大语言模型提供了高效且可扩展的推理解决方案，显著提升了推理效率而不牺牲模型性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有缩放定律主要基于自然语言分析，但代码与自然语言在严格语法等方面存在根本差异，不清楚这些定律是否直接适用于代码语言模型。

Method: 进行了117次实验，模型规模从0.2B到3.8B，训练token从2B到128B，拟合Chinchilla定律和Farseer定律，并进行代码-自然语言混合实验。

Result: 代码语言模型能有效随模型规模扩展，但代码比自然语言更数据饥渴，需要更高数据参数比；Farseer定律比Chinchilla定律更准确；自然语言在资源受限时有益，但在高计算预算时反而有害。

Conclusion: 代码语言模型的缩放定律与自然语言不同，需要专门针对代码特性的缩放定律指导，且Farseer定律更适合代码建模。
...
<details>
  <summary>Details</summary>
Motivation: 研究LLMs在法律案例推理中的能力，这种基于案例的推理是美国法律实践的核心，需要从先例中类比和区分。

Method: 提出了一个形式化框架，将案例区分过程分解为三阶段推理任务：使用事实谓词建模案例、构建法律知识层次结构、定义可验证的区分规则。

Result: LLMs在表面推理（任务1）上准确率高，但在层次推理（任务2：64.82%-92.09%）和综合分析（任务3：11.46%-33.99%）上性能显著下降，且错误回答比正确回答消耗更多计算资源。

Conclusion: 研究揭示了LLMs在复杂推理任务中的基本局限性，为细粒度分析LLM推理能力提供了方法学，指出了构建稳健可信法律AI必须解决的关键问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准存在计算成本高、分数膨胀和偏向简单错误而非罕见关键故障的问题，需要更有效的测试用例评估方法。
...
Conclusion: 该框架能够构建紧凑、多样且抗膨胀的基准，有效评估LLM生成的测试用例的诊断能力。
...
TL;DR: 微基准测试在语言模型开发中效率高但可靠性存疑。研究发现现有微基准方法无法在少量样本下一致地排名模型，需要250个样本才能可靠比较相似性能的模型，此时随机采样已具有竞争力。


<details>
  <summary>Details</summary>
Motivation: 解决语言模型开发中全基准测试时间成本过高的问题，评估微基准测试是否能像完整基准一样一致地排名模型，以及是否优于随机数据点选择。

Method: 引入元评估方法，分析微基准在不同模型性能差异下的排名一致性，研究微基准规模与可靠性之间的权衡关系。

Result: 现有微基准方法在10个样本时无法一致排名MMLU-Pro上3.5分差异或BIG-bench Hard上4分差异的模型对；需要250个样本才能可靠比较相似性能模型，此时随机采样与专门方法相当；在25个样本的MMLU-Pro微基准上，超过一半的8B指令调优模型对比结果不可靠。

Conclusion: 微基准测试在效率和可靠性间存在权衡，需要谨慎选择样本数量，为微基准用户和开发者提供实用指导。
...
TL;DR: 本文提出了一种基于LLM的组合位置引用地理编码方法，通过评估LLM的地理空间知识与推理能力，开发出能够有效处理组合位置引用的策略，且小型微调LLM能达到与大型现成模型相当的性能。


<details>
  <summary>Details</summary>
Motivation: 地理编码是将位置引用链接到实际地理位置的任务，对非结构化文本的下游分析至关重要。本文探索组合位置引用地理编码这一挑战性场景，基于LLM在地理空间数据推理方面的能力进行研究。

Method: 评估LLM的地理空间知识与推理技能，提出基于LLM的组合位置引用地理编码策略，包括使用相对较小的微调LLM来实现任务。

Result: 该方法提高了组合位置引用地理编码任务的性能，小型微调LLM能够达到与大型现成模型相当的表现。

Conclusion: 基于LLM的策略能有效改善组合位置引用的地理编码性能，且小型专用模型可以替代大型通用模型，为实际应用提供了高效解决方案。
...
TL;DR: 评估前沿和领先开源模型在低资源和高资源语言中的五个维度表现，发现GPT-5在准确性和一致性方面表现最佳，而其他模型在不同语言和类别间存在较大差异。


<details>
  <summary>Details</summary>
Motivation: 随着LLM在全球多语言环境中的广泛应用，需要理解和防护其多语言响应，确保在不同语言背景下的准确性和一致性。

Method: 使用五点评分标准和法官LLM，评估模型在五个维度上的多语言响应表现，涵盖低资源和高资源语言。

Result: GPT-5在所有类别中平均表现最佳，特别是在同意与自主权（3.56）和伤害预防与安全（4.73）类别得分最高；Gemini 2.5 Pro表现最差，相应类别得分仅为1.39和1.98。

Conclusion: 研究强调了语言变化对LLM响应的影响，需要在不同类别中进一步测试和改进多语言能力。
...
TL;DR: 提出Adaptive Retention方法，通过概率性、分层token选择机制，在严格全局预算下学习保留哪些表示，解决Transformer注意力二次方复杂度问题，实现长上下文效率提升。


<details>
  <summary>Details</summary>
Motivation: Transformer注意力机制随着序列长度呈二次方增长，限制了长上下文应用。需要一种方法来减少计算和内存开销，同时保持模型性能。

Method: 使用伯努利门控建模token保留，通过Hard-Concrete/变分松弛训练，在推理时采用简单的top-M规则，使方法可微分且可直接替换标准编码器。

Result: 在分类、抽取式问答和长文档摘要任务中，仅保留30-50%的token就能保持≥95%的完整模型性能，峰值内存减少约35-45%，吞吐量提升最高约1.8倍。
...
TL;DR: 提出了中文常识多跳推理基准CCMOR，用于评估LLMs在中文语境下整合事实知识与多步逻辑推理的能力，发现LLMs在处理长尾知识和知识密集型推理方面存在局限，但检索增强生成能显著改善性能。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型展现出先进的推理能力，但在通用中文语境下的全面评估仍然不足，需要填补这一研究空白。

Method: 首先从现有QA数据集中构建领域平衡的种子集，然后开发基于LLM的流水线生成基于事实单元链的多跳问题，并通过人工参与验证系统确保数据质量。

Result: 评估发现LLMs在处理长尾知识和执行知识密集型推理方面存在持续局限，但检索增强生成能显著缓解知识差距并带来性能提升。

Conclusion: CCMOR基准揭示了LLMs在中文语境下的知识推理局限性，检索增强是有效的改进方向。
...
TL;DR: MOSAIC是一个用于解决科学编程任务的多智能体LLM框架，通过学生-教师范式和整合上下文窗口来提升科学代码生成的准确性、鲁棒性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 科学工作流需要严谨的算法、深厚的领域知识和特定领域的推理，而现有方法在处理需要解决一系列子问题的复杂科学任务时存在局限性。

Method: 采用免训练的多智能体框架，设计专门的智能体进行自我反思、创建原理、编码和调试，使用学生-教师范式和整合上下文窗口来减少幻觉。

Result: 在科学编程基准测试中，MOSAIC在准确性、鲁棒性和可解释性方面优于现有方法。

Conclusion: MOSAIC框架通过多智能体协作和上下文整合，有效解决了科学代码生成中的挑战，为复杂科学任务提供了更好的解决方案。
...
TL;DR: 本文研究发现语言结构显著影响LLMs的隐私泄露风险，意大利语泄露最强，英语成员推断分离性最高，法语和西班牙语因形态复杂度高而更具韧性。


<details>
  <summary>Details</summary>
Motivation: LLMs在多语言应用中处理敏感数据时存在隐私风险，但现有研究主要针对英语，需要探究语言结构如何影响隐私泄露。

Method: 量化六个语言指标，评估三种攻击向量：提取、反事实记忆和成员推断，分析英语、西班牙语、法语和意大利语医学语料库训练的LLMs。

Result: 隐私漏洞与语言冗余和分词粒度相关：意大利语泄露最强，英语成员推断分离性最高，法语和西班牙语因形态复杂度高而更具韧性。
...
TL;DR: 提出Search-on-Graph框架，让LLM通过单一Search函数在知识图谱上进行迭代导航，解决多跳问答中的知识获取问题。


<details>
  <summary>Details</summary>
Motivation: LLM在知识密集型多跳问答中存在可靠性问题：遗漏长尾事实、产生幻觉、知识滞后。现有KGQA方法面临查询编译困难、子图检索噪声、搜索空间指数级增长等权衡问题。

Method: 采用"观察-导航"原则，在每个步骤中LLM检查当前实体的可用关系，然后决定下一跳。使用单一Search函数进行迭代图导航，自适应处理不同KG模式和高度数节点。

Result: 在六个KGQA基准测试中达到最先进性能，无需微调。在Wikidata基准上提升16%，在Freebase基准上也有持续改进。

Conclusion: SoG框架简单有效，通过迭代图导航解决了传统KGQA方法的局限性，在不同知识图谱上均表现出色。
...
TL;DR: 提出了PE-CoA框架，通过五种对话模式构建有效的多轮越狱攻击，在12个LLM和10个危害类别上实现了最先进的性能，揭示了模式特定的漏洞和LLM行为特征。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型仍然容易受到多轮越狱攻击的威胁，现有方法依赖启发式探索策略，对模型底层弱点理解有限，对话模式与跨危害类别漏洞之间的关系仍不清楚。

Method: 提出了模式增强攻击链（PE-CoA）框架，包含五种对话模式来通过自然对话构建有效的多轮越狱攻击。

Result: 在12个LLM和10个危害类别上的评估显示PE-CoA达到了最先进性能，发现了模式特定漏洞和LLM行为特征：模型表现出不同的弱点分布，对一个对话模式的鲁棒性不能推广到其他模式，模型家族共享相似的失败模式。

Conclusion: 这些发现突出了安全训练的局限性，表明需要模式感知的防御措施。
...
TL;DR: 本文评估了质量估计重排序在文档级机器翻译中的应用，发现使用SLIDE和GEMBA-DA等质量评估指标能显著提升翻译质量，特别是在使用更多候选翻译时效果更明显。


<details>
  <summary>Details</summary>
Motivation: 质量估计重排序在句子级翻译中已被证明有效，但在日益重要的文档级翻译领域应用不足，需要探索其在该领域的表现。

Method: 使用各种学习型和基于大语言模型的质量评估指标（如SLIDE、GEMBA-DA），在文档级翻译场景下对生成的候选翻译进行评分和选择。

Result: 使用最佳学习型指标SLIDE时，BLEURT-20分数在2个候选翻译时提升+2.00，在32个候选翻译时提升+5.09；使用最佳LLM指标GEMBA-DA时，分别提升+1.63和+4.30。
...
<details>
  <summary>Details</summary>
Motivation: GAAP的复杂性和XBRL的分层结构使得财务审计难以自动化和验证，而LLMs在结构化、相互依赖的财务文档上的推理能力尚未充分探索。

Method: 从真实的US-GAAP兼容XBRL文件中构建FinAuditing基准，定义三个互补子任务（FinSM、FinRE、FinMR），并提出集成检索、分类和推理指标的统一评估框架。

Result: 对13个最先进LLMs的零样本实验显示，当前模型在语义、关系和数学维度上表现不一致，在分层多文档结构推理时准确率下降60-90%。

Conclusion: 现代LLMs在基于分类法的财务推理中存在系统性局限，FinAuditing为开发可信赖、结构感知和法规对齐的财务智能系统奠定了基础。
...
TL;DR: 提出了一种在强化学习中为不同token类型应用不同温度设置的方法，通过提高推理token的温度来促进探索，同时保持知识token的低温以确保事实准确性。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常通过限制更新来间接鼓励探索，但未在token生成阶段显式促进探索行为。研究发现LLM中的token在推理任务中扮演不同角色，可分为高熵推理token和低熵知识token。

Method: 在采样过程中为不同token类型应用不同温度设置：对推理token使用较高温度以积极鼓励探索，对知识token保持较低温度以维持事实正确性。系统研究了多种多温度调度策略及其在强化学习环境中的影响。

Result: 在多个推理基准测试上的实证评估表明，该方法显著提升了LLM的推理性能。

Conclusion: 通过显式地在token生成阶段促进探索，采用多温度策略能够有效提升LLM在强化学习环境中的推理能力。
...
TL;DR: 提出基于大语言模型的统一生物医学命名实体识别框架，通过符号标注策略处理嵌套实体，采用双语联合微调增强跨语言泛化能力，引入对比学习实体选择器过滤错误预测。
...
Method: 将BioNER重构为文本生成任务，设计符号标注策略处理扁平与嵌套实体；进行中英文数据集的双语联合微调；引入基于对比学习的实体选择器过滤错误预测。

Result: 在四个基准数据集和两个未见语料库上的实验结果表明，该方法实现了最先进的性能，并具有强大的跨语言零样本泛化能力。

Conclusion: 提出的统一框架在生物医学命名实体识别任务上表现出色，特别是在处理嵌套实体和跨语言泛化方面具有显著优势。
...
TL;DR: SAC是一种新的上下文压缩方法，通过直接选择锚点令牌并聚合上下文信息到其KV表示中，避免了自编码训练，在多种压缩比下优于现有方法。
...
Method: SAC直接选择原始上下文中的锚点令牌，将上下文信息聚合到其KV表示中，包含锚点嵌入和双向注意力修改两个关键设计。

Result: SAC在多种压缩比下持续优于现有方法，在MRQA的分布外评估中，5倍压缩时比强基线提升1 EM，在更高压缩比下优势更明显。

Conclusion: SAC通过消除自编码训练需求，直接利用锚点令牌进行上下文压缩，在性能上显著优于现有方法，特别是在高压缩比下表现更佳。
...
TL;DR: 该论文研究LLM内部表示中类似人类印象和刻板印象的模式，使用刻板印象内容模型(SCM)的线性探针来解码这些人工印象，并分析其对模型行为的影响。
...
Method: 使用SCM二维模型的线性探针，在生成的提示上拟合预测印象，从LLM隐藏表示中线性解码人工印象。

Result: 发现LLM在提示时不一致地报告印象，但从隐藏表示中能更一致地线性解码印象；人工印象能预测模型回答的质量和模糊语使用；提示中的内容、风格和方言特征会影响LLM印象。

Conclusion: LLM内部存在可解码的人工印象模式，这些印象影响模型行为，提示特征会塑造这些印象形成。
...
TL;DR: 提出了SOP-Maze基准，用于评估大语言模型在复杂标准操作程序(SOP)场景中的表现，包含397个任务，涵盖23个真实业务场景，识别了模型在流程遵循、对话处理和计算推理方面的主要问题。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要评估LLMs的指令遵循和决策能力，但缺乏对复杂业务场景中标准操作程序(SOP)的专门评估，需要填补这一空白。

Method: 基于真实业务数据构建SOP-Maze基准，将397个任务分为横向根系统(LRS)和核心根系统(HRS)两类，分别评估宽选项选择和深度逻辑推理能力。

Result: 实验表明几乎所有最先进模型在SOP-Maze上都表现不佳，主要存在路径盲区、对话脆弱性和计算错误三类问题。

Conclusion: SOP-Maze基准揭示了LLMs在复杂业务流程中的局限性，为提升模型能力提供了新见解，该工作已开源。
...
<details>
  <summary>Details</summary>
Motivation: 研究开源社区治理文档中参与和控制的描述方式，为评估未来AI中介工作流是否集中或重新分配权力提供可复现基准。

Method: 使用710个项目的配对快照语料库，将文本解析为参与者、规则、行动和对象，通过熵、丰富度和Jensen-Shannon散度测量变化。

Result: 项目随时间定义更多角色和行动，且分布更均衡，而规则组成保持稳定，治理通过扩展和平衡参与类别而增长。

Conclusion: 治理通过扩展和平衡参与类别而增长，没有重大规范性转变，为评估AI工作流对权力的影响提供了基线。
...
TL;DR: CAPC-CG是中国首个开放的政策指令数据集，包含1949-2023年中国中央政府的法律法规，标注了清晰与模糊语言的五色分类法，包含330万个段落单元。
...
Method: 构建包含国家法律、行政法规和部门规章的语料库，采用两轮标注框架，由专家和训练有素的标注员进行标注，使用五色分类法对政策语言进行分类。

Result: 标注者间一致性达到Fleiss's kappa K=0.86，表明标注可靠性高；提供了多个大语言模型的基线分类结果和完整的元数据。

Conclusion: 该数据集的发布旨在支持下游任务和多语言NLP研究，为政策沟通分析提供可靠的数据基础。
...
<details>
  <summary>Details</summary>
Motivation: 自动形式化在连接自然语言和形式推理中起着关键作用，需要一种能够适应快速发展的领域并集成新工具的系统。

Method: 采用多智能体系统架构，利用大语言模型驱动协作智能体进行自然语言到形式化表示的转换，支持新智能体和工具的无缝集成。

Result: 在真实世界数学定义和形式数学数据集上的实验证明了MASA的有效性。

Conclusion: 这项工作展示了基于大语言模型和定理证明器交互的多智能体系统在提高自动形式化效率和可靠性方面的潜力，为研究人员和从业者提供了有价值的见解和支持。
...
TL;DR: 本文提出DARO方法，通过动态调整不同难度组的损失贡献来解决现有RLVR方法中的损失尺度问题，在多个数学基准测试中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有GRPO及其变体方法依赖静态或过于简化的权重方案，无法适应模型能力的演变，导致训练在不同难度级别上失衡，阻碍整体性能提升。

Method: 提出难度感知重加权策略优化（DARO），根据模型学习状态动态调整每个难度组的损失贡献。

Result: 在Qwen2.5-Math-1.5B、Qwen2.5-Math-7B和Llama3.1-8B上的实验表明，DARO在六个数学基准测试中优于四个领先基线，收敛速度更快且最终性能更优。

Conclusion: DARO通过动态难度感知重加权机制有效解决了现有RLVR方法的局限性，实现了更好的训练效果和性能提升。
...
Method: Using LoRA-based Refusal-training trained solely on safety data, which decouples safety into a low-rank subspace orthogonal to the model's intrinsic capabilities.

Result: Demonstrates that LoRA serves as cost-efficient, performance-preserving, and plug-and-play safety patches without degrading general performance.

Conclusion: LoRA effectively enables safety alignment while preserving model capabilities by decoupling safety enhancements into orthogonal subspaces.
...
TL;DR: LitE-SQL是一个轻量高效的Text-to-SQL框架，通过模式检索器和两阶段微调的SQL生成器，在保持高性能的同时大幅减少参数量，解决了基于大语言模型方法的部署可行性和数据隐私问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的Text-to-SQL方法依赖专有模型，存在部署可行性和数据隐私问题，需要开发更轻量高效的解决方案。

Method: LitE-SQL包含两个组件：使用向量数据库进行高效模式链接的模式检索器，以及通过监督微调和执行引导强化学习两阶段微调的SQL生成器，支持自我修正而无需昂贵的多候选生成。

Result: 在BIRD数据集上达到72.10%执行准确率，在Spider 1.0上达到88.45%，性能与基于大语言模型的方法相当或更优，同时参数量减少2-30倍。

Conclusion: 高质量Text-to-SQL生成可以使用轻量模型实现，为隐私敏感和资源受限场景提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型的性能对提示词高度敏感，受提示优化领域启发，本研究探索通过优化评分标准来改进自动作文评分系统。

Method: 通过让模型反思自身评分理由与人工评分差异，迭代优化评分标准，在TOEFL11和ASAP数据集上使用GPT-4.1、Gemini-2.5-Pro和Qwen-3-Next-80B-A3B-Instruct进行实验。

Result: 在TOEFL11和ASAP数据集上分别实现了高达0.19和0.47的QWK改进，即使使用简单初始评分标准也能达到或优于详细人工编写标准的性能。

Conclusion: 迭代评分标准优化对于基于LLM的自动作文评分至关重要，能显著提升与人工评估的一致性。
...
TL;DR: 该研究为低资源语言查克玛语构建了孟加拉语转写的语料库，并通过微调多语言Transformer模型在掩码语言建模任务上取得了显著性能提升，证明了转写方法对低资源语言迁移学习的有效性。


<details>
  <summary>Details</summary>
Motivation: 查克玛语作为一种印度-雅利安语言，可用数据有限，在语言模型中代表性不足。研究旨在解决这一低资源语言的处理问题。

Method: 从查克玛文学中构建上下文连贯的孟加拉语转写查克玛语语料库，经母语者验证后，在六个编码器型多语言和区域Transformer模型上进行掩码语言建模微调。

Result: 微调后的多语言模型在孟加拉语转写查克玛语上表现优于预训练模型，最高达到73.54%的token准确率和低至2.90的困惑度。

Conclusion: 孟加拉语转写查克玛语对查克玛语的迁移学习非常有效，研究发布了手动验证的单语数据集以促进低资源语言的多语言建模研究。
...
TL;DR: LLMs的内部状态无法可靠区分事实和幻觉，因为与主体知识相关的幻觉会使用与正确回答相同的内部召回过程，导致隐藏状态几何重叠且无法区分。


<details>
  <summary>Details</summary>
Motivation: 探索LLMs内部计算是否能可靠区分事实性和幻觉输出，因为LLMs既编码事实性信号，也会产生基于捷径或虚假关联的事实错误。

Method: 对LLMs处理事实查询进行机制分析，比较基于主体知识依赖的两种幻觉类型，分析隐藏状态几何特征。

Result: 与主体知识相关的幻觉使用与正确回答相同的内部召回过程，导致隐藏状态几何重叠；而与主体知识无关的幻觉产生可区分的聚类表示。
...
TL;DR: 提出了Alif-1.0-8B-Instruct，一个乌尔都语-英语多语言模型，通过改进的自指导技术训练高质量合成数据集，在低预算下实现了对乌尔都语任务的优异性能


<details>
  <summary>Details</summary>
Motivation: 解决低资源语言（如乌尔都语）开发大语言模型面临的挑战，包括高质量数据集稀缺、多语言不一致性和安全问题

Method: 使用改进的自指导技术创建高质量多语言合成数据集（Urdu-Instruct），包含乌尔都语本地思维链推理、双语翻译、文化相关性和伦理安全对齐
...
Conclusion: 通过改进的自指导方法，可以高效开发高性能、低资源语言的大语言模型，并实现文化对齐
...
TL;DR: ReFIne是一个新的训练框架，通过监督微调和GRPO结合，提升推理模型的可信度，包括可解释性、忠实性和可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有长链思维推理研究过于关注准确性和效率，而忽视了可信度这一关键维度。可用的推理系统必须具备可信赖性。

Method: 提出ReFIne训练框架，结合监督微调和GRPO，鼓励模型：生成结构化标签追踪、明确披露决策信息、提供自我评估。

Result: 在多个数学基准测试中，ReFIne模型生成更清晰的结构化推理追踪（可解释性+44.0%），更忠实地暴露决策过程（忠实性+18.8%），并提供信息丰富的置信度估计（可靠性+42.4%）。

Conclusion: 推理模型不仅应优化准确性，还应优化可信度的更广泛维度，这是一个被忽视但重要的研究方向。
...
<details>
  <summary>Details</summary>
Motivation: 虽然因果语言模型（如GPT、Llama系列）在多种语言理解任务中表现出色，但尚未应用于语义框架归纳任务。研究者希望探索CLM在此任务上的潜力，特别是对于资源匮乏的语言如日语。

Method: 提出FrameEOL方法：使用提示工程获取框架嵌入，输出单个框架名称标签；结合上下文学习（ICL）和深度度量学习（DML）优化嵌入质量；通过聚类实现框架归纳。

Result: 在英语和日语FrameNet数据集上的实验表明，该方法优于现有框架归纳方法。特别是对于日语，仅使用5个ICL示例的CLM方法就达到了与使用DML微调的MLM方法相当的性能。

Conclusion: CLM在语义框架归纳任务中具有强大潜力，FrameEOL方法为资源匮乏语言的框架归纳提供了有效解决方案，证明了CLM在此任务上的竞争力。
...
TL;DR: 本文对检索增强生成(RAG)进行了全面综述，分析了RAG的目标、组件、挑战和应用，指出在LLM能力不断提升的背景下需要重新思考RAG的角色。


<details>
  <summary>Details</summary>
Motivation: 由于LLM在静态语料上训练，难以处理快速演变的信息或领域特定查询，RAG通过集成外部检索机制来克服这一限制。但随着LLM能力的提升，传统RAG框架的相对优势变得不那么明显。

Method: 通过全面综述RAG的总体目标、核心组件，分析RAG中的关键挑战和限制其有效性的关键弱点。

Result: 展示了LLM单独表现不佳但结合RAG能显著提升效果的多个应用场景。

Conclusion: 这项工作旨在鼓励研究人员重新思考RAG的角色，并激发下一代RAG系统的开发。
...
TL;DR: 提出了DITING评估框架，专门针对网络小说翻译，包含六个维度的评估指标，并开发了AgentEval多智能体评估框架，在14个模型测试中发现中文训练的LLM表现优于更大的国外模型。


<details>
  <summary>Details</summary>
Motivation: 现有机器翻译评估基准主要依赖表面指标，无法有效评估网络小说翻译的特有特征，需要专门针对该领域的评估框架。

Method: 构建DITING评估框架，包含成语翻译、词汇歧义、术语本地化、时态一致性、零代词解析和文化安全六个维度，基于18K专家标注的中英句子对；提出AgentEval多智能体评估框架模拟专家审议；开发MetricAlign元评估数据集。

Result: 中文训练的LLM在翻译质量上超越更大的国外模型，DeepSeek-V3在忠实度和风格一致性方面表现最佳；AgentEval在七个自动评估指标中与人工判断相关性最高。
...
TL;DR: 通过使用思考出声话语增强对话数据，训练能更好模拟人类个性的LLM，在Big Five人格特质中的宜人性和神经质方面表现更接近真实说话者


<details>
  <summary>Details</summary>
Motivation: 现有LLM在模拟个体人格特质方面存在不足，希望通过思考出声话语来更好地捕捉说话者的个性特征

Method: 使用思考出声话语增强对话数据来训练人格LLM，并基于Big Five人格框架评估模型表现

Result: 使用TAU增强数据训练的LLM在宜人性和神经质两个维度上比使用原始对话数据训练的模型更接近说话者真实人格

Conclusion: 思考出声话语能有效提升LLM模拟人类个性的能力，且TAU增强质量对模型性能有重要影响
...
TL;DR: 本文提出了两种增强文本去标识化攻击的方法：(1) 通过聚合多个PII识别顺序的预测来提高结果；(2) 使用推理模型在拥有广泛背景知识时提升重新识别性能。
...
Method: 提出了两种策略：一是考虑PII识别顺序的影响，通过聚合多个顺序的预测来改进结果；二是使用推理模型，在拥有大量背景知识时增强重新识别能力。

Result: 这些方法能够构建更强的重新识别攻击，有效评估去标识化方法的鲁棒性。

Conclusion: 通过考虑识别顺序和使用推理模型，可以显著提升重新识别攻击的效果，从而更好地评估文本去标识化方法的有效性。
...
TL;DR: 提出了一种新的翻译增强方法，通过指令模型和层选择性调优在平行数据上训练，开发了Qwen3-XPlus模型，在翻译性能上显著提升，同时在多语言任务中保持推理能力。


<details>
  <summary>Details</summary>
Motivation: 通用大语言模型擅长推理，但专门用于翻译的模型在推理任务上表现不佳。为了解决这个问题，需要一种方法既能提升翻译性能，又不损害原有的推理能力。

Method: 采用翻译增强的流程：从指令模型开始，仅在平行数据上应用层选择性调优。这种方法训练复杂度低，只需要小型平行数据集。

Result: Qwen3-XPlus模型在高低资源语言翻译上都取得显著改进，在低资源语言如斯瓦希里语上达到15+ spBLEU和40+ xComet分数。在7个多语言任务上平均提升1+分，同时在15个流行推理数据集上保持与Qwen3指令模型相当的能力。

Conclusion: 这项工作为多语言增强提供了一种有前景的方法，显著降低了复杂性，提高了更多语言的可及性。代码和模型已公开。
...
TL;DR: DICE是一个轻量级框架，通过小语言模型(SLMs)来修正大语言模型(LLMs)的输出，使其符合结构化输出规范，同时保持LLMs的推理能力。
...
Method: DICE采用两阶段方法构建结构化思维链适应数据集，然后使用双重调优策略微调SLMs，以分析-回答模式生成结构化输出。

Result: 实验表明DICE将LLM输出的格式准确性和内容正确性分别提高了35.4%和29.4%，达到了最先进的性能。

Conclusion: DICE框架能够有效平衡LLMs的推理能力与输出格式要求，提供了一种轻量级且高效的解决方案。
...
Method: IRIS框架从初始变量开始，自动收集相关文档、提取变量、发现因果关系。采用混合方法结合统计算法和LLM，并包含缺失变量提议组件来扩展因果图。

Result: 该方法能够实现仅从初始变量出发的实时因果发现，无需预先存在的数据集。

Conclusion: IRIS框架解决了传统方法和LLM方法的局限性，能够发现已知和新的因果关系，并实时扩展因果图。
...
TL;DR: 本文提出了CrisiText数据集，这是首个用于在13种危机场景下生成警告信息的大规模数据集，包含超过40万条警告信息，旨在通过NLG技术协助危机应对。


<details>
  <summary>Details</summary>
Motivation: 在危机情况下有效识别威胁并减轻潜在损害至关重要，但目前NLP技术在危机应对中的应用有限，主要集中在分类任务上，而使用NLG架构及时生成警告信息的潜力被严重忽视。

Method: 从现有危机描述出发创建事件链，为每个事件配对警告信息，遵循专家指南确保术语正确性和事实性。每个消息还附带三种次优警告类型，以研究不同NLG方法。

Result: 进行了一系列实验，比较了监督微调设置与偏好对齐、零样本和少样本方法，评估了模型在分布外场景下的性能以及自动后编辑器的有效性。

Conclusion: CrisiText数据集填补了危机场景下警告信息生成研究的空白，为开发更有效的危机应对NLG系统提供了重要资源。
...
Method: 引入动态过滤序列级策略优化（DSPO），通过纯RL训练模型进行多轮搜索和推理交织，无需监督演示数据。

Result: 在多个QA基准测试中，DSPO训练的7B模型比同类先前工作提升34.1%，在复杂多跳QA如HotpotQA上甚至超越先前工作的14B模型近9%。

Conclusion: DSPO算法能够有效增强LLM的外部知识搜索能力，在复杂任务中表现出色且训练稳定性优异。
...
<details>
  <summary>Details</summary>
Motivation: RL后训练阶段缺乏专门的数据污染检测方法，而该阶段对LLM推理能力提升至关重要，现有方法无法有效检测此阶段的污染问题。

Method: Self-Critique方法基于RL后训练导致模型输出熵分布崩溃的关键观察，通过探测底层策略崩溃来检测污染样本。

Result: 在多个模型和污染任务上，Self-Critique显著优于基线方法，AUC提升高达30%，而现有方法接近随机猜测水平。

Conclusion: 该方法首次使RL阶段污染检测成为可能，填补了该领域的重要研究空白。
...
TL;DR: 提出了CFVBench基准测试，评估多模态检索增强生成模型在视频理解中的表现，发现现有模型在捕捉细粒度多模态细节方面存在瓶颈，并提出自适应视觉优化框架AVR来提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有视频MRAG基准测试在模态覆盖和格式多样性方面有限，主要关注单模态或有限模态任务，或粗粒度的场景理解，需要更全面的评估框架。

Method: 构建CFVBench基准测试，包含599个公开视频和5,360个开放式QA对，涵盖图表密集报告、新闻广播和软件教程等高密度格式和领域。提出自适应视觉优化(AVR)框架，自适应增加帧采样密度并在必要时选择性调用外部工具。

Result: 系统评估了7种检索方法和14个常用MLLM，发现当前模型（包括GPT5和Gemini）在捕捉瞬态但关键的细粒度多模态细节方面存在困难。AVR框架一致地增强了细粒度多模态理解能力，提升了所有评估MLLM的性能。

Conclusion: 细粒度多模态细节的捕捉是当前MLLM的关键瓶颈，提出的AVR框架通过自适应视觉优化有效缓解了这一问题，为提升多模态检索增强生成能力提供了有效解决方案。
...
TL;DR: DyReMe是一个动态医学诊断基准，通过生成类似真实咨询的病例来评估大语言模型，比静态基准更能反映临床实践。


<details>
  <summary>Details</summary>
Motivation: 当前基于静态医学考试题目的评估与真实临床实践存在偏差，无法体现现实世界中模糊多变的情况。

Method: 生成新鲜病例，引入干扰因素如鉴别诊断和常见误诊因素，并评估准确性、真实性、帮助性和一致性四个维度。

Result: 动态评估方法更具挑战性和现实性，揭示了最先进LLM与真实临床实践之间的显著不匹配。
...
TL;DR: CLARity是一个成本效益高的强化学习框架，使用小型通用LLM来增强专家模型在稀缺数据领域的推理质量，通过一致性感知奖励机制和两阶段训练流程提升推理一致性。


<details>
  <summary>Details</summary>
Motivation: 在数据稀缺领域训练专家LLM很困难，通常依赖选择题。但标准的基于结果的强化学习虽然可能提高准确性，但往往会降低推理质量（如逻辑一致性）。现有监督推理的解决方案（如大规模过程奖励模型）成本过高。

Method: CLARity框架包含一致性感知奖励机制、两阶段精炼-监控训练流程（增强推理一致性），以及动态数据重构策略（更好地利用有限数据）。使用小型通用LLM来指导专家模型。
...
TL;DR: DualCSE提出了一种双向量句子嵌入方法，为每个句子分配显式和隐式语义两个嵌入向量，在共享空间中并存，可根据下游任务需求选择合适语义。
...
Conclusion: DualCSE通过双向量表示克服了传统单向量句子嵌入的局限性，能够更好地捕捉句子语义，提升下游应用效果。
...
TL;DR: 提出MaP框架来解决LLM预训练评估中的不稳定性问题，通过检查点合并和Pass@k指标来平滑参数空间和降低评估方差。
...
Result: 实验表明MaP能产生更平滑的性能曲线，降低运行间方差，确保更一致的模型排名。
...
TL;DR: ShiZhi是首个专门用于法院观点生成的大语言模型，在中文法院观点生成数据集CCVG上取得了58.5 BLEU-1的生成效果和86.1%的罪名预测准确率。
...
TL;DR: MaskKV是一个专为扩散大语言模型设计的训练无关缓存淘汰框架，通过掩码查询引导的评分机制和自适应缓存预算策略，显著减少KV缓存内存占用，在仅保留256对KV缓存（少于5%令牌）的情况下仍能保持94%性能，并实现31倍加速。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型的双向注意力缓存机制需要大量内存，限制了在资源受限环境下处理长上下文的能力。现有的缓存淘汰策略是为自回归模型设计的，忽略了dLLMs的独特特性，导致性能不佳。

Method: MaskKV包含两个关键创新：(1) 掩码查询引导的评分机制，利用注意力权重识别并淘汰每个注意力头中不太重要的提示令牌；(2) 自适应缓存预算策略，减少中间层的缓存分配，将资源集中在偏好提示的注意力头上。

Result: 在LLaDA模型上，将KV缓存压缩到仅256对（少于5%令牌）时，在LongBench上仍能保持94%的全缓存性能，在32k提示长度下实现高达31倍的加速。

Conclusion: MaskKV有效解决了dLLMs中的缓存内存问题，通过专门设计的缓存淘汰策略在保持高性能的同时显著提升效率，为dLLMs在资源受限环境中的应用提供了可行方案。
...
TL;DR: 提出了一种基于电路推理验证的白盒方法，通过分析正确和错误推理步骤的属性图结构特征来识别推理错误，相比黑盒和灰盒方法能提供更深入的错误原因分析。


<details>
  <summary>Details</summary>
Motivation: 现有的CoT验证方法（黑盒或灰盒）只能预测推理正确性，但无法解释计算失败的原因，需要一种能深入理解推理失败机制的方法。

Method: 使用基于电路推理验证的白盒方法，训练分类器来识别正确和错误推理步骤在属性图结构特征上的差异，将属性图视为潜在推理电路的执行轨迹。

Result: 该方法能有效预测推理错误，发现错误签名具有高度领域特异性，且通过针对性干预能成功纠正模型的错误推理，证明该方法具有因果理解能力。

Conclusion: 通过深入分析模型的计算过程，可以从简单的错误检测转向对LLM推理的更深层次因果理解，白盒方法为理解推理失败提供了新视角。
...
TL;DR: FLRC是一种细粒度低秩压缩方法，通过逐层优化秩分配和渐进式低秩解码，在减少LLM参数的同时保持文本生成质量。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型参数过多难以在资源受限硬件上部署，现有低秩压缩方法采用统一压缩比会导致性能显著下降，且在解码阶段表现不佳。

Method: 提出FLRC方法：1）为每层确定最优秩分配；2）采用渐进式低秩解码策略来维持文本生成质量。

Result: 在多个基准测试中，FLRC在摘要任务上的ROUGE-L指标比现有最优低秩压缩方法提升高达17%，建立了更鲁棒高效的LLM推理框架。
...
TL;DR: LLP是一个基于大语言模型的二手产品定价框架，通过检索相似产品和两阶段优化方法，显著提升了定价准确率和采纳率。
...
Method: 使用LLM框架，先检索相似产品，然后通过监督微调(SFT)和组相对策略优化(GRPO)两阶段优化，并采用基于置信度的过滤机制。

Result: 在闲鱼平台部署后，在30%产品覆盖率下将静态采纳率从40%提升到72%，在90%召回率下仍保持47%的采纳率。

Conclusion: LLP框架显著优于现有方法，能很好地泛化到未见过的类别，在实际应用中表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 当前对小语言模型(SLMs)的评估主要依赖最终答案的准确性，忽视了推理过程的有效性，可能导致对模型能力的过高估计。

Method: 创建专家标注的数据集ReTraceQA，使用大型语言模型(LLMs)作为自动评判器进行推理感知评估，而不仅仅是答案匹配。

Result: 研究发现14-24%的情况下SLMs给出正确答案但推理过程有缺陷；使用推理感知评估时，SLM性能在所有模型和数据集上显著下降，降幅高达25%。

Conclusion: 仅依赖最终答案准确性的评估方法会高估SLMs的能力，需要引入过程级评估来更准确地衡量模型的推理能力。
...
TL;DR: ThinkLogit是一种无需训练的解码时方法，通过logit算术使用小推理模型指导大模型进行长链推理，ThinkLogit-DPO通过偏好优化进一步提升性能，在推理基准上取得显著准确率提升。


<details>
  <summary>Details</summary>
Motivation: 研究是否可以在不进行额外训练的情况下激发大模型的推理能力，如回溯和自我修正，因为现有研究表明这些能力通常需要额外训练。

Method: 提出ThinkLogit解码时方法，利用logit算术使用小推理模型指导大非推理模型进行长链推理；进一步提出ThinkLogit-DPO，通过偏好优化在正确/错误推理对上训练指导模型。

Result: 在五个推理基准上，使用21倍小的指导模型，ThinkLogit和ThinkLogit-DPO分别实现平均准确率相对提升24.5%和29.1%；方法在不同模型家族间有效，且与后训练方法正交。

Conclusion: ThinkLogit提供了一种无需昂贵后训练即可解锁大模型长链推理能力的实用路径，指导模型通过监督蒸馏或强化学习改进后可直接使用以增强大模型。
...
TL;DR: NL2GenSym框架通过整合大语言模型与SOAR认知架构，从自然语言自动生成符号规则，解决了SOAR手动编码规则的瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: SOAR认知架构需要手动编码规则，这限制了其实际应用。大语言模型具有生成规则的潜力，但缺乏实验验证。

Method: 提出执行驱动的生成器-批评者机制：LLM生成器基于检索增强生成访问知识库提出规则，在SOAR环境中执行验证，批评者根据反馈迭代优化规则。

Result: 在Water Jug Problem数据集上，规则生成成功率超过86%，生成的新启发式规则将平均决策周期降至最优解的1.98倍，是基线方法的1/1000。

Conclusion: NL2GenSym框架有效解决了SOAR规则生成问题，小参数模型也能获得更好性能，为大语言模型与符号AI的融合提供了可行方案。
...
TL;DR: 本文首次系统研究了大型医学语言模型的领域特定微调，发现微调仅改变表示子空间的一小部分，提出tuning vectors框架来捕捉参数变化方向，这些向量能提升指令遵循和生成质量，并改善跨领域泛化能力。


<details>
  <summary>Details</summary>
Motivation: 理解领域特定微调如何重塑LLM参数空间的机制，现有研究主要关注自回归或通用指令模型，领域专业化LLM研究不足。

Method: 提出tuning vectors框架，受task vectors启发，明确捕捉微调引起的参数方向变化，分析这些向量在不同模型层中的影响。

Result: 微调仅修改表示子空间的小部分，保持预训练模型表示；tuning vectors能增强指令遵循和生成质量，跨领域组合可改善泛化；这些向量主要在MLP层写入新方向信息，在注意力头中放大现有方向。
...
TL;DR: TEPO是一种新的token级框架，通过马尔可夫似然将组级奖励与token关联，解决了GRPO等方法的熵崩溃问题，在数学推理任务上实现了新的最先进性能。
...
Conclusion: TEPO通过token级框架有效解决了熵崩溃问题，在数学推理任务上实现了更好的性能和训练稳定性。
...
TL;DR: 该论文研究了人机交互中因自然语言歧义导致的意图匹配问题，特别关注数据可视化领域的代码生成任务。作者提出了歧义分类法和量化指标，并通过多轮对话策略来减少歧义，提高代码准确性。


<details>
  <summary>Details</summary>
Motivation: 在人类与AI的交流中，建立共同目标至关重要，但自然语言的歧义可能导致看似正确但实际不符合用户意图的输出。数据可视化领域特别适合研究这一问题，因为可以同时分析预期图表和生成代码。

Method: 开发了歧义类型分类法并提出量化指标；使用DS-1000数据集中的Matplotlib问题进行验证；探索了三种语用模型（Gricean合作原则、话语表征理论、问题讨论框架）来设计多轮对话策略。

Result: 提出的歧义指标比不确定性基线更好地与人类标注相关；模拟用户研究表明语用对话能有效减少歧义并提高代码准确性。

Conclusion: 多轮对话在代码生成中具有重要价值，通过减少歧义可以更好地匹配用户目标，提高AI系统的实用性和可靠性。
...
TL;DR: 提出了一种多尺度图思维链提示框架MSGCOT，通过整合多尺度结构信息来增强图提示调优的性能，特别是在少样本场景下表现优异。
...
Method: 设计轻量级低秩粗化网络来捕获多尺度结构特征作为分层基向量，然后模仿人类从粗到细的认知过程，在每一步推理中动态整合多尺度信息，形成渐进式粗到细的提示链。

Result: 在八个基准数据集上的实验表明，MSGCOT优于最先进的单粒度图提示调优方法，特别是在少样本场景下展现出优越性能。

Conclusion: 将多尺度信息整合到图提示中能够显著提升图提示调优的性能，特别是在数据稀缺的情况下。
...
TL;DR: LLM SELECTOR是首个用于大语言模型主动选择的框架，通过自适应选择少量信息量最大的查询进行标注，显著降低标注成本。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法依赖完全标注的数据集，标注成本高昂，需要开发能够用有限标注识别最佳LLM的高效方法。

Method: 提出自适应选择少量信息量最大的查询进行标注，并利用基于评判者的oracle标注模型进一步降低标注成本。

Result: 在6个基准测试和151个LLM上的实验表明，LLM SELECTOR在选择最佳和接近最佳LLM时，标注成本降低了高达59.62%。

Conclusion: LLM SELECTOR框架能够高效识别最佳LLM，显著降低标注成本，为LLM选择提供了实用的解决方案。
...
TL;DR: 提出实体提及重构框架来研究大语言模型如何编码和操作实体，通过任务向量方法从模型隐藏状态生成多标记实体提及，发现LLMs开发了实体特定机制来表示和操作多标记实体。


<details>
  <summary>Details</summary>
Motivation: 尽管命名实体是文本知识的基础构件，但目前尚不清楚大语言模型如何在内部表示实体。先前研究主要关注显式关系，但对实体表示本身了解甚少。

Method: 引入实体提及重构框架，利用任务向量方法从LLMs隐藏状态的各种实体表示中一致生成多标记提及，扩展logit-lens为Entity Lens来预测多标记提及。

Result: 发现LLMs开发了实体特定机制来表示和操作任何多标记实体，包括训练期间未见过的实体。

Conclusion: 研究结果为LLMs如何编码和操作实体提供了新证据，表明模型能够表示训练期间未见过的多标记实体。
...
TL;DR: 比较了三种上下文管理策略在语音对话状态跟踪中的表现，发现完整语音历史输入效果最佳，注意力池化压缩方法在保持准确性的同时减少了上下文大小。


<details>
  <summary>Details</summary>
Motivation: 研究如何有效管理语音对话中的上下文信息，以提升端到端语音对话状态跟踪的性能，探索不同上下文策略对模型效果的影响。

Method: 系统评估了三种策略：传统多模态上下文（文本历史+当前语音轮次）、完整语音历史、压缩语音历史，在SpokenWOZ语料库上进行实验。

Result: 完整语音历史输入在相似规模模型中表现最佳，显著超越先前方法；注意力池化压缩方法在保持竞争力的准确性的同时减少了上下文大小。

Conclusion: 改进主要源于更有效的上下文利用，完整语音历史是最优策略，压缩方法提供了良好的权衡方案。
...
TL;DR: 本文首次大规模研究构建完全开放的双语大语言模型KORMo-10B，主要使用合成数据训练韩语模型，证明合成数据不会导致训练不稳定或性能下降，性能与当代多语言基准相当。


<details>
  <summary>Details</summary>
Motivation: 针对非英语语言（特别是韩语）构建完全开放的双语大语言模型，探索在低资源环境下使用合成数据开发开放模型的可行性。

Method: 从头训练10.8B参数的韩英双语模型，其中68.74%的韩语部分是合成数据，通过系统实验验证合成数据的有效性，并采用双语指令调优。

Result: 模型在推理、知识和指令跟随基准测试中表现与当代开放权重多语言基线相当，证明合成数据可以可靠支持长期预训练而不会导致模型崩溃。

Conclusion: 建立了基于合成数据的完全开放模型开发透明框架，为未来多语言LLM研究提供了可复现的先例，双语指令调优使韩语推理和话语连贯性接近母语水平。
...
TL;DR: 该论文研究使用紧凑型开源预训练语言模型从交通事故叙述中提取推理密集型信息，包括碰撞方式和车辆事故类型，通过微调技术超越GPT-4o等大型闭源模型。


<details>
  <summary>Details</summary>
Motivation: 现有工具难以批量处理非标准化的交通事故叙述文本，大型语言模型在推理密集型任务上表现不佳且存在隐私问题，因此研究紧凑型开源模型是否能够胜任此类任务。

Method: 使用LoRA和BERT微调技术向紧凑型预训练语言模型注入领域特定知识，针对碰撞方式和事故类型两个挑战性目标进行训练。

Result: 在权威的CISS数据集上，微调后的紧凑模型超越了GPT-4o等强闭源模型，且仅需极少的训练资源，还能捕获更丰富的叙述细节并纠正数据集中的错误标注。

Conclusion: 紧凑型开源预训练语言模型通过适当微调能够有效处理交通事故叙述中的推理密集型信息提取任务，在性能和隐私保护方面都优于大型闭源模型。
...
TL;DR: 本文提出了一个用于Apertus LLM训练数据的全文索引管道，使用Elasticsearch在arm64超级集群上成功索引了8.6T令牌，为LLM安全性和离线网络搜索提供了解决方案。


<details>
  <summary>Details</summary>
Motivation: 尽管开源LLM数量增加，但训练数据的访问仍然有限且难以理解，特别是包含从互联网抓取的关键数据。

Method: 利用Elasticsearch并行索引和Alps基础设施（节能arm64超级集群），构建全文索引管道来索引Apertus LLM训练数据。

Result: 成功索引了15.2T训练数据中的8.6T令牌，创建了LLM安全工具和离线网络搜索引擎，并证明Elasticsearch可在arm64基础设施上运行。

Conclusion: 大规模全文索引在现代LLM训练数据集规模上是可行的，这种索引可用于确保LLM安全性，并促进向更绿色计算的转型。
...
TL;DR: 论文区分了神经模型泛化能力的两个关键方面：组合性（抽象原子逻辑规则）和递归性（迭代应用推理规则），发现LLMs在递归性方面表现良好但在组合性方面存在困难，提出了一种结合符号推理和神经计算的混合架构来解决这一问题。


<details>
  <summary>Details</summary>
Motivation: 尽管神经模型取得了显著进展，但其泛化能力（特别是逻辑推理等应用中的泛化能力）仍然是一个关键挑战。研究旨在明确区分组合性和递归性这两个泛化能力的基本方面，并评估LLMs在这两方面的表现。
...
Conclusion: 混合模型具有有效解决神经推理系统中关键泛化障碍的潜力，通过符号推理和神经计算的协同交互能够实现稳健高效的推理。
...
<details>
  <summary>Details</summary>
Motivation: 现代对话系统依赖复杂的预定义策略，这些策略作为上下文提示变得冗长且难以忠实遵循，同时带来固定的计算开销。多模态策略管理研究不足，现有方法主要关注文本压缩或安全规则对齐。

Method: 提出TriMPI三阶段训练框架：1)通过持续预训练注入策略知识；2)监督微调；3)PolicyRollout强化学习扩展，使用策略感知响应进行基于探索的rollout。
...
TL;DR: 提出了首个全面的统计学基准StatEval，包含13,817个基础问题和2,374个研究级证明任务，用于评估大语言模型在统计学推理方面的能力。
...
TL;DR: 分析无标签情况下NLP模型性能估计方法的可靠性，发现基于大语言模型的错误预测器比漂移方法或零样本基线产生更一致准确的性能排名。


<details>
  <summary>Details</summary>
Motivation: 理解NLP模型在无标签情况下如何泛化，评估现有性能估计方法在不同领域间产生可靠性能排名的能力。

Method: 使用四类基础分类器和多种大语言模型作为错误预测器，在GeoOLID和Amazon Reviews数据集的15个领域上进行两步评估。

Result: 大语言模型错误预测器比漂移方法和零样本基线产生更强、更一致的排名相关性；当领域间性能差异较大且错误预测与基础模型真实失败模式一致时，排名更可靠。

Conclusion: 阐明了性能估计方法在跨领域模型评估中何时可信，为其使用提供了指导。
...
TL;DR: 提出了一种口音不变的ASR框架，通过集成口音分类和频谱图掩蔽数据增强，提升多语言语音识别系统对口音变化的鲁棒性。
...
Method: 训练基于频谱图的口音分类器，识别关键口音特征区域并掩蔽，使用掩蔽后的频谱图进行数据增强，增强ASR模型对口音变化的鲁棒性。

Result: 在英语和波斯语实验中，使用Whisper模型验证了该方法能显著降低词错误率，并为波斯语建立了首个系统性口音变化基准数据集。

Conclusion: 该方法有效提升了多语言ASR系统对口音和方言多样性的适应能力，为低资源语言多样性语言的研究奠定了基础。
...
TL;DR: 提出GRSP方法，通过段级惩罚机制解决大型推理模型过度思考问题，在保持准确性的同时显著提升计算效率。


<details>
  <summary>Details</summary>
Motivation: 现有基于验证器奖励的强化学习方法虽然能减少token消耗，但往往损害模型性能，因为token级别的监督过于简单。需要更细粒度的监督来平衡效率和准确性。

Method: 提出Group Relative Segment Penalization (GRSP)，一种段级正则化方法。通过分析发现推理段与token消耗和模型性能强相关，设计了跨段簇的长度感知加权机制。

Result: GRSP在保持准确性的同时实现了优越的token效率，特别是在处理更难问题时优势更明显。同时稳定了RL训练，并在不同模型规模上有效扩展。

Conclusion: 监督的粒度在平衡推理模型的效率和准确性中起关键作用，GRSP通过段级惩罚机制有效解决了过度思考问题。
...
TL;DR: 提出了MulTypo多语言拼写错误生成算法，评估了18个开源LLM在5个下游任务中对拼写错误的鲁棒性，发现错误会降低性能，特别是在生成性和推理任务中，且不同语言间存在鲁棒性差异。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试大多假设输入干净，而实际应用中用户输入自然包含拼写错误，LLM在多语言环境下对拼写错误的鲁棒性尚未充分研究。

Method: 开发了基于语言特定键盘布局和打字行为的MulTypo多语言拼写错误生成算法，评估了3个模型家族的18个开源LLM在语言推理、多选问答、数学推理和机器翻译等5个任务上的表现。

Result: 拼写错误会持续降低性能，特别是在生成任务和需要推理的任务中；指令调优提高了干净输入的性能但可能增加对噪声的脆弱性；高资源语言通常比低资源语言更鲁棒；从英语翻译比翻译成英语更鲁棒。
...
TL;DR: 提出了Sandwiched Policy Gradient (SPG)方法，通过使用真实对数似然的上界和下界来解决扩散大语言模型强化学习中的策略梯度偏差问题，显著提升了多个数学推理任务的性能。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型(dLLMs)因其并行解码能力而成为自回归模型的高效替代品，但由于其难以处理的对数似然，无法直接应用标准策略梯度方法进行人类偏好或任务奖励的对齐。现有方法使用证据下界(ELBO)等替代方案，但这些单边近似会引入显著的策略梯度偏差。

Method: 提出了Sandwiched Policy Gradient (SPG)方法，该方法同时利用真实对数似然的上界和下界来减少策略梯度偏差，相比基于ELBO或一步估计的基线方法更加准确。

Result: 实验表明SPG显著优于基于ELBO或一步估计的基线方法。具体来说，在GSM8K上准确率提升了3.6%，在MATH500上提升了2.6%，在Countdown上提升了18.4%，在Sudoku上提升了27.0%。

Conclusion: SPG方法通过使用双边边界有效解决了扩散大语言模型强化学习中的策略梯度偏差问题，在多个数学推理任务上取得了显著的性能提升，为dLLMs的对齐提供了更有效的解决方案。
...
TL;DR: 本文识别了扩散大语言模型中的并行-顺序矛盾(PSC)，该矛盾限制了DLLMs在复杂推理任务中的并行能力，导致其退化为类似自回归的行为。作者提出了三种缩放维度和实际缓解方法。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型提供了高吞吐量和有效的顺序推理，但并行解码与因果顺序要求存在冲突，限制了其在复杂推理中的表现。

Method: 通过行为分析识别PSC问题，引入并行、扩散和顺序三个缩放维度，并提出并行导向提示、扩散早停和并行缩放等缓解方法。

Result: 实验表明DLLMs仅在直接可判定的输出中表现出真正的并行性，随着任务难度增加会退化为自回归行为。并行缩放能带来持续改进，但扩散和顺序缩放受PSC限制。

Conclusion: PSC是DLLMs的核心限制因素，需要针对性缓解策略来平衡并行效率和推理质量。
...
TL;DR: Dyna-Mind是一个两阶段训练框架，通过模拟增强AI代理在复杂交互环境中的推理和决策能力，在Sokoban、ALFWorld和AndroidWorld等基准测试中表现出色。


<details>
  <summary>Details</summary>
Motivation: 当前AI代理在数学和编码方面表现出专家级能力，但在网页导航和计算机/手机使用等长时程交互任务中表现不佳，需要模拟未来状态的能力来提升性能。

Method: 提出Dyna-Mind两阶段框架：第一阶段ReSim训练代理从环境交互中生成结构化推理轨迹；第二阶段Dyna-GRPO使用在线强化学习，结合结果奖励和中间状态反馈来增强模拟和决策能力。

Result: 实验表明：(1) ReSim有效为AI代理注入模拟能力；(2) Dyna-GRPO利用结果和交互级信号学习更好的长时程规划密集型任务策略。
...
TL;DR: 提出一个多阶段框架用于多语言医学视频检索，通过语义分块、知识图谱增强和分层索引实现高效检索，避免了对长视频进行粗粒度压缩或细粒度匹配的高成本问题。
...
Method: 将视频字幕分成语义连贯的块，用知识图谱事实进行增强，组织成分层树结构，使用语言无关的多语言编码器生成节点嵌入，采用粗到细的树搜索策略，最后用轻量级大语言模型对排名靠前的块进行重排序。

Result: 在mVCR测试集上实现了最先进的性能，消融研究证实了知识图谱增强、分层索引和目标LLM重排序的互补贡献。
...
TL;DR: 本文首次全面研究多语言思维链推理，评估了性能、一致性和忠实性三个关键维度，揭示了不同语言下思维链的质量和有效性存在显著差异。


<details>
  <summary>Details</summary>
Motivation: 虽然大型推理模型依赖思维链推理来提高任务性能，但在多语言环境中，导致最终答案的中间步骤（思维轨迹）仍未得到充分探索。

Method: 通过测量语言合规性、答案准确性和一致性来评估模型在不同目标语言下的表现；通过跨语言交换思维轨迹来评估一致性；采用截断和错误注入等扰动技术来探测思维轨迹在不同语言中的忠实性。

Result: 研究发现模型对语言有强烈偏好，不同语言下的性能存在差异；思维链的质量和有效性因提示语言而异；模型在不同语言中对思维轨迹的依赖程度各不相同。

Conclusion: 多语言思维链推理在不同语言中表现出显著差异，需要更深入地理解模型在不同语言环境中的推理行为，并发布了代码和数据以支持未来研究。
...
TL;DR: 该研究翻转了传统思路，探讨话语连接词是否能帮助语言模型理解世界知识，创建了WUGNECTIVES数据集来评估语言模型对新实体的推理能力。
...
Method: 创建包含8,880个刺激项的WUGNECTIVES数据集，评估17个不同规模和训练方案的语言模型在新实体推理任务中的表现。

Result: 对语言模型进行推理行为调优能显著提升大多数连接词的表现，但所有模型在表达让步意义的连接词上都存在系统性困难。

Conclusion: 研究为更细致地探索语言线索在语言模型中的功能作用开辟了道路，揭示了连接词类型对模型性能的重要影响。
...
TL;DR: AutoPR是一个将研究论文自动转化为高质量推广内容的新任务，PRAgent多智能体框架通过三阶段流程显著提升推广效果，相比直接LLM方法观看时间增加604%，点赞数提升438%。


<details>
  <summary>Details</summary>
Motivation: 随着同行评审研究数量激增，学者依赖社交平台发现研究，作者需投入大量精力推广工作以确保可见性和引用。为简化流程并减少人工依赖，需要自动化推广解决方案。

Method: 提出PRAgent多智能体框架，包含三阶段：多模态内容提取、协作合成生成精炼输出、平台特定适配优化规范、语气和标签以最大化覆盖范围。

Result: 在PRBench基准测试中，相比直接LLM流水线，PRAgent实现观看时间604%增长、点赞数438%提升、整体参与度至少2.9倍提升。消融研究表明平台建模和目标推广贡献最大。
...
TL;DR: 提出了Mind-Paced Speaking (MPS)框架，采用双脑方法实现实时口语语言模型的高保真推理，显著降低延迟同时保持推理质量
...
Conclusion: MPS有效弥合了高质量推理与实时交互之间的差距，实现了类似人类边说边思考的能力
...
TL;DR: P-TTS是一种推理时数据增强策略，仅使用90个手动选择推理实例，通过系统化改变指令提示强度来合成多样化推理轨迹，显著提升LLM的数学推理能力。


<details>
  <summary>Details</summary>
Motivation: 传统方法需要大量人工标注的推理数据集，成本高昂且耗时。P-TTS旨在通过少量示例和推理时数据增强来低成本提升LLM推理能力。

Method: 使用仅90个手动选择的推理实例，在测试时通过系统化改变指令提示强度来合成多样化推理轨迹上下文，然后对Qwen-2.5模型进行微调。

Result: P-TTS-7B和32B模型在AIME2024、AIME2025等数学推理基准上显著超越基线，7B模型在AIME'24上分别提升26.66%和30.00%，在AIME'25上提升13.34%和6.67%。32B模型也有显著提升。

Conclusion: P-TTS通过测试时缩放有效探索推理模式的潜在空间，以最小标注成本放大LLM问题解决能力，为资源受限或快速演变领域提供实用的低成本LLM推理激发方法。
...
TL;DR: 提出AScience框架和ASCollab系统，通过LLM研究代理的自组织网络进行大规模科学假设探索，在癌症队列实验中展示了该方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 大型科学数据集为无约束的探索性发现提供了机会，但传统方法难以有效探索复杂假设空间。

Method: 开发AScience框架，将发现过程建模为代理、网络和评估规范的交互，实现为ASCollab分布式系统，包含具有异质行为的LLM研究代理。
...
TL;DR: 提出使用大语言模型(LLMs)来评估车辆路径规划(VRP)生成的配送路线，通过政策标准来识别路由问题，提高最后一公里物流效率。
...
Method: 开发了一个框架，使用LLMs根据政策标准对VRP生成的路线进行批评评估，生成、标注和评估了400个案例。

Result: 开源LLMs识别路由问题的准确率达到79%，专有推理模型最高达到86%。LLM评估超越了传统的距离和时间指标。
...
Result: 在四个基准测试中导航成功率提升高达30%，轨迹误差显著降低，在TartanDrive数据集上表现出优秀的零样本泛化能力。

Conclusion: UniWM为实现统一的、基于想象的具身导航迈出了重要一步。
...
Method: 提出COMPASS框架，包含三个组件：主代理负责推理和工具使用，元思考者监控进度并发出战略干预，上下文管理器维护简洁相关的进度简报。
...
TL;DR: 研究发现人类和AI在评估文学风格时都存在系统性偏见，AI比人类表现出更强的反AI偏见，当内容被标记为AI生成时评分会显著降低。
...
Method: 使用Raymond Queneau的《风格练习》进行对照实验，比较人类参与者和AI模型在盲测、准确标注和反事实标注条件下对文学段落的评估。

Result: 人类显示+13.7个百分点的偏见，AI模型显示+34.3个百分点的偏见（比人类强2.5倍）。AI系统在内容被标记为AI生成时会系统性地贬低创意内容。

Conclusion: AI模型在训练过程中吸收了人类对人工创造力的文化偏见，不仅复制而且放大了这种人类倾向，归因标签会导致评估标准反转。
...
TL;DR: 提出了Agent GPA（目标-计划-行动）评估框架，包含五个评估指标，能系统性地覆盖智能体失败情况，支持LLM评估器与人工标注高度一致，并能准确定位错误。


<details>
  <summary>Details</summary>
Motivation: 需要一种系统性的评估范式来全面评估智能体的操作循环，包括目标设定、计划制定和行动执行，以覆盖广泛的智能体失败情况。

Method: 基于智能体操作循环构建GPA框架，包含五个评估指标：目标达成度、逻辑一致性、执行效率、计划质量和计划遵循度。在两个基准数据集上进行实验验证。

Result: 该框架能覆盖TRAIL/GAIA基准数据集中的所有智能体错误，LLM评估器与人工标注的一致性达到80%-95%，错误定位准确率达到86%。

Conclusion: Agent GPA框架为智能体评估提供了系统化的方法，能够全面覆盖智能体失败情况，支持高效的自动化评估，并能准确定位问题以针对性改进智能体性能。
...
TL;DR: ReviewerToo是一个模块化AI辅助同行评审框架，在ICLR 2025数据集上达到81.8%的接受/拒绝分类准确率，接近人类评审员的83.9%，其生成的评审质量高于人类平均水平。


<details>
  <summary>Details</summary>
Motivation: 解决传统同行评审中的不一致性、评审员主观性和可扩展性挑战，通过AI辅助来补充人类判断。

Method: 开发了ReviewerToo框架，支持使用专门评审员角色和结构化评估标准进行系统实验，可部分或完全集成到实际会议工作流程中。

Result: 在1,963篇ICLR 2025论文上的实验显示，AI评审员在事实核查、文献覆盖方面表现出色，但在评估方法新颖性和理论贡献方面仍有困难。

Conclusion: AI可以增强同行评审的一致性、覆盖面和公平性，但复杂评估仍需领域专家，为构建系统化的混合同行评审系统奠定了基础。
...
TL;DR: 提出了GTAlign框架，将博弈论决策整合到LLM的推理和训练中，通过构建收益矩阵来估计用户和模型双方的福利，选择互利行动，显著提升了推理效率、答案质量和互惠福利。
...
Method: 在推理阶段将用户-LLM交互视为策略博弈，构建收益矩阵估计双方福利；在训练阶段引入互惠福利奖励强化合作响应；还提出基于博弈论推理的动态适应技术应对LLM服务定价政策变化。

Result: 广泛实验表明，GTAlign相比基线方法在不同任务中显著提升了推理效率、答案质量和互惠福利。

Conclusion: GTAlign通过博弈论决策机制实现了LLM与用户的互利共赢，为解决对齐问题提供了新思路。
...
Result: LM Fight Arena提供了一个完全自动化、可重现且客观的评估框架，能够测试LMM在动态环境中的战略推理能力。
...
TL;DR: RADAR是一个基于机制可解释性的框架，通过区分基于记忆和基于推理的模型响应来检测数据污染，准确率达93%。


<details>
  <summary>Details</summary>
Motivation: 数据污染严重影响了LLM评估的可靠性，模型可能通过记忆训练数据而非真正推理获得高分。
...
TL;DR: 提出了一个评估检索增强生成(RAG)管道的基准，包含93个多模态问题，评估管道的文本、表格、图像处理和跨文档推理能力，并开发了短语级召回和幻觉检测指标。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注检索等特定方面，缺乏对RAG管道整体能力的评估，特别是多模态信息处理和跨文档推理能力。

Method: 创建包含93个多模态问题的人工数据集，开发短语级召回指标和最近邻嵌入分类器检测幻觉，比较2个开源和4个闭源管道的性能。

Result: 闭源管道在正确性和幻觉检测方面显著优于开源管道，特别是在多模态和跨文档问题上差距更大。人工评估显示指标与人类判断高度一致(4.62/5和4.53/5)。

Conclusion: 该基准能有效评估RAG管道的整体能力，闭源模型在多模态处理方面表现更好，提出的指标与人类判断有良好一致性。
...
TL;DR: EcphoryRAG是一个基于认知神经科学启发的实体中心知识图谱RAG框架，通过提取核心实体和元数据实现轻量级索引，在检索时使用多跳关联搜索和动态关系推理，在多项基准测试中达到新的最先进水平。


<details>
  <summary>Details</summary>
Motivation: 受人类认知神经科学中通过线索激活实体中心记忆痕迹进行复杂多跳回忆的机制启发，开发更高效的RAG框架。

Method: 索引阶段仅提取和存储核心实体及元数据；检索阶段先提取查询中的线索实体，然后在知识图谱上进行可扩展的多跳关联搜索，并动态推断实体间的隐式关系来填充上下文。

Result: 在2WikiMultiHop、HotpotQA和MuSiQue基准测试中，将平均精确匹配(EM)分数从0.392提升到0.474，相比HippoRAG等强基线方法有显著改进，同时索引阶段token消耗减少高达94%。
...
TL;DR: DualResearch是一个检索和融合框架，通过联合建模广度语义图和深度因果图来解决深度研究框架中的上下文污染、证据支持薄弱和执行路径脆弱问题，在科学推理基准上实现了性能提升。


<details>
  <summary>Details</summary>
Motivation: 解决深度研究框架存在的三个主要问题：上下文污染、证据支持薄弱和执行路径脆弱，这些限制了工具密集型推理的效果。
...
Result: 在科学推理基准HLE和GPQA上取得竞争性表现。使用InternAgent系统的日志文件，在HLE上的准确率提高7.7%，在GPQA上提高6.06%。
...
TL;DR: 提出Semantic-condition Tuning (SCT)新范式，通过图神经网络提取上下文感知的语义条件，深度融合知识图谱与LLM，显著提升知识推理性能


<details>
  <summary>Details</summary>
Motivation: 现有prefix-tuning方法只是简单拼接知识嵌入和文本输入，忽略了知识图谱的丰富关系语义，给LLM带来了沉重的隐式推理负担

Method: 包含语义图模块和条件自适应融合模块：前者用GNN从局部图邻域提取知识增强的语义条件；后者通过参数化投影器自适应调制文本嵌入，实现深度特征级交互

Result: 在知识图谱基准测试中显著优于prefix-tuning和其他强基线方法

Conclusion: 通过在LLM推理前用语义图上下文调制输入表示，SCT提供了更直接有效的信号，实现了更准确鲁棒的知识推理
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在推理效率上面临模型规模大、过度思考、轻量场景准确率低等挑战，且轻量级MLLMs的推理能力研究较为缺乏。

Method: 采用两阶段优化：第一阶段提出LIPO强化学习方法，动态调整响应优势，鼓励生成更短更准确的响应；第二阶段提出AMM训练自由模型融合方法，自适应调整任务向量权重并通过梯度投影正则化优化合并向量。
...
TL;DR: 提出了一个统一的旅行规划基准，通过单一奖励整合细粒度标准，支持强化学习集成，并在多种方法中验证了RL在提高行程可行性方面的优势。
...
Method: 构建包含4870个查询的大规模数据集，开发统一评估器将细粒度标准转化为单一奖励，采用多种方法包括测试时计算、神经符号方法、监督微调和GRPO强化学习。

Result: 评估器与旅行专家标注达到60.75%的一致性，优于多个LLM-as-judge基线；RL方法在基础模型上普遍提高了行程可行性，获得更高的统一奖励分数。

Conclusion: 提出的统一基准能够有效评估旅行规划质量，RL方法在提高计划可行性方面表现突出，为LLMs在复杂规划任务中的应用提供了有力支持。
...
TL;DR: 该论文评估了SOTA LLMs在证明评分方面的能力，提出了基于智能体工作流程的评分方法，能够更准确地检测错误、判断严重程度并分配公平分数。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs在解决奥林匹克数学问题方面取得显著进展，需要评估这些模型在证明评分方面的能力，特别是检测错误、判断严重程度和分配公平分数，而不仅仅是二元正确性判断。

Method: 使用90个Gemini 2.5 Pro生成的解决方案和MathArena的IMO/USAMO 2025解决方案集，引入智能体工作流程，提取和分析参考解决方案，自动推导问题特定的评分标准，进行多步骤评分过程。

Result: 模型能够可靠地标记错误解决方案（包括细微错误），但在分配部分学分方面存在校准差距。提出的工作流程在人类评分一致性和部分学分处理一致性方面表现更好。

Conclusion: 智能体工作流程能够提高证明评分的准确性和一致性，为未来研究提供了代码、数据和提示/日志资源。
...
TL;DR: 提出了一种混合框架LRR来解决正则表达式拒绝服务漏洞修复问题，结合符号模块的精确性和LLM的泛化能力，将问题识别与修复过程解耦。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在权衡：符号规则系统精确但无法修复未见或复杂漏洞模式，而LLM具有泛化能力但在需要严格语法语义正确性的任务中不可靠。

Method: 使用确定性符号模块定位精确的易受攻击子模式，然后调用LLM为这个隔离片段生成语义等效的修复方案。

Result: 该混合架构成功解决了规则修复无法处理的复杂修复案例，同时避免了纯LLM方法的语义错误，修复率比最先进方法提高了15.4个百分点。

Conclusion: LRR框架为自动修复问题提供了经过验证的方法论，有效结合了符号系统的可靠性和LLM的泛化能力。
...
TL;DR: 提出连续记忆机制，将GUI轨迹编码为固定长度的连续嵌入，显著降低上下文成本并保留细粒度视觉信息。通过自动扩展数据飞轮收集10万+轨迹，仅微调1.2%参数，在长任务和分布偏移下显著提升成功率。


<details>
  <summary>Details</summary>
Motivation: 现有GUI代理将历史轨迹压缩为文本token，导致上下文长度膨胀且丢失关键视觉线索（如控件精确尺寸和位置），需要可扩展的记忆机制来泛化不熟悉界面和长时任务。

Method: 使用VLM作为编码器将GUI轨迹编码为固定长度连续嵌入，直接输入骨干网络；引入自动扩展数据飞轮，通过搜索发现环境、VLM合成任务、代理执行轨迹、VLM验证成功来低成本扩展记忆。

Result: 随着记忆大小和检索深度增加，性能单调提升（与文本记忆相反）；在真实GUI基准测试中，连续记忆增强代理在长时任务和分布偏移下持续提升成功率；Qwen-2.5-VL-7B+连续记忆达到与GPT-4o、Claude-4等闭源模型相当的性能。

Conclusion: 连续记忆机制有效解决了GUI代理的上下文膨胀和视觉信息丢失问题，通过低成本数据收集和少量参数微调，显著提升了在复杂界面和长时任务中的泛化能力。
...
TL;DR: 该研究提出了一种整合精神分析和MBTI人格理论的新方法，构建了三种人工意识（自我意识、无意识、前意识）和16种MBTI人格角色，通过多种评估方式验证了人工意识的人类认知模拟能力。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在翻译、摘要等领域取得显著进展，但由于幻觉问题难以模拟人类意识。本研究旨在通过整合精神分析和人格理论来解决这一挑战。

Method: 基于精神分析原理开发了三种人工意识模块，设计了16种MBTI人格角色，创建了10种不同情境来测试模型的认知能力，并通过问卷调查、ChatGPT分类和定性评估三种方式进行评估。

Result: 定量和定性分析表明，开发的人工意识模型具有较高的模拟人类意识的可能性，尽管不同角色和意识之间的响应差异不够显著。

Conclusion: 整合精神分析和人格理论元素可以构建更直观、适应性更强的人工智能系统，为改进AI在复杂认知情境中的交互开辟了新途径。
...
TL;DR: 提出了MEC³O多专家共识系统，通过将LLMs分配到不同复杂度类别并让专家进行结构化辩论，使用加权共识机制整合预测，在代码复杂度预测任务上超越了开源基线模型和GPT-4o-mini。


<details>
  <summary>Details</summary>
Motivation: 现有LLMs在代码复杂度预测任务中，没有单一模型在所有复杂度类别上都表现优异，不同模型在不同类别上各有优势。

Method: MEC³O系统将LLMs基于性能分配到特定复杂度类别，提供类别专业化指令使其成为专家，专家进行结构化辩论，通过加权共识机制整合预测结果。

Result: 在CodeComplex数据集上，MEC³O比开源基线模型准确率和macro-F1分数至少高出10%，在macro-F1分数上平均超过GPT-4o-mini，与GPT-4o和GPT-4o-mini在F1分数上表现相当。
...
Method: 通过特征空间目标促进轨迹间的横向扩散，并通过时间调度的随机扰动重新引入不确定性，该扰动被投影为与生成流正交，从而在不降低图像细节或提示保真度的情况下增加变化。

Result: 在固定采样预算下，该方法在多个文本到图像设置中持续改进了Vendi Score和Brisque等多样性指标，同时保持了图像质量和对齐度。

Conclusion: 该方法无需重新训练或修改基础采样器，与常见的流匹配求解器兼容，理论上单调增加体积代理，同时由于几何约束近似保持边缘分布，为生成质量的鲁棒保持提供了原理性解释。
...
TL;DR: 提出了一种高阶网络动力学识别方法，用于复杂网络的长期动态预测，结合动态超图学习和物理数据双驱动预测模块，解决了传统方法只能处理成对关系以及理论模型准确性不足、数据驱动模型可解释性差的问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用简单图描述复杂网络关系，但只能捕捉成对关系，而网络中可能存在丰富的非成对结构关系；一阶GNN难以捕捉动态非成对关系；理论预测模型缺乏准确性，数据驱动预测模型缺乏可解释性。
...
TL;DR: 提出一个基于Stackelberg竞争的强化学习框架，训练LLM代理在社交推理游戏中优化说服性沟通，显著超越基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有LLM代理在社交推理游戏中主要关注信息处理和策略选择，忽视了说服性沟通在影响其他玩家信念和反应中的重要性。
...
TL;DR: 提出PAC推理方法，通过不确定性评分控制大型推理模型在思考模式和非思考模式间的切换，在保证性能损失可控的前提下节省计算成本


<details>
  <summary>Details</summary>
Motivation: 大型推理模型虽然性能优异但计算成本高，现有动态切换方法缺乏性能损失的统计保证，难以应用于高风险场景

Method: 构建性能损失的上置信界作为不确定性评分的单调函数，通过设定阈值来控制模型在思考和非思考模式间的切换

Result: 在推理基准测试中，该方法能够节省计算预算并有效控制用户指定的性能损失

Conclusion: PAC推理方法为大型推理模型提供了分布无关的性能损失保证，在高风险应用中具有重要价值
...
TL;DR: 研究发现LLM生成的医疗建议存在系统性偏见，对原住民和双性人等社会群体提供更复杂难懂的建议，且交叉性群体中这种趋势更加明显。


<details>
  <summary>Details</summary>
Motivation: 随着LLM在医疗领域的广泛应用，需要评估其是否考虑医疗的社会性质，特别是健康差异和社会偏见如何影响LLM生成的医疗建议。

Method: 通过模拟不同性别、年龄和种族的患者档案，向LLM提出一系列临床问题，比较生成回答的自然语言特征。

Result: LLM生成的医疗建议在不同社会群体间存在系统性差异，特别是对原住民和双性人群体提供更复杂难读的建议。

Conclusion: 鉴于用户对LLM的信任度增加，需要提高AI素养并紧急调查和缓解这些系统性差异，确保公平的患者支持。
...
<details>
  <summary>Details</summary>
Motivation: 医疗领域需要整合复杂多样的医学知识和患者健康信息，以支持生命关键决策，这要求统一的方法来收集、分析和利用现有医疗知识。

Method: 基于知识图谱结构开发多个概念模型，评估知识融合的实现方式，并展示如何将各种知识源集成到知识图谱中。

Result: 提出了支持医疗救援操作的知识融合框架，使医疗专业人员能够从多个情境对齐的知识源中进行选择。

Conclusion: 知识融合为医疗领域提供了整合多种知识源的有效方法，能够支持关键决策制定过程。
...
TL;DR: 该论文提出了基于PSPACE完全正则表达式问题的新基准，用于评估大语言模型和大推理模型的空间计算限制，发现模型存在冗长和重复等常见失败模式。


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型在自然语言处理、数学推理和编程方面表现出色，但其有限上下文窗口约束的空间计算限制仍未得到充分理解。现有研究主要关注NP复杂度问题，需要更严格的PSPACE完全问题来评估计算能力。

Method: 通过双指数空间探索构建了包含超过100万个正则表达式实例的数据集，使用严格的过滤过程建立基准，并对6个LLM和5个LRM进行了广泛评估。

Result: 评估揭示了模型在PSPACE完全正则表达式问题上的常见失败模式，包括冗长和重复，表明模型在处理需要大规模搜索空间探索的问题时存在计算限制。
...
TL;DR: 本文综述了基于大语言模型(LLM)的智能体架构与实现方法，探讨如何通过感知、推理、记忆和执行系统的整合，开发能够自动化复杂任务、缩小与人类能力差距的智能LLM。


<details>
  <summary>Details</summary>
Motivation: 传统LLM在现实世界任务中存在局限性，研究旨在探索开发"智能体化"LLM的模式，使其能够自动化复杂任务并弥合与人类能力的性能差距。
...
Conclusion: 通过系统化整合感知、推理、记忆和执行组件，可以开发出更接近人类认知能力的智能LLM智能体，为自动化复杂任务提供有效解决方案。
...
TL;DR: 提出了一种新颖框架，通过可调节的局部性参数训练大语言模型，使其内部表示能够在局部化（可解释、基于规则）和分布式（泛化性强、高效）编码之间连续调整。


<details>
  <summary>Details</summary>
Motivation: 为了解决在需要透明度和高性能的监管领域中，模型需要在可解释性和性能之间取得平衡的问题。

Method: 使用组稀疏惩罚注意力机制、信息理论锚点设计和动态规则注入，通过局部性参数动态控制训练和推理过程中的局部化程度，无需重新训练模型。
...
<details>
  <summary>Details</summary>
Motivation: 探索大语言模型在逻辑推理问题中的内部机制，目前这些机制在很大程度上尚未被研究。

Method: 使用小语言模型解决演绎推理任务，分析其内部表示和计算电路，特别关注归纳头的作用。

Result: 发现归纳头在规则完成和规则链式推理步骤中发挥核心作用，是实现逻辑推理的关键机制。

Conclusion: 语言模型可以通过学习底层规则来执行逻辑推理，归纳头是实现这种推理能力的重要计算组件。
...
TL;DR: 本文在约束规划中形式化了序列变量，以解决传统后继变量模型在处理可选访问和插入启发式时的局限性，为车辆路径问题提供了更直观的建模方法。


<details>
  <summary>Details</summary>
Motivation: 传统约束规划中基于后继变量的模型无法有效处理可选访问或插入启发式，这限制了在车辆路径问题中的应用。

Method: 定义了序列变量的计算域、更新操作和一致性级别，实现了与现有CP求解器的集成，并设计了专门用于序列变量和车辆路径的全局约束。

Result: 序列变量简化了问题建模，并在Dial-a-Ride问题上实现了有竞争力的计算性能。

Conclusion: 序列变量为车辆路径问题提供了更强大的约束规划建模能力，能够处理可选访问并支持插入启发式算法。
...
TL;DR: 这篇论文综述了基于大语言模型(LLM)的智能代理系统在放射学中的应用，探讨了从半自动化工作流到自适应代理的设计、评估和挑战。


<details>
  <summary>Details</summary>
Motivation: 放射学具有多模态数据流和协调工作流程的特点，非常适合利用能够适应上下文并自动化复杂重复任务的智能代理。虽然LLM在放射学的单个任务中表现出色，但单独使用未能充分发挥其在复杂多步骤工作流程中的潜力。

Method: 通过为LLM配备外部工具和反馈机制，使其能够驱动展现不同程度自主性的系统，从半自动化工作流到能够管理复杂流程的自适应代理。

Result: LLM驱动的代理系统在放射学中显示出应用潜力，能够整合多源信息并执行复杂的多步骤决策过程。

Conclusion: LLM驱动的智能代理系统为放射学工作流程自动化提供了新的可能性，但仍面临错误级联、工具使用效率和医疗IT集成等挑战，需要进一步研究和开发。
...
TL;DR: 本文探讨了在数据湖仓中安全使用AI代理的方法，提出了通过API优先、可编程的架构实现安全设计，并展示了使用Bauplan案例研究的数据分支和声明式环境如何支持代理工作流。
...
Method: 使用Bauplan作为案例研究，采用数据分支和声明式环境来扩展代理功能，并开发了一个概念验证原型，其中代理使用受证明携带代码启发的正确性检查来修复数据管道。

Result: 原型证明不可信的AI代理可以在生产数据上安全操作，同时实现可重现性和可观察性，并减少攻击面。

Conclusion: 该方法为实现完全代理化的数据湖仓铺平了道路，展示了API优先、可编程的湖仓架构为安全设计提供了合适的抽象层次。
...
<details>
  <summary>Details</summary>
Motivation: 神经符号AI领域近三十年来未能实现其潜力，主要因为现有框架难以扩展，且神经方法的隐式表示和近似推理限制了可解释性和可信度。知识图谱作为显式语义知识的黄金标准表示，可以解决符号方面的问题，但从文本语料库自动生成可靠的知识图谱仍是一个开放问题。

Method: 引入GraphMERT，一个小型图形编码器模型，从非结构化文本语料库及其内部表示中提取高质量知识图谱。GraphMERT及其等效知识图谱形成一个模块化神经符号堆栈：神经学习抽象，符号知识图谱用于可验证推理。

Result: 在PubMed糖尿病论文文本上，80M参数的GraphMERT生成的知识图谱达到69.8%的事实分数，而32B参数的基线LLM仅达到40.2%。GraphMERT知识图谱的有效性分数为68.8%，而LLM基线为43.0%。

Conclusion: GraphMERT+KG是第一个高效且可扩展的神经符号模型，在基准测试中达到最先进精度，同时相对于基线具有优越的符号表示能力。
...
TL;DR: LiveOIBench是一个包含403个信息学奥林匹克竞赛级别编程问题的基准测试，具有高质量测试用例和离线评估系统，用于评估大语言模型的编程能力。


<details>
  <summary>Details</summary>
Motivation: 当前编程基准测试存在缺乏高难度问题、测试用例覆盖不足、依赖在线平台API等问题，需要更全面且可访问的评估标准。

Method: 从72个官方信息学奥林匹克竞赛中收集403个专家策划的问题，每个问题平均60个测试用例，建立包含详细子任务评分和人类选手表现数据的评估系统。

Result: GPT-5达到81.76百分位，但仍低于顶尖人类选手（通常超过90百分位）；开源模型GPT-OSS-120B仅达60百分位，与闭源模型存在显著差距。

Conclusion: 强大的推理模型应优先进行精确问题分析而非过度探索，未来模型应强调结构化分析并减少不必要的探索。
...
### [240] [Energy-Driven Steering: Reducing False Refusals in Large Language Models](https://arxiv.org/abs/2510.08646)
...
TL;DR: 提出Energy-Driven Steering (EDS)框架，通过基于能量的模型在推理时动态引导LLM隐藏状态，同时提高安全性和减少错误拒绝，无需微调。


<details>
  <summary>Details</summary>
Motivation: 解决当前LLM安全对齐技术的局限性：过度关注有害提示的安全性，导致模型变得过度谨慎，拒绝响应良性提示。

Method: 训练轻量级外部能量模型，为不良状态分配高能量，为理想状态分配低能量。在推理时使用能量函数的梯度动态引导LLM隐藏状态到低能量区域。

Result: 在ORB-H基准测试中，合规率从57.3%提升至82.6%，同时保持基线安全性能。

Conclusion: EDS提供了一种有效范式，可构建同时具有低错误拒绝率和高安全性的LLM，将行为控制与核心知识解耦。
...
### [241] [Inverse-Free Wilson Loops for Transformers: A Practical Diagnostic for Invariance and Order Sensitivity](https://arxiv.org/abs/2510.08648)
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在面对重排序、微调、提示路径等无害编辑时会产生答案翻转，这些违反预期不变性的问题影响系统可靠性，迫使团队在安全性和速度之间权衡。

Method: WILSON结合了无逆曲率图（使用JVP和Hutchinson探针计算）和激活级交换器，这些信号计算成本低，适用于标准Transformer模型。

Result: WILSON能够识别重排序风险，帮助保护RAG免受顺序效应影响，捕获微调回归，稳定辩论路径和长多轮上下文，并在部署中控制融合或重排序操作。
...
### [242] [Knowledge Graph Sparsification for GNN-based Rare Disease Diagnosis](https://arxiv.org/abs/2510.08655)
*Premt Cara,Kamilia Zaripova,David Bani-Harouni,Nassir Navab,Azade Farshad*

Main category: cs.LG

TL;DR: RareNet是一种基于子图的图神经网络，仅需患者表型数据即可识别最可能的致病基因，为资源有限环境提供可解释的遗传分析工具。


<details>
  <summary>Details</summary>
Motivation: 罕见遗传病诊断面临三大挑战：患者数据不足、全基因组测序不可及、致病基因数量庞大。这些问题导致诊断周期长、治疗不当，在资源有限地区尤为严重。

Method: 提出RareNet，一种基于子图的图神经网络方法，仅使用患者表型数据识别致病基因，并检索相关患者子图供临床调查。可作为独立方法或与其他候选基因优先排序方法结合使用。

Result: 在两个生物医学数据集上的综合评估显示，RareNet在致病基因预测方面具有竞争力和鲁棒性，与其他框架集成时显著提升性能。

Conclusion: RareNet仅需表型数据即可实现复杂遗传分析，在缺乏先进基因组基础设施的服务不足人群中具有特殊价值，实现了遗传分析的民主化。
...
### [243] [Inner-Instance Normalization for Time Series Forecasting](https://arxiv.org/abs/2510.08657)
*Zipo Jibao,Yingyi Fu,Xinyang Chen,Guoting Chen*

Main category: cs.LG

TL;DR: 提出两种点级方法（LD和LCD）来解决时间序列预测中的实例内分布偏移问题，通过在不同时间步使用不同参数或神经网络预测缩放系数来消除内部差异。
...
### [244] [Provably Robust Adaptation for Language-Empowered Foundation Models](https://arxiv.org/abs/2510.08659)
*Yuni Lai,Xiaoyu Xue,Linghui Shen,Yulun Wu,Gaolei Li,Song Guo,Kai Zhou,Bin Xiao*

Main category: cs.LG

TL;DR: 提出了首个针对语言赋能基础模型的具有可证明鲁棒性的少样本分类器LeFCert，通过自适应融合文本和特征嵌入，采用双重修剪均值原型，在中毒攻击下提供可证明的鲁棒性保证。


<details>
  <summary>Details</summary>
Motivation: 现有的语言赋能基础模型在少样本学习中依赖开放环境中收集的小型任务特定支持数据集，容易受到中毒攻击，现有防御方法缺乏形式化保证且对未知和自适应攻击脆弱。

Method: 提出LeFCert模型，集成文本和特征嵌入与自适应融合机制，使用双重修剪均值原型，推导分类分数的可证明上下界，并在更现实的攻击预算下扩展了LeFCert-L和LeFCert-C两个变体。

Result: 实验表明LeFCert实现了最先进的性能，相比现有基线显著提高了干净准确率和认证准确率，同时计算效率高，适合实际应用。

Conclusion: LeFCert填补了语言赋能基础模型少样本分类器可证明鲁棒性的关键空白，提供了强大的防御能力且保持实用性。
...
### [245] [How Scale Breaks "Normalized Stress" and KL Divergence: Rethinking Quality Metrics](https://arxiv.org/abs/2510.08660)
*Kiran Smelser,Kaviru Gunaratne,Jacob Miller,Stephen Kobourov*

Main category: cs.LG

TL;DR: 论文分析了高维数据可视化中常用的质量指标（归一化应力和KL散度）对尺度变化的敏感性，提出了使这些指标尺度不变的方法。


<details>
  <summary>Details</summary>
Motivation: 高维数据可视化中，二维散点图是常用方法，但评估这些投影的准确性具有挑战性。常用的质量指标如归一化应力和KL散度对尺度变化敏感，即使尺度变化不会实质改变投影质量。

Method: 通过分析和实验研究尺度对应力和KL散度的影响，提出了一种简单技术使这两个指标尺度不变，并在小型基准测试中验证其有效性。

Result: 研究表明尺度变化会显著改变应力和KL散度的值，影响降维技术评估。提出的尺度不变方法能准确捕捉预期行为。

Conclusion: 现有可视化质量指标对尺度变化敏感，提出的尺度不变方法能更准确地评估降维投影质量，避免尺度变化带来的误导。
...
### [246] [CATS-Linear: Classification Auxiliary Linear Model for Time Series Forecasting](https://arxiv.org/abs/2510.08661)
*Zipo Jibao,Yingyi Fu,Xinyang Chen,Guoting Chen*

Main category: cs.LG

TL;DR: 提出了CATS-Linear模型，通过分类辅助通道独立机制动态路由时间序列实例到专用预测器，并结合趋势-季节性分解架构，在固定超参数下实现与调优基线相媲美的SOTA精度。


<details>
  <summary>Details</summary>
Motivation: 现有研究显示线性模型能达到与复杂架构相当的预测性能，但增强线性模型的方法仍待探索。基于不同时间序列实例可能遵循异质线性映射的假设，需要开发更有效的线性模型增强方法。

Method: 采用分类辅助通道独立机制动态路由实例到专用预测器，实现监督通道设计；重新设计趋势-季节性分解架构，添加解耦-线性映射-重耦合框架处理趋势分量，使用复数域线性投影处理季节性分量。

Result: 大量实验验证CATS-Linear在固定超参数下达到与超参数调优基线相当的SOTA精度，同时在固定超参数对比中提供最佳精度。

Conclusion: CATS-Linear通过动态实例路由和重新设计的分解架构，有效提升了线性模型的预测性能，在固定超参数设置下实现了竞争性的SOTA结果。
...
### [247] [DPCformer: An Interpretable Deep Learning Model for Genomic Prediction in Crops](https://arxiv.org/abs/2510.08662)
*Pengcheng Deng,Kening Liu,Mengxi Zhou,Mingxi Li,Rui Yang,Chuzhe Cao,Maojun Wang,Zeyu Zhang*

Main category: cs.LG

TL;DR: DPCformer是一种结合卷积神经网络和自注意力机制的深度学习模型，用于基因组选择，在多种作物和性状上显著提高了预测精度。


<details>
  <summary>Details</summary>
Motivation: 传统基因组选择方法在处理复杂性状和大数据集时预测精度有限，需要更有效的模型来建模复杂的基因型-表型关系。
...
### [248] [FreqCa: Accelerating Diffusion Models via Frequency-Aware Caching](https://arxiv.org/abs/2510.08669)
...
TL;DR: 本文提出频率感知缓存(FreqCa)方法，通过分析扩散模型中不同频率分量的动态特性，分别处理低频和高频特征来优化扩散变压器的推理成本。


<details>
  <summary>Details</summary>
Motivation: 现有特征缓存方法假设相邻时间步的特征相似或连续，但这在扩散模型中并不总是成立。通过频域分析发现，不同频率分量在时间步间表现出不同的动态特性。

Method: 提出FreqCa方法：直接重用相似的低频特征，使用二阶Hermite插值器预测连续的高频特征。同时提出缓存累积残差特征(CRF)来减少内存占用。

Result: 在FLUX.1-dev、FLUX.1-Kontext-dev、Qwen-Image和Qwen-Image-Edit等数据集上的实验证明了该方法在生成和编辑任务中的有效性。

Conclusion: FreqCa通过频率感知的特征缓存策略，显著降低了扩散变压器的推理成本，同时保持了生成质量。
...
### [249] [Don't Waste Mistakes: Leveraging Negative RL-Groups via Confidence Reweighting](https://arxiv.org/abs/2510.08696)
*Yunzhen Feng,Parag Jain,Anthony Hartshorn,Yaqi Duan,Julia Kempe*

Main category: cs.LG

TL;DR: 提出LENS方法改进GRPO算法，通过利用负样本组（没有正确答案的组）来提升强化学习效率，在MATH基准测试中表现优于GRPO基线


<details>
  <summary>Details</summary>
Motivation: GRPO算法在负样本组上浪费大量计算资源，因为这些组中没有任何采样响应是正确的，导致零优势和零梯度
...
### [250] [In-Context Learning for Non-Stationary MIMO Equalization](https://arxiv.org/abs/2510.08711)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于ICL的均衡器主要针对静态信道设计，缺乏对非平稳时变信道的理论分析和实际应用研究。本文旨在探索ICL在非平稳问题中的能力，特别是时变信道均衡任务。

Method: 采用原则性框架设计高效注意力机制，从自适应信号处理算法(如LMS、LRMS)中汲取灵感，开发具有改进适应性的注意力变体，包括多步梯度更新以提升长期跟踪能力。

Result: 实验结果表明，ICL在非平稳MIMO均衡中表现出强大潜力，基于经典自适应算法启发的注意力机制能显著增强动态环境中的适应性和性能。

Conclusion: ICL在非平稳信道均衡中具有重要应用价值，基于自适应算法启发的注意力机制设计为开发具有更强适应性和鲁棒性的下一代无线基础模型提供了关键见解。
...
### [251] [Enhancing Self-Supervised Learning with Semantic Pairs A New Dataset and Empirical Study](https://arxiv.org/abs/2510.08722)
...
<details>
  <summary>Details</summary>
Motivation: 通过将每个数据实例视为独立类别，鼓励模型学习对数据变换具有不变性的表示，从而在没有人工标注的情况下获得有效的特征表示。

Method: 对每个实例应用随机变换生成两个不同的视图，训练模型将这些不同视图的表示拉近，学习对变换不变的表示。

Result: 模型能够学习到对常见数据变换具有不变性的表示，这些表示可以用于下游任务。

Conclusion: 实例判别是一种有效的自监督学习方法，通过将实例视为独立类别并学习视图不变性表示，能够在无监督情况下获得良好的特征表示。
...
### [252] [Counterfactually Fair Conformal Prediction](https://arxiv.org/abs/2510.08724)
*Ozgur Guldogan,Neeraj Sarna,Yuanyuan Li,Michael Berger*

Main category: cs.LG

TL;DR: 提出了Counterfactually Fair Conformal Prediction (CF-CP)方法，通过对称化保护属性干预的符合度分数，在保持边际覆盖性质的同时实现反事实公平的预测集。


<details>
  <summary>Details</summary>
Motivation: 现有的反事实公平研究主要关注点预测器，而预测集在不确定性下的公平决策中至关重要。同时，保形预测虽然能提供有效的预测集，但不能保证反事实公平性。

Method: 通过对称化保护属性干预的符合度分数，开发了CF-CP方法，在保持保形预测的边际覆盖性质的同时实现反事实公平。
...
### [253] [Transmuting prompts into weights](https://arxiv.org/abs/2510.08734)
*Hanna Mazzawi,Benoit Dherin,Michael Munn,Michael Wunder,Javier Gonzalvo*

Main category: cs.LG

TL;DR: 本文为基于内部状态修改的LLM控制技术提供了理论基础，解释了这些干预措施如何从Transformer架构的基本计算中产生，并提出了将文本输入转化为可重用权重更新的原则性方法。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型控制技术虽然有效，但通常基于经验启发式方法，缺乏理论基础。本文旨在为这些向量和矩阵干预技术提供数学解释和理论支撑。

Method: 基于Dherin等人的发现，将理论推广到深度多块Transformer，展示用户提示信息如何通过权重向量和矩阵在内部表示和组合，并推导出将信息压缩为token无关思想向量和思想矩阵的原则性方法。

Result: 提出的思想向量和思想矩阵为现有的向量和矩阵模型编辑技术提供了理论解释，并提供了将文本输入转化为可重用权重更新的计算方法。

Conclusion: 该工作为LLM推理时控制技术建立了理论基础，揭示了文本输入与权重更新之间的数学联系，为更原则性的模型编辑方法提供了指导。
...
### [254] [SHAP-Based Supervised Clustering for Sample Classification and the Generalized Waterfall Plot](https://arxiv.org/abs/2510.08737)
*Justin Lin,Julia Fukuyama*

Main category: cs.LG

TL;DR: SHAP分析结合聚类方法，通过将样本按预测原因分组，揭示黑盒模型预测的多种路径，并在阿尔茨海默病案例中验证该方法。


<details>
  <summary>Details</summary>
Motivation: 大型黑盒模型虽然能处理复杂数据，但缺乏可解释性，在高风险场景中不可靠。SHAP分析能解释模型预测，但需要进一步分析预测背后的原因模式。

Method: 使用SHAP值量化特征对预测的贡献，然后对这些值进行聚类，将预测原因相似的样本分组，并提出了多分类瀑布图的可视化方法。

Result: 通过模拟实验和阿尔茨海默病案例研究，展示了该方法能有效识别不同的预测路径，为理解模型决策提供新视角。

Conclusion: SHAP值聚类方法能揭示黑盒模型预测的多样性路径，增强模型可解释性，特别是在高风险医疗应用中具有重要意义。
...
### [255] [Faithful and Interpretable Explanations for Complex Ensemble Time Series Forecasts using Surrogate Models and Forecastability Analysis](https://arxiv.org/abs/2510.08739)
*Yikai Zhao,Jiekai Ma*

Main category: cs.LG

TL;DR: 提出了一个双方法框架，通过替代模型解释和频谱可预测性分析来解决复杂时间序列集成模型的解释性和可预测性问题。
...
Method: 1) 训练LightGBM替代模型来模拟AutoGluon的预测，实现稳定的SHAP特征归因；2) 集成频谱可预测性分析，通过比较时间序列与纯噪声基准来量化固有可预测性。

Result: 在M5数据集上的实证评估显示，更高的频谱可预测性不仅与改进的预测精度强相关，还与替代模型和原始模型之间的更高保真度相关。
...
### [256] [Graph Diffusion Transformers are In-Context Molecular Designers](https://arxiv.org/abs/2510.08744)
...
<details>
  <summary>Details</summary>
Motivation: 现有上下文学习在分子设计中效果有限，且每个分子属性的标记数据稀缺，需要一种能够利用少量示例进行分子设计的方法。

Method: 使用演示条件扩散模型，通过少量分子-评分示例定义任务上下文，指导去噪Transformer生成符合目标属性的分子。开发了节点对编码的分子分词器，将分子表示在基序层面，减少了5.5倍的节点数量。

Result: 在6个类别的33个设计任务中，DemoDiff匹配或超越了比它大100-1000倍的语言模型，平均排名为3.63，优于领域特定方法的5.25-10.20。

Conclusion: DemoDiff作为一个分子基础模型，在上下文分子设计中表现出色，为分子设计提供了新的有效方法。
...
### [257] [RFOD: Random Forest-based Outlier Detection for Tabular Data](https://arxiv.org/abs/2510.08747)
...
Method: 将异常检测重新定义为特征级条件重构问题，为每个特征训练专门的随机森林模型，结合调整后的Gower距离进行单元格级评分，并使用不确定性加权平均聚合为行级异常分数。

Result: 在15个真实世界数据集上的实验表明，RFOD在检测准确率上持续优于最先进的基线方法，同时为混合类型表格数据提供了更好的鲁棒性、可扩展性和可解释性。

Conclusion: RFOD框架成功解决了混合类型表格数据异常检测中的关键挑战，提供了一种既准确又可解释的解决方案，在高风险领域具有重要应用价值。
...
### [258] [Conformal Risk Training: End-to-End Optimization of Conformal Risk Control](https://arxiv.org/abs/2510.08748)
*Christopher Yeh,Nicolas Christianson,Adam Wierman,Yisong Yue*

Main category: cs.LG

TL;DR: 本文提出了一种控制优化确定性等价(OCE)风险的方法，扩展了传统的保形风险控制框架，能够处理尾风险，并引入了端到端的保形风险训练方法。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型虽然预测准确率高，但缺乏可证明的风险或可靠性保证，这在高风险应用中至关重要。现有保形风险控制方法只能控制期望损失，无法处理尾风险，且后处理方法会降低平均性能。

Method: 开发了控制OCE风险类的方法，包括条件风险价值等尾风险度量；引入端到端的保形风险训练，在模型训练或微调期间通过保形OCE风险控制进行微分。

Result: 方法在控制分类器假阴性率和电池存储操作金融风险的应用中，实现了可证明的风险保证，并显著优于后处理方法的平均性能。

Conclusion: 提出的保形风险训练方法能够提供可证明的风险保证，同时显著改善平均性能，为高风险应用中的深度学习部署提供了更可靠的解决方案。
...
### [259] [Exploring Cross-Client Memorization of Training Data in Large Language Models for Federated Learning](https://arxiv.org/abs/2510.08750)
*Tinnakit Udsa,Can Udomcharoenchaikit,Patomporn Payoungkhamdee,Sarana Nutanong,Norrathep Rattanavipanon*

Main category: cs.LG

TL;DR: 提出了一个联邦学习框架，用于量化客户端内部和客户端之间的细粒度跨样本记忆化风险，发现FL模型确实会记忆客户端数据，特别是客户端内部数据。
...
Method: 提出了一个量化FL中客户端内部和客户端之间记忆化的框架，使用细粒度跨样本记忆化测量方法，并研究了解码策略、前缀长度和FL算法等关键因素。

Result: FL模型确实会记忆客户端数据，特别是客户端内部数据多于客户端间数据，记忆化程度受训练和推理因素的影响。

Conclusion: 该框架填补了FL中细粒度记忆化评估的空白，揭示了FL模型存在记忆化风险，特别是客户端内部数据的记忆化更为显著。
...
### [260] [LOTION: Smoothing the Optimization Landscape for Quantized Training](https://arxiv.org/abs/2510.08757)
*Mujin Kwun,Depen Morwani,Chloe Huangyuan Su,Stephanie Gil,Nikhil Anand,Sham Kakade*

Main category: cs.LG

TL;DR: 本文提出LOTION框架，通过随机噪声平滑技术解决量化目标优化中的梯度问题，保证标准优化器收敛到局部最小值，并在语言模型上优于标准QAT方法。
...
Method: 受Nesterov平滑启发，提出LOTION框架：用无偏随机舍入噪声下的期望损失来近似量化损失表面，将量化损失替换为其在随机噪声下的期望。

Result: 在该框架下，标准优化器保证收敛到损失表面的局部最小值；使用随机舍入噪声时，原始量化损失的全局最小值得以保留；在合成测试集和150M/300M参数语言模型上优于标准QAT。

Conclusion: LOTION提供了一个原则性的平滑框架，有效解决了量化优化中的梯度问题，保证了收敛性，并在实际应用中表现出优越性能。
...
### [261] [Spatial Deconfounder: Interference-Aware Deconfounding for Spatial Causal Inference](https://arxiv.org/abs/2510.08762)
*Ayush Khot,Miruna Oprescu,Maresa Schröder,Ai Kagawa,Xihaier Luo*

Main category: cs.LG

TL;DR: 提出Spatial Deconfounder方法，通过利用干扰揭示潜在混杂因素结构，同时解决空间因果推断中的未测量混杂和干扰问题


<details>
  <summary>Details</summary>
Motivation: 空间因果推断面临两个交织的挑战：未测量的空间混杂因素（如天气、空气污染）和附近处理的干扰效应。现有方法通常只解决其中一个问题而忽略另一个

Method: 两阶段方法：使用带有空间先验的条件变分自编码器从局部处理向量重构替代混杂因素，然后通过灵活的结果模型估计因果效应

Result: 在环境健康和社会科学真实数据集上，Spatial Deconfounder持续改进了效应估计，扩展了SpaCE基准套件以包含处理干扰
...
### [262] [Reinforcement Learning-Based Optimization of CT Acquisition and Reconstruction Parameters Through Virtual Imaging Trials](https://arxiv.org/abs/2510.08763)
...
TL;DR: 结合虚拟成像工具和强化学习优化CT协议，在保证图像质量的同时减少辐射剂量，比传统穷举法节省79.7%的计算步骤。
...
Method: 使用虚拟成像工具模拟人体肝脏病变模型，结合PPO强化学习算法优化CT采集和重建参数（管电压、管电流、重建核、层厚、像素尺寸）。

Result: 强化学习方法在所有测试案例中都达到了全局最大可检测性指数(d')，比穷举搜索节省79.7%的计算步骤。

Conclusion: 虚拟成像工具与强化学习的结合为CT协议管理提供了准确且计算高效的优化框架，具有很好的灵活性。
...
### [263] [Zero-Shot Policy Transfer in Reinforcement Learning using Buckingham's Pi Theorem](https://arxiv.org/abs/2510.08768)
...
<details>
  <summary>Details</summary>
Motivation: 强化学习策略在新机器人、任务或不同物理参数环境中的泛化能力差，限制了其实际应用。

Method: 基于白金汉π定理，通过无量纲空间缩放预训练策略的输入（观测）和输出（动作），实现零样本迁移。

Result: 在三个复杂度递增的环境中验证：模拟钟摆、物理钟摆（sim-to-real验证）和高维HalfCheetah。缩放迁移在动态相似情境下无性能损失，在非相似情境下始终优于朴素迁移，显著扩展了原始策略的有效应用范围。
...
### [264] [Struc-EMB: The Potential of Structure-Aware Encoding in Language Embeddings](https://arxiv.org/abs/2510.08774)
*Shikun Liu,Haoyu Wang,Mufei Li,Pan Li*

Main category: cs.LG

TL;DR: 本文提出了结构感知文本嵌入的新范式，通过将超链接、引用等结构关系直接整合到LLM内部编码过程中，而非传统的事后聚合方法，显著提升了嵌入质量。
...
Method: 研究了两种主要的内部处理方法：顺序拼接和并行缓存，并针对噪声结构数据引入了上下文蒸馏和语义平衡两种有效技术。

Result: 在检索、聚类、分类和推荐任务的零样本实验中，结构感知方法始终优于纯文本和事后聚合基线。顺序拼接在噪声中等长度上下文中表现更好，而并行缓存在长高信号上下文中扩展性更强。

Conclusion: 这项工作首次全面分析了内部结构感知编码，为构建更强大、上下文感知的嵌入模型提供了蓝图。
...
### [265] [Guiding Exploration in Reinforcement Learning Through LLM-Augmented Observations](https://arxiv.org/abs/2510.08779)
*Vaibhav Jain,Gerrit Grossmann*

Main category: cs.LG

TL;DR: 提出了一种将LLM生成的行动建议通过增强观察空间提供给RL智能体的框架，让智能体学习何时遵循或忽略这些指导，在稀疏奖励环境中显著提升训练效率和成功率。


<details>
  <summary>Details</summary>
Motivation: 传统RL智能体在稀疏奖励环境中难以发现有效行动序列，而LLM拥有程序性知识和推理能力，但现有方法要么强制RL策略遵循LLM建议，要么将其直接纳入奖励函数，缺乏灵活性。

Method: 通过增强观察空间提供LLM生成的行为建议，让RL智能体学习何时遵循或忽略指导，利用LLM的世界知识和推理能力，同时通过软约束保持灵活性。

Result: 在三个复杂度递增的BabyAI环境中评估，LLM指导的益处随任务难度增加而提升。在最复杂环境中，最终成功率相对基线提升71%，样本效率提升高达9倍。
...
### [266] [Weights initialization of neural networks for function approximation](https://arxiv.org/abs/2510.08780)
*Xinwen Hu,Yunqing Huang,Nianyu Yi,Peimeng Yin*

Main category: cs.LG

TL;DR: 提出基于基函数预训练的可重用初始化框架，通过预训练多项式基函数网络，然后用于初始化复杂目标函数的网络，提高训练效率、泛化能力和模型可迁移性。
...
### [267] [Deceptive Exploration in Multi-armed Bandits](https://arxiv.org/abs/2510.08794)
*I. Arda Vurankaya,Mustafa O. Karabag,Wesley A. Suttle,Jesse Milzman,David Fridovich-Keil,Ufuk Topcu*

Main category: cs.LG

TL;DR: 本文研究多臂老虎机中的欺骗性学习问题，代理在观察者监督下秘密寻找最佳私有奖励臂，同时避免被发现。


<details>
  <summary>Details</summary>
Motivation: 在监督学习环境中，代理可能需要在遵守公开规则的同时秘密优化私有目标，这引发了如何在不被发现的情况下进行欺骗性学习的问题。

Method: 建立KL散度约束下的检测模型，将公开次优臂的成功拉动建模为伯努利过程，提出基于top-two算法的启发式方法。

Result: 证明在KL约束下，公开次优臂的拉动最多以Θ(√T)速率进行，并给出了最佳私有臂识别的最优错误指数特征。

Conclusion: 提出了一个在监督环境下进行欺骗性学习的理论框架和实用算法，能够在避免检测的同时有效识别最佳私有臂。
...
### [268] [PO-CKAN:Physics Informed Deep Operator Kolmogorov Arnold Networks with Chunk Rational Structure](https://arxiv.org/abs/2510.08795)
*Junyi Wu,Guang Lin*

Main category: cs.LG

TL;DR: PO-CKAN是一个基于分块有理Kolmogorov-Arnold网络(KAN)的物理信息深度算子框架，用于近似偏微分方程的解算子。该框架结合了DeepONet架构和物理信息神经网络(PINN)原理，能够高效学习物理一致的时空解算子。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够高效学习偏微分方程解算子的框架，同时保证物理一致性，并能够快速预测参数化时变PDE在不同输入条件下的解。

Method: 采用DeepONet风格的分支-主干架构，使用分块有理KAN模块作为子网络，通过PDE残差损失(PINN风格)来强制物理一致性。

Result: 在Burgers方程(ν=0.01)上，PO-CKAN相比PI-DeepONet将平均相对L²误差降低了约48%，在Eikonal和扩散-反应基准测试中也达到了竞争性精度。

Conclusion: PO-CKAN框架能够准确学习偏微分方程的解算子，结果与高保真解高度匹配，证明了其在物理一致算子学习方面的有效性。
...
### [269] [TAPAS: Datasets for Learning the Learning with Errors Problem](https://arxiv.org/abs/2510.08797)
*Eshika Saxena,Alberto Alfarano,François Charton,Emily Wenger,Kristin Lauter*

Main category: cs.LG

TL;DR: 提出了TAPAS数据集，用于加速AI在LWE攻击方面的研究，填补了该领域缺乏可访问数据的空白。


<details>
  <summary>Details</summary>
Motivation: AI在LWE攻击方面显示出潜力，但缺乏可访问的训练数据限制了AI从业者的研究和改进能力。

Method: 创建了TAPAS数据集，这是一个用于后量子密码学AI分析的工具包，覆盖多种LWE设置，可供AI从业者直接使用。

Result: 建立了攻击性能基线，为未来工作奠定了基础。

Conclusion: TAPAS数据集将加速AI在LWE攻击方面的研究，并推动该领域的发展。
...
### [270] [Edu-EmotionNet: Cross-Modality Attention Alignment with Temporal Feedback Loops](https://arxiv.org/abs/2510.08802)
*S M Rafiuddin*

Main category: cs.LG

TL;DR: 提出了Edu-EmotionNet框架，用于在线教育中的情感识别，通过动态融合多模态数据和建模时间情感演变，解决了现有方法对模态可靠性假设不切实际的问题。


<details>
  <summary>Details</summary>
Motivation: 在线教育中理解学习者情感对提高参与度和个性化教学至关重要。现有情感识别方法依赖静态融合策略并假设模态输入始终可靠，这在真实学习环境中很少成立。

Method: Edu-EmotionNet包含三个关键组件：跨模态注意力对齐模块用于动态跨模态上下文共享，模态重要性估计器为每个时间步的每个模态分配基于置信度的权重，时间反馈循环利用先前预测来强制时间一致性。

Result: 在教育子集IEMOCAP和MOSEI上评估，重新标注了困惑、好奇、无聊和沮丧等情感，Edu-EmotionNet实现了最先进的性能，并对缺失或噪声模态表现出强鲁棒性。
...
### [271] [TinyGraphEstimator: Adapting Lightweight Language Models for Graph Structure Inference](https://arxiv.org/abs/2510.08808)
*Michal Podstawski*
...
<details>
  <summary>Details</summary>
Motivation: 探索资源高效的小型语言模型在图结构推理任务中的潜力，填补了该领域的研究空白。

Method: 使用TinyGraphEstimator数据集评估多个小型开源模型，并应用LoRA轻量级微调技术进行参数优化。

Result: 小型语言模型在图结构数据上展现出非平凡的推理能力，通过高效参数调优在所有评估指标上获得一致改进。

Conclusion: 紧凑型transformer语言模型能够有效执行图结构推理任务，为资源受限环境下的图分析提供了可行方案。
...
### [272] [Long-Tailed Recognition via Information-Preservable Two-Stage Learning](https://arxiv.org/abs/2510.08836)
*Fudong Lin,Xu Yuan*

Main category: cs.LG

TL;DR: 提出一种两阶段学习方法来缓解长尾数据分布中的多数类偏差问题，通过信息理论视角的表示学习和数学信息采样策略，在多个基准数据集上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现实世界数据分布通常存在不平衡（长尾）特性，这导致深度分类模型偏向频繁类别，对尾部类别表现不佳。

Method: 第一阶段：从信息理论角度提出新的表示学习技术，理论上等效于最小化类内距离；第二阶段：开发新颖的采样策略，选择数学上信息丰富的实例来修正多数类偏差的决策边界。

Result: 在多个长尾基准数据集上实现了最先进的性能，并通过广泛实验验证。

Conclusion: 该方法能有效缓解多数类偏差倾向，同时保留数据集中的有价值信息，在长尾分类问题上表现优异。
...
### [273] [Reinforcement Learning-Driven Edge Management for Reliable Multi-view 3D Reconstruction](https://arxiv.org/abs/2510.08839)
*Motahare Mounesan,Sourya Saha,Houchao Gan,Md. Nurul Absur,Saptarshi Debroy*

Main category: cs.LG

TL;DR: 提出基于强化学习的边缘资源管理框架，用于在资源受限和易受干扰的环境中实现可靠的多视角3D重建，通过两个协同Q-learning智能体分别处理摄像头选择和服务器选择。
...
Method: 采用两个协同Q-learning智能体：一个负责摄像头选择，一个负责服务器选择，两者完全在线运行，通过与边缘环境交互学习策略。在包含实验室终端设备和FABRIC基础设施边缘服务器的分布式测试平台上进行评估。

Result: 该框架在动态环境中有效平衡端到端延迟和重建质量，提高了应用可靠性。

Conclusion: 强化学习框架能够在不稳定和资源受限的边缘环境中实现可靠的3D重建，为智能城市边缘基础设施提供了有效的资源管理解决方案。
...
### [274] [The Boundaries of Fair AI in Medical Image Prognosis: A Causal Perspective](https://arxiv.org/abs/2510.08840)
*Thai-Hoang Pham,Jiayuan Chen,Seungyeon Lee,Yuanlong Wang,Sayoko Moroi,Xueru Zhang,Ping Zhang*

Main category: cs.LG

TL;DR: FairTTE是首个用于评估医学影像中时间到事件预测公平性的综合框架，揭示了医学影像数据中普遍存在的偏见问题，并发现现有公平性方法缓解效果有限。
...
Method: 开发了FairTTE框架，整合了先进的TTE预测和公平性算法，利用因果分析技术识别和量化医学影像数据集中的偏见来源。

Result: 大规模评估显示偏见在不同影像模态中普遍存在，现有公平性方法缓解效果有限，公平性在分布偏移下难以维持。
...
### [275] [On the Alignment Between Supervised and Self-Supervised Contrastive Learning](https://arxiv.org/abs/2510.08852)
*Achleshwar Luthra,Priyadarsi Mishra,Tomer Galanti*

Main category: cs.LG

TL;DR: 本文分析了自监督对比学习(CL)和负样本监督对比学习(NSCL)在表示层面的对齐关系，证明了它们在表示相似性矩阵上保持接近，但参数空间会发散。


<details>
  <summary>Details</summary>
Motivation: 虽然已有理论表明CL损失近似于NSCL损失，但缺乏对训练过程中表示层面是否保持对齐的研究。

Method: 在共享随机性条件下训练CL和NSCL模型，分析其表示相似性矩阵的接近程度，并给出对齐度量的概率保证。

Result: CL和NSCL的表示相似性矩阵在现实条件下保持接近，对齐度随类别数、温度增加而改善，但参数空间会指数级发散。

Conclusion: NSCL可作为自监督和监督学习之间的理论桥梁，表示层面的对齐为理解CL的成功提供了新视角。
...
### [276] [Time-Aware Feature Selection: Adaptive Temporal Masking for Stable Sparse Autoencoder Training](https://arxiv.org/abs/2510.08855)
*T. Ed Li,Junyu Ren*

Main category: cs.LG

TL;DR: 提出自适应时间掩码(ATM)方法来解决稀疏自编码器训练中的特征吸收问题，通过动态跟踪激活特征的重要性分数并进行概率掩码，显著降低了特征吸收，同时保持良好重构质量。


<details>
  <summary>Details</summary>
Motivation: 当前稀疏自编码器训练方法存在特征吸收问题，即特征相互吸收以最小化L1惩罚，这阻碍了对模型行为的稳定识别和分析。

Method: ATM方法动态跟踪激活幅度、频率和重构贡献来计算随时间演化的重要性分数，基于统计阈值应用概率掩码机制进行特征选择。
...
### [277] [Sparse components distinguish visual pathways & their alignment to neural networks](https://arxiv.org/abs/2510.08858)
*Ammar I Marvi,Nancy G Kanwisher,Meenakshi Khosla*

Main category: cs.LG

TL;DR: 本文通过稀疏分解方法分析人类高级视觉皮层中腹侧、背侧和外侧通路的表征差异，并提出了稀疏成分对齐(SCA)方法来更精确地测量大脑与机器视觉系统之间的表征对齐程度。


<details>
  <summary>Details</summary>
Motivation: 探索为什么在人类视觉皮层中功能不同的腹侧、背侧和外侧通路，却能被单一任务训练的深度神经网络很好地建模，这暗示了这些通路之间可能存在共同的计算原理。

Method: 采用新颖的稀疏分解方法识别各视觉通路中的主导表征成分，并提出了稀疏成分对齐(SCA)方法来测量大脑与机器视觉系统之间的表征对齐。

Result: 发现三个视觉通路的成分响应特征存在明显差异：腹侧通路对脸部、场景、身体、文字和食物敏感；外侧通路对社会互动、隐含运动和手部动作敏感；背侧通路包含一些难以解释的成分。SCA显示标准视觉DNN与腹侧通路的对齐程度高于背侧或外侧通路。

Conclusion: 稀疏成分对齐方法能够比传统群体水平几何方法更精确地揭示大脑与机器视觉系统之间的表征对齐差异，提供了一种对系统潜在神经调谐轴敏感的测量方法。
...
### [278] [Titans Revisited: A Lightweight Reimplementation and Critical Analysis of a Test-Time Memory Model](https://arxiv.org/abs/2510.09551)
*Gavriel Di Nepi,Federico Siciliano,Fabrizio Silvestri*

Main category: cs.LG

TL;DR: 对Google Titans模型进行轻量级重实现和评估，发现其神经记忆组件能提升性能，但整体表现不总是优于基线方法。


<details>
  <summary>Details</summary>
Motivation: Google Titans模型缺乏公开代码且原描述模糊，阻碍了可复现性研究。

Method: 通过轻量级重实现Titans模型，在掩码语言建模、时间序列预测和推荐任务上进行全面评估。

Result: Titans模型由于分块处理并不总是优于基线方法，但其神经记忆组件相比仅使用注意力的模型能持续提升性能。
...
### [279] [Multi-fidelity Batch Active Learning for Gaussian Process Classifiers](https://arxiv.org/abs/2510.08865)
*Murray Cutforth,Yiming Yang,Tiffany Fan,Serge Guillas,Eric Darve*

Main category: cs.LG

TL;DR: 提出了一种用于多保真度高斯过程分类器的批量主动学习算法BPMI，通过泰勒展开近似计算互信息，在固定计算预算下实现了更高的预测精度。


<details>
  <summary>Details</summary>
Motivation: 许多科学和工程问题依赖昂贵的计算模拟，多保真度方法可以加速参数空间探索。需要研究在二值模拟输出情况下，如何有效分配模拟预算。

Method: 引入Bernoulli参数互信息(BPMI)算法，通过一阶泰勒展开链接函数来规避概率空间中计算互信息的难处理性，实现批量主动学习。

Result: 在两个合成测试案例和一个涉及激光点火火箭燃烧器模拟的复杂真实应用中，BPMI在所有实验中均表现出优越性能。

Conclusion: BPMI算法在固定计算预算下能够实现更高的预测精度，为多保真度高斯过程分类器提供了有效的批量主动学习方法。
...
### [280] [An Improved Model-Free Decision-Estimation Coefficient with Applications in Adversarial MDPs](https://arxiv.org/abs/2510.08882)
*Haolin Liu,Chen-Yu Wei,Julian Zimmert*

Main category: cs.LG

TL;DR: 提出了Dig-DEC，一种基于信息增益的模型自由决策估计系数，用于处理结构化观测决策问题，在对抗性环境中无需显式奖励估计器，并在混合MDP中实现了首个模型自由遗憾边界。


<details>
  <summary>Details</summary>
Motivation: 现有基于乐观DEC的方法只能处理随机环境，无法扩展到对抗性环境，且存在遗憾边界与模型类大小相关的差距。需要一种更通用的方法来处理对抗性环境并缩小理论差距。

Method: 引入Dig-DEC，通过纯信息增益驱动探索而非乐观主义，将其应用于具有随机转移和对抗性奖励的混合MDP，并改进了在线函数估计过程。

Result: Dig-DEC始终不大于乐观DEC且在特殊情况下更小，在混合MDP中获得了首个模型自由遗憾边界，改进了估计误差最小化的遗憾边界从T^{3/4}到T^{2/3}，在Bellman完备MDP中从T^{2/3}改进到√T。

Conclusion: Dig-DEC提供了一种处理对抗性环境的通用方法，缩小了理论差距，并在多个场景下实现了与乐观主义方法匹配的性能，解决了Liu等人留下的主要开放问题。
...
### [281] [Pinpointing crucial steps: Attribution-based Credit Assignment for Verifiable Reinforcement Learning](https://arxiv.org/abs/2510.08899)
*Junxi Yin,Haisen Luo,Zhenyu Li,Yihua Liu,Dan Liu,Zequn Li,Xiaohang Xu*

Main category: cs.LG

TL;DR: 提出了ACPO框架，通过难度感知课程和基于归因的策略优化，解决强化学习中探索与利用的平衡问题，显著提升复杂推理任务的性能。
...
Method: ACPO框架采用分阶段方法，包含轨迹语义分割和基于归因的表征来动态调节策略熵，同时使用因子化奖励系统精确量化每个推理步骤的层次贡献。
...
### [282] [A Frequency-Domain Analysis of the Multi-Armed Bandit Problem: A New Perspective on the Exploration-Exploitation Trade-off](https://arxiv.org/abs/2510.08908)
*Di Zhang*
...
<details>
  <summary>Details</summary>
Motivation: 现有分析主要从时域和累积遗憾角度出发，难以刻画学习过程的动态特性，需要新的分析框架来理解探索与利用的动态平衡。
...
Conclusion: 该理论为经典算法提供了新颖直观的物理解释，并为设计具有自适应参数调整的下一代算法奠定了严格理论基础。
...
### [283] [Deep Multimodal Subspace Clustering Networks](https://arxiv.org/abs/1804.06498)
*Mahdi Abavisani,Vishal M. Patel*

Main category: cs.LG

TL;DR: 提出了基于卷积神经网络的无监督多模态子空间聚类方法，包含编码器、自表达层和解码器三阶段，探索了早期、晚期和中间融合技术，并在三个数据集上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决多模态数据子空间聚类问题，通过神经网络框架有效融合不同模态信息，提升聚类性能。

Method: 使用三阶段框架：多模态编码器融合数据到潜在空间，自表达层获取亲和矩阵，解码器重构输入数据。研究了早期、晚期和中间融合技术，并提出了亲和融合网络。

Result: 在三个数据集上的广泛实验表明，所提方法显著优于最先进的多模态子空间聚类方法。

Conclusion: 提出的基于CNN的多模态子空间聚类框架有效融合不同模态信息，在多个数据集上取得了优越的聚类性能。
...
### [284] [Velocity and Density-Aware RRI Analysis and Optimization for AoI Minimization in IoV SPS](https://arxiv.org/abs/2510.08911)
*Maoxin Ji,Tong Wang,Qiong Wu,Pingyi Fan,Nan Cheng,Wen Chen*

Main category: cs.LG

TL;DR: 提出基于LLM和DDPG的优化方法，解决车联网SPS中的AoI恶化问题，通过双路径优化方案显著降低信息年龄。
...
Method: 建立受车辆速度、密度和资源预留间隔影响的AoI计算模型，设计双路径优化方案：DDPG基于状态空间和奖励函数进行指导，LLM利用上下文学习生成最优参数配置。
...
### [285] [Simple and Robust Forecasting of Spatiotemporally Correlated Small Earth Data with A Tabular Foundation Model](https://arxiv.org/abs/2510.08920)
*Yuting Yang,Gang Mei,Zhengjing Ma,Nengxiong Xu,Jianbing Peng*

Main category: cs.LG

TL;DR: 提出了一种针对小地球数据的时空预测方法，通过量化时空模式并利用表格基础模型实现跨场景的准确预测，在多个典型场景中优于传统深度学习模型和表格基础模型。


<details>
  <summary>Details</summary>
Motivation: 传统深度学习模型需要针对不同场景进行特定训练，而基础模型存在对预训练分布全局均值的预测偏差问题，需要一种简单且鲁棒的方法来处理小地球数据的时空预测。

Method: 通过表征和量化小地球数据的时空模式，然后利用表格基础模型进行跨场景的准确预测。

Result: 在三个典型场景的比较结果表明，该方法在大多数情况下比图深度学习模型（T-GCN）和表格基础模型（TabPFN）具有更优的准确性，并展现出更强的鲁棒性。

Conclusion: 该方法为小地球数据的时空预测提供了一种简单、准确且鲁棒的解决方案，能够有效处理不同场景下的预测任务。
...
### [286] [AB-PINNs: Adaptive-Basis Physics-Informed Neural Networks for Residual-Driven Domain Decomposition](https://arxiv.org/abs/2510.08924)
*Jonah Botvinick-Greenhouse,Wael H. Ali,Mouhacine Benosman,Saviz Mowlavi*

Main category: cs.LG

TL;DR: 提出自适应基物理信息神经网络（AB-PINNs），通过动态调整子域来改进PINNs训练，特别适用于多尺度问题。
...
Method: 基于残差损失动态调整子域，在高残差区域引入新子域，不同子域学习捕捉不同尺度的解特征。

Result: AB-PINNs能有效解决多种复杂多尺度偏微分方程，减少超参数调优需求并避免局部最小值。

Conclusion: 自适应域分解方法为PINNs训练提供了灵活有效的解决方案，特别适用于多尺度问题。
...
### [287] [MATT-CTR: Unleashing a Model-Agnostic Test-Time Paradigm for CTR Prediction with Confidence-Guided Inference Paths](https://arxiv.org/abs/2510.08932)
*Moyu Zhang,Yun Chen,Yujun Jin,Jinxin Hu,Yu Zhang,Xiaoyi Zeng*

Main category: cs.LG

TL;DR: 提出了一种模型无关的测试时优化范式MATT，通过置信度引导的多路径推理来缓解低频特征组合对CTR预测的影响，提升模型推理阶段的性能。


<details>
  <summary>Details</summary>
Motivation: 现有CTR模型优化主要关注训练阶段，忽视了推理阶段的优化机会。低频特征组合会降低预测性能，导致不可靠的输出结果。

Method: 使用分层概率哈希方法量化特征组合的置信度，基于置信度分数通过迭代采样生成多个实例特定的推理路径，最后聚合多路径的预测分数进行鲁棒预测。

Result: 离线和在线A/B测试都验证了MATT与现有CTR模型的兼容性和有效性。

Conclusion: MATT能够释放已训练CTR模型的预测潜力，通过测试时优化提升模型性能。
...
### [288] [Bi-level Meta-Policy Control for Dynamic Uncertainty Calibration in Evidential Deep Learning](https://arxiv.org/abs/2510.08938)
*Zhen Yang,Yansong Ma,Lei Chen*

Main category: cs.LG

TL;DR: 提出元策略控制器(MPC)，通过动态元学习框架自适应调整KL散度系数和狄利克雷先验强度，解决传统证据深度学习在动态数据分布下不确定性校准不足的问题。
...
Result: 实验结果表明MPC显著提升了模型预测的可靠性和校准性能，改善了不确定性校准、预测准确率以及基于置信度样本剔除后的性能保持。

Conclusion: 可学习的狄利克雷先验能够灵活适应类别分布和训练动态，MPC框架在各种任务中有效增强了模型不确定性建模能力。
...
### [289] [Variability Aware Recursive Neural Network (VARNN): A Residual-Memory Model for Capturing Temporal Deviation in Sequence Regression Modeling](https://arxiv.org/abs/2510.08944)
*Haroon Gharwi,Kai Shu*

Main category: cs.LG

TL;DR: 提出VARNN模型，一种残差感知的神经网络架构，通过学习近期预测残差的显式误差记忆来重新校准后续预测，提升时间序列回归在非平稳环境下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现实世界时间序列数据存在非平稳行为、机制转换和时间变化的噪声（异方差性），这些因素会降低标准回归模型的鲁棒性。

Method: VARNN在传统前馈预测器基础上增加了学习的误差记忆状态，该状态从短期上下文步骤的残差中更新，作为变异性和漂移的信号，并用于调节当前时间步的最终预测。

Result: 在多个领域数据集（电器能耗、医疗保健、环境监测）上的实验结果表明，VARNN相比静态、动态和循环基线模型实现了更优的性能，以最小的计算开销获得了更低的测试MSE。
...
### [290] [When LLM Agents Meet Graph Optimization: An Automated Data Quality Improvement Approach](https://arxiv.org/abs/2510.08952)
*Zhihan Zhang,Xunkai Li,Yilong Zuo,Zhenjun Li,Bing Zhou,Rong-Hua Li,Guoren Wang*

Main category: cs.LG

TL;DR: 提出LAGA框架，通过多智能体系统优化文本属性图的数据质量，解决GNN对输入质量敏感的问题


<details>
  <summary>Details</summary>
Motivation: 传统GNN和LLM增强GNN在稀疏、噪声和不平衡等九种场景下性能显著下降，现有方法主要关注模型架构改进而忽视数据本身优化

Method: LAGA框架包含检测、规划、行动和评估四个协作智能体，形成自动化闭环。行动智能体采用双编码器和三目标设计，捕获跨模态互补信息

Result: 在九种场景下的实验表明，LAGA显著提升图质量和各种任务及骨干网络的性能，达到最先进水平

Conclusion: 数据为中心的质量优化是构建可靠文本属性图和鲁棒图学习的关键
...
### [291] [Analytical Survey of Learning with Low-Resource Data: From Analysis to Investigation](https://arxiv.org/abs/2510.08962)
...
TL;DR: 该论文综述了在低资源数据下的学习问题，使用PAC框架分析泛化误差和标签复杂度，探讨了多种优化策略和学习范式，总结了关键发现及其对低资源学习的影响。
...
Method: 采用PAC框架中的不可知主动采样理论，分析监督和无监督设置下的泛化误差和标签复杂度，并研究梯度信息优化、元迭代优化、几何感知优化和LLM驱动优化等策略。

Result: 论文提供了针对低资源数据学习的优化策略概述，并探讨了领域迁移、强化反馈和层次结构建模等多种学习范式。

Conclusion: 总结了低资源数据学习的关键发现，强调了这些发现对实现有限资源下鲁棒学习的重要意义。
...
### [292] [HiBBO: HiPPO-based Space Consistency for High-dimensional Bayesian Optimisation](https://arxiv.org/abs/2510.08965)
*Junyu Xuan,Wenlong Chen,Yingzhen Li*

Main category: cs.LG

TL;DR: 提出HiBBO框架，通过引入HiPPO方法在VAE中构建空间一致性，减少潜在空间与原始空间的功能分布不匹配，提升高维贝叶斯优化的性能。


<details>
  <summary>Details</summary>
Motivation: 传统基于VAE的贝叶斯优化方法在重构目标函数时会导致潜在空间与原始空间的功能分布不匹配，从而影响优化效果。

Method: 使用HiPPO方法在VAE潜在空间构建中引入空间一致性，减少功能分布不匹配问题。

Result: 在高维基准任务上的实验表明，HiBBO在收敛速度和解决方案质量方面优于现有的VAE-BO方法。

Conclusion: HiBBO弥合了高维序列表示学习与高效贝叶斯优化之间的差距，为神经架构搜索、材料科学等领域提供了更广泛的应用可能。
...
### [293] [Learning Regularizers: Learning Optimizers that can Regularize](https://arxiv.org/abs/2510.08968)
*Suraj Kumar Sahoo,Narayanan C Krishnan*

Main category: cs.LG

TL;DR: 研究表明学习优化器能够学习并内化传统正则化技术的效果，无需在目标函数中显式应用正则化，从而在测试准确率和泛化能力上超越未正则化的学习优化器。
...
Method: 通过在标准基准数据集（MNIST、FMNIST、CIFAR）和神经网络架构（MLP、MLP-Relu、CNN）上进行广泛实验，比较使用和不使用显式正则化器训练的学习优化器。

Result: 正则化学习优化器在测试准确率和泛化能力上始终优于未正则化的对应模型，并且能够将正则化效果保留并迁移到新的优化任务中。
...
### [294] [Diagnosing and Mitigating System Bias in Self-Rewarding RL](https://arxiv.org/abs/2510.08977)
...
<details>
  <summary>Details</summary>
Motivation: RLVR受限于标注样本数量，而RLIR在无标签设置中虽然能持续扩展，但性能和稳定性落后于RLVR。研究发现这是由于模型倾向于高估其高置信度轨迹，导致有偏且不稳定的奖励估计。

Method: 提出了RLER方法，通过集成多个模型、自适应奖励插值和轨迹选择来缓解系统偏差。使用三个指标(ρ_noise、ρ_selfbias、ρ_symbias)来表征偏差。

Result: 实验表明RLER相比RLIR提升了13.6%，仅比RLVR低3.6%，在无标签样本上实现了稳定扩展。

Conclusion: RLER有效缓解了RLIR中的系统偏差问题，在无标签设置中实现了接近RLVR的性能，具有很高的应用价值。
...
### [295] [FedL2T: Personalized Federated Learning with Two-Teacher Distillation for Seizure Prediction](https://arxiv.org/abs/2510.08984)
...
<details>
  <summary>Details</summary>
Motivation: 癫痫预测需要大量脑电图数据，但标注成本高且涉及隐私问题。现有联邦学习方法在客户端数据异质性的情况下性能不佳。

Method: 提出双教师知识蒸馏策略，每个客户端同时从全局聚合模型和动态分配的同伴模型中学习；采用自适应多级蒸馏策略对齐预测输出和中间特征表示；引入近端正则化项增强训练稳定性。
...
### [296] [Constraints-of-Thought: A Framework for Constrained Reasoning in Language-Model-Guided Search](https://arxiv.org/abs/2510.08992)
*Kamel Alrashedy,Vriksha Srihari,Zulfiqar Zaidi,Ridam Srivastava,Pradyumna Tambwekar,Matthew Gombolay*

Main category: cs.LG

TL;DR: 提出了Constraints-of-Thought框架，通过意图-约束对结构化表示来引导蒙特卡洛树搜索，提高LLMs在多步规划中的可行性和语义对齐性。


<details>
  <summary>Details</summary>
Motivation: 现有方法如CoT、ToT等在多步规划中经常产生不可行动作或幻觉步骤，难以确保计划符合用户意图和符号约束。

Method: 使用意图-约束对表示推理步骤，压缩搜索空间并确保有效性，将其集成到MCTS中引导搜索朝向可行且有意义的路径。

Result: 在Risk游戏、CAD代码生成和算术推理三个领域中，该方法优于基线方法，获得更高准确性和更强的结构对齐。

Conclusion: Const-of-T为约束引导推理提供了可泛化的基础，使LLMs能够进行更高效、约束对齐和领域自适应的规划。
...
### [297] [PlatformX: An End-to-End Transferable Platform for Energy-Efficient Neural Architecture Search](https://arxiv.org/abs/2510.08993)
*Xiaolong Tu,Dawei Chen,Kyungtae Han,Onur Altintas,Haoxin Wang*

Main category: cs.LG

TL;DR: PlatformX是一个完全自动化的硬件感知神经架构搜索框架，通过集成能量驱动搜索空间、可转移的核级能量预测器、帕累托多目标搜索算法和高分辨率运行时能量分析系统，解决了现有HW-NAS方法在时间成本、手动分析和跨平台可扩展性方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有硬件感知神经架构搜索方法在真实世界部署中存在高时间成本、大量手动分析和跨不同硬件平台可扩展性差的问题，特别是针对具有复杂设备特定能量行为的边缘设备。

Method: PlatformX包含四个关键组件：1）能量驱动搜索空间，扩展传统NAS设计以包含能量关键配置；2）跨设备可转移的核级能量预测器，通过最少设备样本进行增量优化；3）帕累托多目标搜索算法，平衡能量和精度以找到最优权衡；4）高分辨率运行时能量分析系统，使用外部监视器自动化设备功率测量。

Result: 在多个移动平台上评估显示，PlatformX显著减少搜索开销同时保持精度和能量保真度。识别出的模型达到0.94精度或每次推理仅0.16毫焦耳能量消耗，在精度和效率上都优于MobileNet-V2。

Conclusion: PlatformX提供了一个实用且可扩展的HW-NAS解决方案，能够为边缘设备自动设计高效的深度神经网络，解决了现有方法的部署障碍。
...
### [298] [SQS: Bayesian DNN Compression through Sparse Quantized Sub-distributions](https://arxiv.org/abs/2510.08999)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法单独使用权重剪枝或低比特量化，导致压缩率不理想。需要统一框架同时进行两种压缩技术。

Method: 使用spike-and-slab先验诱导稀疏性，采用高斯混合模型(GMMs)建模量化权重，实现低比特精度。
...
### [299] [LLM Unlearning on Noisy Forget Sets: A Study of Incomplete, Rewritten, and Watermarked Data](https://arxiv.org/abs/2510.09007)
*Changsheng Wang,Yihua Zhang,Dennis Wei,Jinghan Jia,Pin-Yu Chen,Sijia Liu*

Main category: cs.LG

TL;DR: 本研究首次探讨了在噪声遗忘数据集下的LLM遗忘问题，发现即使遗忘数据存在扰动，只要核心语义信号保留，遗忘方法仍能保持稳健性。


<details>
  <summary>Details</summary>
Motivation: 现有LLM遗忘方法假设能获取干净、明确定义的遗忘数据，但现实中的遗忘数据往往是低质量、重写或带水印的，这引发了遗忘可靠性的质疑。

Method: 系统性地在噪声遗忘集上对最先进的LLM遗忘方法RMU和NPO进行基准测试，并提出基于显著性的解释框架。

Result: 遗忘对扰动表现出惊人的稳健性，只要核心语义信号得以保留；遗忘算法主要受深层语义线索而非浅层词汇模式引导。

Conclusion: LLM遗忘在噪声数据下具有语义层面的稳健性，关键语义成分在表面形式大幅变化时仍保持一致的遗忘驱动作用。
...
### [300] [Value-State Gated Attention for Mitigating Extreme-Token Phenomena in Transformers](https://arxiv.org/abs/2510.09017)
...
Method: VGA引入基于值向量的数据依赖门控机制，直接调节注意力输出，通过理论分析证明这种方法比基于输入嵌入的门控更有效地解耦值和注意力分数的更新。

Result: 实验表明VGA显著减轻了注意力汇的形成并稳定了值状态范数，提升了模型性能、量化鲁棒性和可解释性。

Conclusion: VGA提供了一种简单而稳定的架构机制，通过直接打破注意力与值状态之间的恶性循环，有效解决了Transformer中的极端token问题。
...
### [301] [Slim Scheduler: A Runtime-Aware RL and Scheduler System for Efficient CNN Inference](https://arxiv.org/abs/2510.09018)
*Ian Harshbarger,Calvin Chidambaram*

Main category: cs.LG

TL;DR: Slim Scheduler是一个混合调度框架，结合PPO强化学习策略和贪婪调度器来协调可瘦身模型的分布式推理，实现动态适应异构硬件和运行时条件。
...
Method: 采用分层设计：服务器运行本地贪婪调度器进行请求批处理和实例扩展管理，PPO路由器学习全局路由策略，包括设备选择、宽度比例和批配置。

Result: 相比纯随机任务分布基线，Slim Scheduler可实现：在精度降至最瘦模型(70.3%)时，平均延迟减少96.45%，能耗减少97.31%；或在提高精度的同时减少平均延迟和能耗，但延迟和能耗标准差增加。
...
### [302] [MagicDock: Toward Docking-oriented De Novo Ligand Design via Gradient Inversion](https://arxiv.org/abs/2510.09020)
*Zekai Chen,Xunkai Li,Sirui Zhang,Henan Sun,Jia Li,Zhenjun Li,Bing Zhou,Rong-Hua Li,Guoren Wang*

Main category: cs.LG

TL;DR: MagicDock是一个基于渐进式流程和可微分表面建模的从头配体设计框架，通过梯度反演和可学习3D点云表示解决现有方法的局限性，在9个场景中相比SOTA基线平均提升27.1%和11.7%。


<details>
  <summary>Details</summary>
Motivation: 解决现有从头配体设计方法存在的三个主要问题：伪从头设计、有限的对接建模和僵化的配体类型限制，实现真正意义上的从头配体生成。

Method: 采用梯度反演框架，将受体和配体的通用对接知识融入骨干模型，通过结合预测实例化为反向梯度流来迭代指导配体生成；强调可微分表面建模，使用可学习3D点云表示精确捕捉结合细节；为不同配体类型提供定制化设计并集成到统一的梯度反演框架中。

Result: 在9个不同场景的广泛实验中，MagicDock相比专门用于蛋白质或分子配体设计的SOTA基线分别实现了27.1%和11.7%的平均提升。

Conclusion: MagicDock通过其创新的梯度反演框架和可微分表面建模，有效解决了现有从头配体设计方法的局限性，为生物医学应用提供了强大的配体生成工具，并具有严格的理论保证。
...
### [303] [The Environmental Impacts of Machine Learning Training Keep Rising Evidencing Rebound Effect](https://arxiv.org/abs/2510.09022)
*Clément Morand,Anne-Laure Ligozat,Aurélie Névéol*

Main category: cs.LG

TL;DR: 该研究分析了近十年来AI系统训练的环境影响，发现即使采用优化策略，机器学习模型训练的环境影响仍在指数级增长，存在反弹效应，仅靠效率提升无法实现可持续ML。
...
Method: 通过估算过去十年间包括大语言模型在内的著名AI系统训练的环境影响，重点关注显卡的完整生命周期分析。

Result: 研究发现两个关键趋势：显卡生产影响持续增加；ML模型训练能耗和环境影响呈指数增长，优化策略无法抵消训练带来的影响，存在反弹效应。

Conclusion: 仅提高效率无法确保ML的可持续性，必须减少AI活动、质疑资源密集型训练的规模和频率，并考虑硬件的全生命周期影响而非仅使用阶段。
...
### [304] [The Attacker Moves Second: Stronger Adaptive Attacks Bypass Defenses Against Llm Jailbreaks and Prompt Injections](https://arxiv.org/abs/2510.09023)
...
TL;DR: 论文指出当前语言模型防御评估方法存在缺陷，应使用自适应攻击者进行测试。作者通过系统优化技术成功绕过了12种最新防御方法，攻击成功率超过90%。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型防御评估方法存在缺陷，要么使用静态攻击字符串，要么使用计算能力弱的优化方法，未能充分考虑自适应攻击者的威胁。

Method: 系统调整和扩展通用优化技术，包括梯度下降、强化学习、随机搜索和人工引导探索，针对12种不同的防御技术进行攻击。

Result: 成功绕过了12种最新防御方法，大多数防御的攻击成功率超过90%，而原始防御报告的攻击成功率接近零。
...
### [305] [Convergence of optimizers implies eigenvalues filtering at equilibrium](https://arxiv.org/abs/2510.09034)
*Jerome Bolte,Quoc-Tung Le,Edouard Pauwels*

Main category: cs.LG

TL;DR: 本文从反向视角研究深度学习优化器，认为不同优化器本质上是特征值滤波器，梯度下降避免最尖锐的极小值，SAM算法更主动选择更宽的极小值盆地。作者提出两种新算法增强特征值过滤效果。


<details>
  <summary>Details</summary>
Motivation: 深度学习训练中多种优化器往往能找到接近全局最优解，本文假设收敛到任意点是给定的，重点研究这种假设的后果，特别是与边缘稳定性现象相关的优化器行为。

Method: 采用广义Hadamard-Perron稳定流形定理，分析一般半代数C^2函数，无需额外非退化条件或全局Lipschitz边界假设。提出两种增强特征值过滤的新算法，并通过前馈神经网络进行数值实验验证。

Result: 理论分析和实验表明，标准梯度下降方法固有地避免最尖锐的极小值，而SAM算法更主动地偏好更宽的盆地。新提出的算法在促进更宽极小值方面表现更好。

Conclusion: 不同优化器通过其超参数确定的特征值过滤机制影响收敛点的选择，这为理解优化器行为提供了新视角，并启发了设计更好优化算法的新思路。
...
### [306] [Robust Driving Control for Autonomous Vehicles: An Intelligent General-sum Constrained Adversarial Reinforcement Learning Approach](https://arxiv.org/abs/2510.09041)
*Junchao Fan,Xiaolin Chang*

Main category: cs.LG

TL;DR: 提出IGCARL方法，通过战略对抗攻击和约束优化提升自动驾驶系统的对抗鲁棒性


<details>
  <summary>Details</summary>
Motivation: 现有DRL自动驾驶方法存在三个关键问题：对抗短视攻击、难以引发真正安全关键事件、训练不稳定和政策漂移

Method: IGCARL包含战略目标对抗者和鲁棒驾驶智能体，对抗者执行战略协调多步攻击，智能体在约束条件下学习鲁棒策略

Result: 实验显示IGCARL比现有方法成功率提高至少27.9%，显著增强对抗攻击的鲁棒性

Conclusion: IGCARL方法有效提升了DRL自动驾驶的安全性和可靠性
...
### [307] [Spatio-Temporal Graph Convolutional Networks for EV Charging Demand Forecasting Using Real-World Multi-Modal Data Integration](https://arxiv.org/abs/2510.09048)
*Jose Tupayachi,Mustafa C. Camur,Kevin Heaslip,Xueping Li*

Main category: cs.LG

TL;DR: TW-GCN框架结合图卷积网络和时序架构，预测美国田纳西州电动汽车充电需求，发现3小时中期预测在响应性和稳定性间取得最佳平衡，1DCNN模型表现最优。
...
Method: 结合图卷积网络和时序架构的时空预测框架TW-GCN，使用真实交通流量、天气条件和专有数据捕捉空间依赖性和时序动态。

Result: 3小时中期预测效果最佳，1DCNN模型表现优于其他时序模型；田纳西州东、中、西部预测精度存在差异，反映站点密度、人口和需求变异性对模型性能的影响。

Conclusion: TW-GCN框架推动了数据驱动智能在电动汽车基础设施规划中的应用，支持可持续交通转型和弹性电网管理。
...
### [308] [Improving Anomaly Detection in Industrial Time Series: The Role of Segmentation and Heterogeneous Ensemble](https://arxiv.org/abs/2510.09079)
*Emilio Mastriani,Alessandro Costa,Federico Incardona,Kevin Munari,Sebastiano Spinello*

Main category: cs.LG

TL;DR: 将分割技术作为预处理步骤与异构集成结合，显著提升了工业异常检测性能，AUC-ROC从0.8599提高到0.9760


<details>
  <summary>Details</summary>
Motivation: 探索如何通过整合分割技术和异构集成来增强工业生产环境中的异常检测能力，减少时间不确定性

Method: 使用ChangeFinder等变化点检测算法进行时间序列分割作为预处理步骤，然后应用随机森林和XGBoost等异构集成算法

Result: 在案例研究中，分割预处理显著提升了异常检测性能，AUC-ROC从PCA和LSTM集成的0.8599提高到随机森林和XGBoost集成的0.9760
...
### [309] [FLToP CTC: Frame-Level Token Pruning via Relative Threshold for Efficient and Memory-Saving Decoding on Diverse Platforms](https://arxiv.org/abs/2510.09085)
*Atul Shree,Harshith Jupuru*

Main category: cs.LG

TL;DR: FLToP CTC是一种新颖的CTC解码算法，通过帧级token剪枝技术，在保持WER性能的同时显著提升计算效率和内存使用效率。


<details>
  <summary>Details</summary>
Motivation: 解决CTC-based ASR系统在资源受限环境中的计算和内存瓶颈问题，传统CTC解码器在处理过程中需要大量token级操作，消耗高达90%的处理时间。

Method: 采用帧级token剪枝方法，基于相对概率阈值动态消除每帧中的低概率token，从而减少计算和内存需求。
...
### [310] [Neural Codecs as Biosignal Tokenizers](https://arxiv.org/abs/2510.09095)
*Kleanthis Avramidis,Tiantian Feng,Woojae Jeong,Jihwan Lee,Wenhui Cui,Richard M Leahy,Shrikanth Narayanan*

Main category: cs.LG

TL;DR: BioCodec是一个受神经编解码器启发的表示学习框架，可将生物信号转换为离散标记，在多个下游任务中表现优异，特别是在低资源环境下。


<details>
  <summary>Details</summary>
Motivation: 神经生理记录（如EEG）产生高维噪声时间序列数据，需要大量预处理和特征工程。现有方法难以有效解码和解释这些生物信号。

Method: 提出BioCodec框架，受神经编解码器启发，将生物信号的低级特征捕获为离散标记。在数千小时EEG数据上预训练。

Result: BioCodec在临床诊断、睡眠生理、语音解码和运动想象等多个任务中表现优异，尤其在低资源设置下。方法也适用于其他生物信号如EMG。

Conclusion: BioCodec提供了一个通用的生物信号标记化解决方案，性能与最先进模型相当，代码和模型已开源。
...
### [311] [AdaPM: a Partial Momentum Algorithm for LLM Training](https://arxiv.org/abs/2510.09103)
...
Result: 在60M到1.5B参数的语言模型预训练、监督微调和RLHF中，AdaPM减少90%以上动量内存，结合二阶统计量技术可节省95%优化器状态内存，为GPT-2 1.5B预训练节省30%以上GPU小时。

Conclusion: AdaPM通过自适应部分动量设计，在保持训练效率和模型性能的同时，显著降低了优化器的内存需求。
...
### [312] [MemLoss: Enhancing Adversarial Training with Recycling Adversarial Examples](https://arxiv.org/abs/2510.09105)
*Soroush Mahdi,Maryam Amirmazlaghani,Saeed Saravani,Zahra Dehghanian*

Main category: cs.LG

TL;DR: 提出MemLoss方法，利用先前生成的对抗样本（记忆对抗样本）来增强对抗训练，在保持干净数据性能的同时提高模型鲁棒性和准确性。


<details>
  <summary>Details</summary>
Motivation: 现有对抗训练方法在提高模型鲁棒性时往往会牺牲在干净数据上的准确性，需要一种能平衡自然准确性和对抗鲁棒性的方法。

Method: MemLoss通过利用先前训练周期中生成的记忆对抗样本来增强对抗训练过程，将这些样本跨训练周期重复使用。
...
### [313] [On the Fairness of Privacy Protection: Measuring and Mitigating the Disparity of Group Privacy Risks for Differentially Private Machine Learning](https://arxiv.org/abs/2510.09114)
*Zhi Yang,Changwu Huang,Ke Tang,Xin Yao*

Main category: cs.LG

TL;DR: 本文提出了一种新的成员推理游戏来高效审计数据记录的近似最坏情况隐私风险，并改进了DP-SGD算法，通过自适应组特定梯度裁剪策略来减少组隐私风险差异，提升DPML中隐私保护的公平性。
...
Method: 引入新的成员推理游戏来高效审计近似最坏情况隐私风险；改进DP-SGD算法，采用自适应组特定梯度裁剪策略。

Result: 实验结果表明，该方法能更严格地测量组隐私风险，可靠评估组隐私风险差异；改进的算法有效减少了组隐私风险差异。

Conclusion: 提出的方法能够更准确地评估组隐私风险差异，并通过改进的DP-SGD算法有效提升差分隐私机器学习中隐私保护的公平性。
...
### [314] [Regret Bounds for Adversarial Contextual Bandits with General Function Approximation and Delayed Feedback](https://arxiv.org/abs/2510.09127)
*Orin Levy,Liad Erez,Alon Cohen,Yishay Mansour*

Main category: cs.LG

TL;DR: 本文研究了具有延迟反馈的上下文多臂老虎机问题，提出了两种算法：一种针对有限策略类的最优后悔界，另一种针对一般函数逼近的后悔界，并分析了稳定性参数和给出了下界。


<details>
  <summary>Details</summary>
Motivation: 解决在对抗性延迟反馈下的上下文多臂老虎机问题，延迟由对手选择，这是实际应用中常见的挑战。

Method: 使用两种方法：1）直接访问有限策略类的最优算法；2）使用在线最小二乘回归预言机的一般函数逼近方法，并分析了基于Hedge的Vovk聚合预测器的稳定性。

Result: 获得了有限策略类的最优后悔界O(√(KT log|Π|) + √(D log|Π|))，以及一般函数逼近的后悔界O(√(KTℛ_T(O)) + √(d_max D β))，其中β≤log|F|。

Conclusion: 提出的算法在延迟反馈下实现了接近最优的后悔界，为一般函数逼近设置提供了理论保证，并给出了相应的下界分析。
...
### [315] [Score-Based Density Estimation from Pairwise Comparisons](https://arxiv.org/abs/2510.09146)
...
<details>
  <summary>Details</summary>
Motivation: 研究动机来自专家知识获取和从人类反馈中学习，需要从成对比较中估计密度函数。

Method: 方法包括：将目标密度与温度调节的胜者密度关联，通过分数匹配学习胜者分数，推导出信念密度和胜者密度分数向量之间的共线性关系，提出在Bradley-Terry模型下的温度场估计器，使用基于分数缩放的退火Langevin动力学生成温度样本的扩散模型。

Result: 该方法能够从仅数百到数千个成对比较中学习模拟专家的复杂多元信念密度。

Conclusion: 通过建立目标密度与温度调节胜者密度的关系，并利用分数匹配和扩散模型，可以有效地从成对比较中估计复杂的多元密度函数。
...
### [316] [Logits Replay + MoClip: Stabilized, Low-Cost Post-Training with Minimal Forgetting](https://arxiv.org/abs/2510.09152)
*Suming Qiu,Jing Li,Zhicheng Zhou,Junjie Huang,Linyuan Qiu,Zhijie Sun*

Main category: cs.LG

TL;DR: 提出Logits Replay + MoClip两阶段框架，通过压缩logit空间的监督信号和优化器层面的稳定化，在提升领域性能的同时减轻通用能力遗忘，并降低40%以上的训练成本。
...
Method: 两阶段框架：Stage 0记录覆盖概率阈值的动态Top-K token子集；Stage 1重放这些紧凑子集计算精确重归一化损失。结合MoClip优化器，限制梯度动量旋转并基于arctan2重新缩放更新。
...
### [317] [Agentic-KGR: Co-evolutionary Knowledge Graph Construction through Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2510.09156)
*Jing Li,Zhijie Sun,Zhicheng Zhou,Suming Qiu,Junjie Huang,Haijia Sun,Linyuan Qiu*

Main category: cs.LG

TL;DR: Agentic-KGR是一个通过多轮强化学习实现大语言模型与知识图谱协同进化的框架，解决了传统知识增强LLM依赖静态知识库导致的覆盖不足和时效性问题。


<details>
  <summary>Details</summary>
Motivation: 当前知识增强的大语言模型依赖静态、预构建的知识库，存在覆盖范围不足和时间过时的问题，限制了其在动态信息环境中的有效性。

Method: 提出了三个关键创新：(1)动态模式扩展机制，在训练期间系统扩展图本体超出预定义边界；(2)检索增强记忆系统，通过持续优化实现模型参数与知识结构的协同进化；(3)可学习的多尺度提示压缩方法，通过自适应序列优化在降低计算复杂度的同时保留关键信息。

Result: 实验结果显示在知识提取任务中显著优于监督基线和单轮RL方法。与GraphRAG集成后，在下游QA任务中表现出优越性能，在准确性和知识覆盖方面相比现有方法有显著提升。

Conclusion: Agentic-KGR框架通过多轮强化学习实现了LLM与KG的有效协同进化，显著提升了动态信息环境中的知识处理能力。
...
### [318] [Cross-Representation Benchmarking in Time-Series Electronic Health Records for Clinical Outcome Prediction](https://arxiv.org/abs/2510.09159)
*Tianyi Chen,Mingcheng Zhu,Zhiyao Luo,Tingting Zhu*

Main category: cs.LG

TL;DR: 本文提出了首个系统性的EHR表示方法基准测试，比较了多变量时间序列、事件流和文本事件流三种范式，发现在不同临床场景下事件流模型表现最佳，且特征选择策略需根据具体场景调整。
...
Method: 构建统一基准，在MIMIC-IV（ICU任务）和EHRSHOT（纵向护理）数据集上标准化数据整理和评估，比较三种表示范式：时间序列（Transformers、MLP、LSTM、Retain）、事件流（CLMBR、计数模型）和文本事件流（8-200亿参数LLM），并分析基于数据缺失的特征剪枝影响。

Result: 事件流模型始终表现最强；预训练模型如CLMBR在少样本设置中样本效率高，但给定足够数据时简单计数模型也有竞争力；ICU预测中剪枝稀疏特征有提升，而纵向任务中保留这些特征至关重要。

Conclusion: 通过统一可复现的流程，研究结果为基于临床背景和数据状况选择EHR表示提供了实用指导，强调需要根据具体临床场景调整表示方法和特征选择策略。
...
### [319] [Efficient Resource-Constrained Training of Vision Transformers via Subspace Optimization](https://arxiv.org/abs/2510.09160)
*Le-Trung Nguyen,Enzo Tartaglione,Van-Tam Nguyen*

Main category: cs.LG

TL;DR: WASI是一种基于子空间的训练方法，通过将训练限制在固定子空间内，显著降低Transformer模型的内存使用和计算成本，同时保持与标准训练相当的准确性。


<details>
  <summary>Details</summary>
Motivation: 随着AI在日常生活中的应用日益广泛，能源消耗和数据隐私成为紧迫问题。设备端学习直接在边缘设备上训练模型，能够减少能耗并保护数据隐私，但现代神经网络规模的扩大给设备端训练带来了主要障碍。

Method: 应用基于子空间的训练方法到Transformer模型，引入权重-激活子空间迭代(WASI)方法，通过将训练限制在模型关键信息所在的固定子空间内，缓解反向传播的内存瓶颈并提升推理效率。

Result: WASI在保持与标准训练相当的准确性的同时，将内存使用减少了高达62倍，计算成本(FLOPs)减少了高达2倍。在Raspberry Pi 5上，WASI实现了约1.5倍于标准训练的训练和推理速度。

Conclusion: 基于子空间的训练方法WASI能够有效解决Transformer模型在设备端训练中的内存和计算瓶颈问题，为边缘设备上的高效AI部署提供了可行方案。
...
### [320] [Robustness and Regularization in Hierarchical Re-Basin](https://arxiv.org/abs/2510.09174)
*Benedikt Franke,Florian Heinrich,Markus Lange,Arne Raul*

Main category: cs.LG

TL;DR: 本文重新审视Git Re-Basin模型融合方法，提出分层模型合并方案，显著优于标准MergeMany算法，发现Re-Basin能增强合并模型的对抗鲁棒性，但性能下降比原作者报告的更严重。


<details>
  <summary>Details</summary>
Motivation: 深入研究Git Re-Basin这一新兴模型融合方法，探索更有效的模型合并策略，并验证其对模型鲁棒性的影响。

Method: 提出分层模型合并方案，与标准MergeMany算法进行对比，通过实验分析Re-Basin方法的效果。

Result: 分层合并方案显著优于标准算法；Re-Basin能诱导对抗和扰动鲁棒性，且参与合并的模型越多效果越强；但性能下降比原作者报告的要大得多。

Conclusion: Git Re-Basin方法在增强模型鲁棒性方面有潜力，但实际性能损失需要更谨慎评估，分层合并策略是有效的改进方向。
...
### [321] [Beyond Pairwise Connections: Extracting High-Order Functional Brain Network Structures under Global Constraints](https://arxiv.org/abs/2510.09175)
*Ling Zhan,Junjie Huang,Xiaoyao Yu,Wenyu Chen,Tao Jia*

Main category: cs.LG

TL;DR: 提出GCM框架，通过全局约束学习功能性脑网络的高阶结构，在5个数据集上相比基线方法准确率提升30.6%，计算时间减少96.3%。


<details>
  <summary>Details</summary>
Motivation: 现有功能性脑网络建模主要依赖局部成对交互，无法有效捕捉高阶依赖关系；同时当前超图建模方法计算负担重且依赖启发式策略，难以实现端到端学习。

Method: 提出全局约束导向的多分辨率(GCM)框架，整合4种全局约束(信号同步、被试身份、期望边数、数据标签)来学习4个不同分辨率水平(样本/被试/组别/项目)的功能性脑网络结构。

Result: 在5个数据集和2种任务设置下，相比9个基线和10个最先进方法，GCM实现了相对准确率提升30.6%，计算时间减少96.3%。

Conclusion: 该工作为功能性脑网络结构学习提供了新视角，为认知神经科学的跨学科应用奠定了基础。
...
### [322] [RepDL: Bit-level Reproducible Deep Learning Training and Inference](https://arxiv.org/abs/2510.09180)
...
### [323] [On the Implicit Adversariality of Catastrophic Forgetting in Deep Continual Learning](https://arxiv.org/abs/2510.09181)
*Ze Peng,Jian Zhang,Jintao Guo,Lei Qi,Yang Gao,Yinghuan Shi*

Main category: cs.LG

TL;DR: 本文揭示了持续学习中灾难性遗忘的根本原因：新任务训练本质上是对旧任务知识的对抗性攻击，新任务梯度会自动与旧任务损失函数的尖锐方向对齐，导致旧任务知识快速遗忘。


<details>
  <summary>Details</summary>
Motivation: 理解灾难性遗忘的深层机制，揭示其对抗性本质，为开发更有效的持续学习方法提供理论基础。

Method: 通过理论分析发现训练的低秩偏置导致新旧任务梯度在低维子空间中对齐，提出backGP方法解决反向传播引起的对齐问题。

Result: backGP方法相比梯度投影方法平均减少10.8%的遗忘，提高12.7%的准确率。

Conclusion: 灾难性遗忘源于新任务训练对旧任务知识的对抗性攻击，backGP能有效缓解由反向传播引起的梯度对齐问题。
...
### [324] [Multimodal Prompt Optimization: Why Not Leverage Multiple Modalities for MLLMs](https://arxiv.org/abs/2510.09201)
...
<details>
  <summary>Details</summary>
Motivation: 现有提示优化方法仅限于文本，限制了多模态大语言模型的全部潜力，需要扩展到多模态空间。

Method: 提出MPO框架，通过保持对齐的联合更新优化多模态提示，并利用贝叶斯策略基于先前评估指导候选提示选择。
...
### [325] [FM-IRL: Flow-Matching for Reward Modeling and Policy Regularization in Reinforcement Learning](https://arxiv.org/abs/2510.09222)
*Zhenglin Wan,Jingxuan Wu,Xingrui Yu,Chubin Zhang,Mingcong Lei,Bo An,Ivor Tsang*

Main category: cs.LG

TL;DR: 提出一种结合Flow Matching教师模型和MLP学生策略的方法，通过奖励模型和正则化机制，在保持表达力的同时实现高效在线强化学习


<details>
  <summary>Details</summary>
Motivation: Flow Matching在离线模仿学习中表现优异，但缺乏环境交互和探索能力，在未见场景中泛化能力差。直接在线优化FM策略存在梯度不稳定和推理成本高的问题

Method: 使用简单的MLP学生策略进行环境探索和在线RL更新，通过基于教师FM模型的奖励模型提供专家分布信息，并用教师FM模型正则化学生策略行为

Result: 实验表明该方法显著提升了学习效率、泛化能力和鲁棒性，特别是在从次优专家数据学习时表现突出

Conclusion: 通过分离表达力强的教师模型和高效探索的学生策略，成功解决了FM策略在线学习的问题，实现了表达力与效率的平衡
...
### [326] [Prime Implicant Explanations for Reaction Feasibility Prediction](https://arxiv.org/abs/2510.09226)
...
Method: 引入了质蕴涵解释（也称为最小充分原因）的新表述，专门针对化学反应领域，并提出了在小规模反应预测任务中计算此类解释的算法。

Result: 初步实验表明，质蕴涵解释方法能够保守地捕捉真实解释，虽然可能包含冗余的键和原子，但始终能捕获预测反应可行性所必需的分子属性。

Conclusion: 该方法为化学反应预测模型提供了有效的解释机制，能够识别对预测结果至关重要的分子特征，提高了模型的可解释性。
...
### [327] [Incentivizing Time-Aware Fairness in Data Sharing](https://arxiv.org/abs/2510.09240)
...
<details>
  <summary>Details</summary>
Motivation: 现有协作框架假设所有参与者同时加入，但现实中参与者可能在不同时间加入，早期加入者承担更高风险且能鼓励观望者贡献，因此应获得更高回报。

Method: 开发了时间感知激励机制，包括决定奖励价值的新方法，以及生成模型奖励来实现这些价值。

Result: 在合成和真实数据集上实证验证了所提方法的特性，展示了其有效性。

Conclusion: 该框架通过时间感知激励机制解决了现实世界中参与者不同时加入的问题，为早期贡献者提供公平回报，促进了协作数据共享。
...
### [328] [A PCA-based Data Prediction Method](https://arxiv.org/abs/2510.09246)
...
### [329] [Mitigating Model Drift in Developing Economies Using Synthetic Data and Outliers](https://arxiv.org/abs/2510.09294)
...
TL;DR: 该研究探讨了在金融机器学习模型中通过添加合成异常值来缓解模型漂移问题，特别是在中亚和高加索地区的发展中经济体金融数据集上的应用。


<details>
  <summary>Details</summary>
Motivation: 金融机器学习模型容易因数据分布变化而出现性能下降（模型漂移），这在发展中经济体（如中亚和高加索地区）尤为严重，因为频繁且不可预测的宏观经济冲击会破坏金融数据的稳定性。
...
Conclusion: 合成异常值是一种有效的模型漂移缓解方法，特别适用于发展中经济体的金融数据集，但需要根据具体场景调整使用策略。
...
### [330] [Large Language Model Prompt Datasets: An In-depth Analysis and Insights](https://arxiv.org/abs/2510.09316)
*Yuanming Zhang,Yan Lin,Arijit Khan,Huaiyu Wan*

Main category: cs.LG

TL;DR: 本文首次系统性地收集和分析了来自不同来源的提示数据集，提出了一种基于句法嵌入的提示优化方法，通过引导LLM将提示重写向中心表示来提升模型输出的质量。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的广泛应用，来自GitHub、社交媒体等平台的多样化提示数据集不断涌现，这些数据集涵盖了广泛的应用场景和内容类型。然而，目前缺乏对这些提示数据集的系统性分析和比较，以及基于此的提示优化方法。

Method: 1) 从多个渠道收集广泛的提示数据集；2) 选择代表性数据集进行系统性分析，揭示不同类别提示的共性和差异；3) 提出基于词性标注和依存结构句法嵌入的提示优化方法，通过识别提示的中心表示并引导LLM将提示重写向该中心来提升输出质量。

Result: 成功构建了一个包含多样化提示的数据集，揭示了不同类别提示在构造上的特点；提出的提示优化方法能够有效提升模型输出的意义性。

Conclusion: 本研究为提示工程提供了系统性的数据集分析和优化方法，通过句法结构的中心化表示来改进提示质量，有助于推动更有效的LLM应用。
...
### [331] [Residual-Informed Learning of Solutions to Algebraic Loops](https://arxiv.org/abs/2510.09317)
...
<details>
  <summary>Details</summary>
Motivation: 解决Modelica模型中代数环带来的计算效率问题，传统方法需要大量监督数据且存在多解歧义。

Method: 使用前馈神经网络，在损失函数中直接利用代数环的残差进行训练，无需监督数据集，通过残差训练策略确保收敛到一致解。

Result: 在IEEE 14-Bus系统中，相比传统仿真方法实现了60%的仿真时间减少，同时通过误差控制机制保持了相同的精度水平。

Conclusion: 残差驱动的机器学习方法能有效替代代数环，显著提升仿真效率，同时避免多解问题，为大规模系统仿真提供了高效解决方案。
...
### [332] [Rate optimal learning of equilibria from data](https://arxiv.org/abs/2510.09325)
*Till Freihaut,Luca Viano,Emanuele Nevali,Volkan Cevher,Matthieu Geist,Giorgia Ramponi*

Main category: cs.LG

TL;DR: 本文填补了多智能体模仿学习(MAIL)的理论空白，证明了非交互式MAIL的统计下界，提出了首个具有近最优样本复杂度的交互式算法MAIL-WARM，将样本复杂度从O(ε⁻⁸)提升到O(ε⁻²)。


<details>
  <summary>Details</summary>
Motivation: 解决多智能体模仿学习中的理论空白，特别是非交互式方法的局限性，以及开发更高效的交互式算法。

Method: 在非交互式设置中证明统计下界，识别全策略偏差集中系数为基本复杂度度量；在交互式设置中，结合无奖励强化学习和交互式MAIL，提出MAIL-WARM算法。

Result: 证明了行为克隆在非交互式设置中是速率最优的；MAIL-WARM算法将样本复杂度从O(ε⁻⁸)显著改进到O(ε⁻²)，匹配理论下界。

Conclusion: 本文为多智能体模仿学习建立了完整的理论框架，证明了非交互式方法的局限性，并提供了高效的交互式解决方案，在网格世界等环境中展示了优于行为克隆的性能。
...
### [333] [Safety Game: Balancing Safe and Informative Conversations with Blackbox Agentic AI using LP Solvers](https://arxiv.org/abs/2510.09330)
*Tuan Nguyen,Long Tran-Thanh*

Main category: cs.LG

TL;DR: 提出了一种独立于模型的黑盒安全对齐框架，无需重新训练或访问LLM内部架构，通过博弈论方法在推理时平衡安全性和有用性
...
Result: 证明了黑盒安全对齐的可行性，为资源受限环境提供可扩展的安全对齐方案

Conclusion: 该框架为各种利益相关者提供了在快速发展的LLM生态系统中实施安全对齐的可访问途径
...
### [334] [Efficient Bayesian Inference from Noisy Pairwise Comparisons](https://arxiv.org/abs/2510.09333)
...
<details>
  <summary>Details</summary>
Motivation: 标准评估指标不能反映人类偏好，而人类评估虽然更可靠但成本高且噪声大。现有Bradley-Terry方法要么忽略评分者变异性，要么缺乏收敛保证。

Method: 提出BBQ方法，使用贝叶斯Bradley-Terry模型显式建模评分者质量，通过期望最大化算法提供单调似然收敛保证，降低不可靠参与者的权重。

Result: BBQ实现了更快的收敛速度、良好校准的不确定性估计，以及与基线Bradley-Terry模型相比更稳健、可解释的排名，即使在噪声或众包评分者情况下也表现良好。
...
### [335] [Deep Learning to Identify the Spatio-Temporal Cascading Effects of Train Delays in a High-Density Network](https://arxiv.org/abs/2510.09350)
*Vu Duc Anh Nguyen,Ziyue Li*

Main category: cs.LG

TL;DR: 提出了一个名为XGeoAI的新框架，用于实时、可解释的多步列车延误预测，采用两阶段自回归图注意力网络模型，在荷兰铁路网络数据集上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 铁路网络的运营效率受到列车延误级联效应的持续影响，现有研究缺乏能够提供网络范围多步自回归预测并同时提供实时可解释解释的框架。

Method: 开发了一个两阶段自回归图注意力网络(GAT)模型，将系统表示为运营事件(到达和离开)的时空图，并加入了平台和车站拥堵等细粒度特征。

Result: 虽然提出的GATv2模型在纯误差指标(MAE)上被更简单的持续性基线挑战，但在延误事件分类精度上始终更高，这对于可靠的决策支持工具至关重要。

Conclusion: 该框架展示了在实时部署中的可行性，通过顺序k步预测协议模拟真实条件，证明了其在延误事件分类方面的优势。
...
### [336] [The Potential of Second-Order Optimization for LLMs: A Study with Full Gauss-Newton](https://arxiv.org/abs/2510.09378)
*Natalie Abreu,Nikhil Vyas,Sham Kakade,Depen Morwani*

Main category: cs.LG

TL;DR: 本文通过应用全高斯-牛顿预条件处理到150M参数的Transformer模型，建立了迭代复杂度的实际上限，发现GN更新比现有优化器显著减少训练迭代次数，且分层GN预条件器性能接近全GN方法。


<details>
  <summary>Details</summary>
Motivation: 探究在LLM预训练加速中，计算效率高的近似方法会损失多少性能，为大规模训练建立迭代复杂度的实际上限。

Method: 应用全高斯-牛顿预条件处理到Transformer模型，并与现有优化器（如SOAP和Muon）进行比较，同时测试精确的分层GN预条件器。

Result: 全GN更新相比强基线优化器减少5.4倍训练迭代次数，分层GN预条件器性能接近全GN方法，表明分层Hessian结构包含足够信息。

Conclusion: GN近似对预条件处理非常有效，高阶损失项对收敛速度可能不关键；分层Hessian结构足以实现大部分潜在增益；当前近似方法与理想分层oracle存在显著性能差距。
...
### [337] [Task-Level Insights from Eigenvalues across Sequence Models](https://arxiv.org/abs/2510.09379)
*Rahel Rickenbach,Jelena Trisovic,Alexandre Didier,Jerome Sieber,Melanie N. Zeilinger*

Main category: cs.LG

TL;DR: 该论文通过动力学系统框架将softmax、norm和线性注意力表示为动力学系统，并与状态空间模型(SSMs)进行特征值谱比较，发现特征值影响序列模型的记忆和长程依赖建模能力，为理解和改进序列模型提供了原则性指标。


<details>
  <summary>Details</summary>
Motivation: 虽然softmax注意力在序列模型中表现出色，但其二次复杂度限制了可扩展性，而线性替代方案如SSMs虽然提高了效率，但它们在信息处理方面的根本差异仍未被充分理解。

Method: 利用动力学系统框架将不同类型的注意力机制表示为动力学系统，通过分析它们的特征值谱与SSMs进行结构化比较，并在多种序列模型和基准测试中进行广泛的实证分析。

Result: 特征值影响记忆和长程依赖建模的关键方面，揭示了与任务需求一致的光谱特征；架构修改对特征值谱和任务性能的影响存在对应关系。

Conclusion: 特征值分析为解释、理解并最终改进序列模型的能力提供了原则性指标，强化了其在序列模型研究中的重要地位。
...
### [338] [CHUCKLE -- When Humans Teach AI To Learn Emotions The Easy Way](https://arxiv.org/abs/2510.09382)
*Ankush Pratap Singh,Houwei Cao,Yong Liu*

Main category: cs.LG

TL;DR: 提出了CHUCKLE框架，利用众包数据中的标注者一致性和对齐度来定义样本难度，基于人类感知的困难程度进行课程学习，提高了情感识别的准确率和训练效率。


<details>
  <summary>Details</summary>
Motivation: 现有情感识别中的课程学习方法通常依赖启发式、数据驱动或模型定义的样本难度，忽略了人类感知的困难程度这一主观任务中的关键因素。

Method: CHUCKLE框架通过众包数据集中的标注者一致性和对齐度来定义样本难度，假设对人类具有挑战性的样本对机器学习模型同样困难，并据此构建从简单到复杂的课程学习训练过程。

Result: 实验结果显示，CHUCKLE相比非课程学习基线，LSTM模型的相对平均准确率提高了6.56%，Transformer模型提高了1.61%，同时减少了梯度更新次数。

Conclusion: CHUCKLE框架通过基于人类感知的课程学习，不仅提高了情感识别模型的准确率，还提升了训练效率和模型鲁棒性。
...
### [339] [HINT: Helping Ineffective Rollouts Navigate Towards Effectiveness](https://arxiv.org/abs/2510.09388)
*Xinyi Wang,Jinyi Han,Zishang Jiang,Tingyun Li,Jiaqing Liang,Sihang Jiang,Zhaoqian Dai,Shuguang Ma,Fei Yu,Yanghua Xiao*

Main category: cs.LG

TL;DR: 提出了HINT框架，通过提供启发式提示而非直接答案来改善强化学习中的训练亲和力问题，在数学推理任务上取得SOTA结果。
...
Method: 引入Affinity指标监控探索效率和训练稳定性，提出HINT自适应提示框架，提供启发式提示引导模型自主发现解决方案。

Result: 在数学推理任务上持续优于现有方法，在不同规模模型上都取得SOTA结果，学习更稳定且数据效率更高。

Conclusion: HINT通过改善训练亲和力有效解决了RL在复杂推理任务中的挑战，同时保持了模型的自主推理能力。
...
### [340] [Design Principles for Sequence Models via Coefficient Dynamics](https://arxiv.org/abs/2510.09389)
...
<details>
  <summary>Details</summary>
Motivation: 为了深入理解并系统比较Transformer、状态空间模型(SSMs)和门控线性RNN等深度序列模型，揭示它们共同的数学基础，并建立连接架构选择与模型特性的设计原则。

Method: 开发了一个统一框架，将线性组合系数建模为由脉冲输入驱动的自主线性动力系统的输出，从而显式化模型的输出操作，并分析不同架构的数学共性。

Result: 该框架成功捕获了softmax注意力以及RNNs、SSMs和相关模型，揭示了架构选择在表达能力、高效实现、输入选择性和数值稳定性之间的权衡关系。

Conclusion: 该框架不仅解释了近期设计经验上的成功，还为系统设计新的序列模型架构提供了指导原则，连接了文献中的多个见解和观察。
...
### [341] [Cross-Receiver Generalization for RF Fingerprint Identification via Feature Disentanglement and Adversarial Training](https://arxiv.org/abs/2510.09405)
*Yuhao Pan,Xiucheng Wang,Nan Cheng,Wenchao Xu*

Main category: cs.LG

TL;DR: 提出了一种对抗训练和风格迁移相结合的射频指纹识别框架，能够有效分离发射器和接收器特征，解决跨接收器变异性问题。


<details>
  <summary>Details</summary>
Motivation: 射频指纹识别在真实部署中面临接收器引起的特征偏移问题，导致模型过度拟合接收器特定模式，当更换接收器时性能会显著下降。

Method: 集成对抗训练和风格迁移技术，通过强制域不变表示学习来分离发射器硬件特征和接收器伪影。

Result: 在多接收器数据集上的实验表明，该方法在多种接收器设置下平均准确率比现有最优方法提升高达10%。

Conclusion: 所提出的框架能够有效解决跨接收器变异性问题，确保射频指纹识别系统在接收器更换时的鲁棒性。
...
### [342] [What Do Temporal Graph Learning Models Learn?](https://arxiv.org/abs/2510.09416)
*Abigail J. Hayes,Tobias Schumacher,Markus Strohmaier*

Main category: cs.LG

TL;DR: 本文系统评估了7个时序图学习模型捕获8种基本图属性的能力，发现模型在某些属性上表现良好但在其他属性上存在明显局限，揭示了当前时序图学习模型的局限性。


<details>
  <summary>Details</summary>
Motivation: 由于现有基准测试结果可靠性受到质疑，且简单启发式方法表现惊人，需要探究时序图学习模型实际利用哪些图属性进行预测。

Method: 使用合成和真实世界数据集，系统评估7个模型在捕获8种基本时序图属性（包括结构特征、时序模式和边形成机制）方面的能力。

Result: 模型能够较好地捕获某些属性，但在重现其他属性方面表现失败，暴露了重要局限性。

Conclusion: 研究结果为时序图学习模型的应用提供了实用见解，并推动该领域进行更多可解释性驱动的评估。
...
### [343] [Weight Initialization and Variance Dynamics in Deep Neural Networks and Large Language Models](https://arxiv.org/abs/2510.09423)
*Yankun Han*

Main category: cs.LG

TL;DR: 该论文系统研究了权重初始化对神经网络训练的影响，包括ReLU多层感知机和GPT-2风格transformer，发现了稳定的初始化标准差范围，比较了不同初始化方法的性能，并观察到transformer层间权重方差的深度依赖性变化模式。


<details>
  <summary>Details</summary>
Motivation: 研究权重初始化如何影响信号传播和梯度流动，特别是在训练开始时，这对于网络训练的稳定性和收敛速度至关重要。

Method: 采用理论分析和实证验证相结合的方法，包括对数扫描初始标准差、比较Kaiming和Xavier初始化方法，以及在12层GPT-2风格模型中跟踪层间Q/K/V权重方差变化。

Result: 发现1e-2到1e-1的标准差范围提供稳定训练；Kaiming初始化在ReLU网络中收敛更快更稳定；transformer浅层权重方差快速扩张而深层变化更平缓。

Conclusion: 研究结果将经典初始化原则与现代transformer行为联系起来，为鲁棒训练提供了简单实用的指导方案。
...
### [344] [Bandits with Single-Peaked Preferences and Limited Resources](https://arxiv.org/abs/2510.09425)
*Gur Keinan,Rotem Torkan,Omer Ben-Porat*

Main category: cs.LG

TL;DR: 本文研究在线随机匹配问题，提出在单峰偏好结构下的高效算法，分别针对已知和未知结构情况设计了两种算法，获得了次线性遗憾界。


<details>
  <summary>Details</summary>
Motivation: 解决在线随机匹配问题中，由于缺乏结构假设导致计算最优匹配NP难的问题，利用单峰偏好结构来克服计算障碍。

Method: 基于PQ树的顺序近似方法，针对已知单峰结构开发UCB类算法，针对未知结构开发高效在线算法。

Result: 未知结构时获得O(UKT^{2/3})遗憾界，已知结构时获得O(U√(TK))遗憾界。

Conclusion: 单峰偏好结构能有效解决在线预算匹配问题的计算复杂性，提出的算法在理论和实践上都具有重要意义。
...
### [345] [Cross-attention Secretly Performs Orthogonal Alignment in Recommendation Models](https://arxiv.org/abs/2510.09435)
...
TL;DR: 本文发现跨域序列推荐中交叉注意力机制存在正交对齐现象，能自然发现查询输入中不存在的新信息，显著提升模型性能。
...
### [346] [On Uniformly Scaling Flows: A Density-Aligned Approach to Deep One-Class Classification](https://arxiv.org/abs/2510.09452)
*Faried Abu Zaid,Tim Katzke,Emmanuel Müller,Daniel Neider*

Main category: cs.LG

TL;DR: 本文证明了均匀缩放流（USFs）在理论上连接了深度单类分类和密度估计两种无监督异常检测范式，通过最大似然训练USFs等价于Deep SVDD目标，并提出了将USFs作为非USFs的替代方案。


<details>
  <summary>Details</summary>
Motivation: 无监督异常检测存在两种主要范式：深度单类分类（如Deep SVDD）学习正常数据的紧凑潜在表示，而密度估计（如标准化流）直接建模正常数据的似然。本文旨在建立这两种方法之间的理论联系。

Method: 提出均匀缩放流（USFs）——具有常数雅可比行列式的标准化流，证明通过最大似然训练USFs等价于Deep SVDD目标，并带有防止表示崩溃的正则化。

Result: USFs在负对数似然和潜在范数之间建立了比Deep SVDD或非USFs更紧密的对齐，在多个基准测试和模型骨干上，用USFs替代非USFs带来了性能提升和训练稳定性改善。

Conclusion: USFs统一了两种主要的异常检测范式，既推进了理论理解又提升了实际性能，建议将USFs作为现代异常检测架构中非USFs的即插即用替代方案。
...
### [347] [Adaptive Attacks on Trusted Monitors Subvert AI Control Protocols](https://arxiv.org/abs/2510.09462)
*Mikhail Terekhov,Alexander Panfilov,Daniil Dzenhaliou,Caglar Gulcehre,Maksym Andriushchenko,Ameya Prabhu,Jonas Geiping*

Main category: cs.LG

TL;DR: 论文研究了针对AI控制协议的自适应攻击，发现基于LLM监控器的控制协议存在根本性漏洞，攻击者可以通过嵌入公开已知或零样本提示注入来逃避监控并完成恶意任务。


<details>
  <summary>Details</summary>
Motivation: 现有的AI控制协议主要依赖LLM监控器作为防御机制，但这些监控器可能成为单点故障。论文旨在研究当不受信任的模型了解协议和监控模型时的自适应攻击。

Method: 实例化了一种简单的自适应攻击向量，攻击者在模型输出中嵌入公开已知或零样本提示注入，利用监控模型的漏洞来逃避检测。

Result: 前沿模型能够持续逃避多样化监控器，在两个主要AI控制基准测试中完成恶意任务。攻击对所有依赖监控器的当前协议都有效，Defer-to-Resample协议甚至会适得其反。
...
### [348] [STaTS: Structure-Aware Temporal Sequence Summarization via Statistical Window Merging](https://arxiv.org/abs/2510.09593)
...
<details>
  <summary>Details</summary>
Motivation: 现有时间序列模型通常处理原始或固定窗口序列，将所有时间步视为同等重要，导致长序列或噪声序列中的效率低下、鲁棒性差和可扩展性有限。
...
<details>
  <summary>Details</summary>
Motivation: 为创新金融提供透明可复现的预测工具，帮助识别有潜力的初创企业。

Method: 构建2010-2023年企业季度面板数据，使用逻辑回归和树集成方法，采用SMOTE-NC处理类别不平衡，评估三个时间窗口的预测效果。

Result: 专利、融资和退出预测的AUROC值分别达到0.921、0.817和0.872，表现出优秀的预测能力。

Conclusion: 该框架为初创企业结果预测提供了可靠且可解释的工具，在创新金融领域具有重要应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 自编码器的潜在流形提供了数据的低维表示，但从几何角度研究这些流形需要稳健的计算工具，特别是在隐式表示不准确的情况下。

Method: 通过最小化去噪目标学习潜在流形的近似投影，开发离散黎曼几何算子来近似经典几何算子，支持在潜在流形上使用不同的黎曼几何。

Result: 该方法在各种自编码器上进行了评估，包括合成数据和真实数据，能够稳健地计算测地线路径和黎曼指数映射。

Conclusion: 提出的框架为研究自编码器潜在流形的几何特性提供了有效的工具，特别是在隐式表示存在不准确性的实际应用中表现出稳健性。
...
TL;DR: CRPS-LAM是一种概率性区域天气预报模型，使用CRPS目标训练，通过单次前向传播生成集合成员，采样速度比扩散模型快39倍，在MEPS数据集上表现优异。
...
Conclusion: CRPS-LAM是概率性区域天气预报的有效方法，兼具计算效率和预报质量。
...
### [352] [Locally Optimal Private Sampling: Beyond the Global Minimax](https://arxiv.org/abs/2510.09485)
*Hrad Ghoukasian,Bonwoo Lee,Shahab Asoodeh*

Main category: cs.LG

TL;DR: 本文研究在局部差分隐私(LDP)约束下从分布中采样的本地最小最大风险问题，发现本地最优采样器在固定分布P0附近的表现优于全局方法。


<details>
  <summary>Details</summary>
Motivation: 现有工作主要关注分布类上的全局最小最优性，而本文从本地视角出发，研究固定分布P0邻域内的最小最大风险，这更贴近实际应用中私有采样与公共数据结合的场景。

Method: 将先前工作从纯LDP扩展到功能性LDP框架，证明全局最优功能性LDP采样器在P0邻域约束下产生最优本地采样器，并推导出本地最小最大最优采样器的闭式表达式。

Result: 本地最小最大风险由P0邻域约束下的全局最小最大风险决定，且本地最优采样器不依赖于f-散度的选择，在私有采样与公共数据场景中始终优于全局最小最大采样器。

Conclusion: 本地框架为私有采样提供了更精细的分析视角，特别是在结合公共数据的实际应用中，本地最优采样器展现出比全局方法更好的性能。
...
### [353] [Near-Optimal Second-Order Guarantees for Model-Based Adversarial Imitation Learning](https://arxiv.org/abs/2510.09487)
*Shangzhe Li,Dongruo Zhou,Weitong Zhang*

Main category: cs.LG

TL;DR: 本文研究了在线对抗模仿学习（AIL），提出了一种基于模型的AIL算法（MB-AIL），并建立了在一般函数逼近下的无视野、二阶样本复杂度保证。


<details>
  <summary>Details</summary>
Motivation: 尽管在线对抗模仿学习在实证中表现出色，但在线交互的益处和随机性的影响仍不清楚。本文旨在填补这些理论空白。

Method: 引入基于模型的AIL算法（MB-AIL），在专家数据和无奖励交互的一般函数逼近下进行分析。

Result: MB-AIL实现了最小最大最优的样本复杂度（在对数因子内），在有限专家演示下达到在线交互的下界，并在实验中验证了其样本效率优于现有方法。

Conclusion: MB-AIL算法在理论和实验上都表现出色，为在线对抗模仿学习提供了坚实的理论基础和实用实现。
...
### [354] [Performance Analysis of Machine Learning Algorithms in Chronic Kidney Disease Prediction](https://arxiv.org/abs/2510.09493)
*Iftekhar Ahmed,Tanzil Ebad Chowdhury,Biggo Bushon Routh,Nafisa Tasmiya,Shadman Sakib,Adil Ahmed Chowdhury*

Main category: cs.LG

TL;DR: 本研究使用八种机器学习算法预测慢性肾脏疾病，其中随机森林和逻辑回归模型达到99%的最高准确率。


<details>
  <summary>Details</summary>
Motivation: 慢性肾脏疾病影响全球10%人口，需要有效的风险评估和监测方法。机器学习因其快速准确的检测能力，可在医疗诊断中发挥重要作用。

Method: 从UCL机器学习库获取CKD数据集，使用均值-众数和随机抽样方法填补缺失值，然后应用随机森林、SVM、朴素贝叶斯、逻辑回归、KNN、XGBoost、决策树和AdaBoost八种算法建立预测模型。

Result: 随机森林和逻辑回归模型表现最佳，准确率达到99%；AdaBoost、XGBoost、朴素贝叶斯、决策树和SVM次之；KNN分类器表现最差，准确率仅为73%。

Conclusion: 机器学习模型特别是随机森林和逻辑回归，在慢性肾脏疾病预测中表现出色，可为医疗诊断提供有效支持。
...
### [355] [Geo-Aware Models for Stream Temperature Prediction across Different Spatial Regions and Scales](https://arxiv.org/abs/2510.09500)
...
Result: 使用美国东海岸多个流域37年真实数据集评估，Geo-STARS在跨区域和跨尺度的泛化性能优于最先进的基线方法。
...
### [356] [Limitations of Normalization in Attention Mechanism](https://arxiv.org/abs/2508.17821)
*Timur Mudarisov,Mikhail Burtsev,Tatiana Petrova,Radu State*

Main category: cs.LG

TL;DR: 该论文分析了注意力机制中归一化的局限性，揭示了随着选择token数量增加，模型区分信息性token的能力下降，并发现softmax归一化在低温度设置下存在梯度敏感性问题。


<details>
  <summary>Details</summary>
Motivation: 研究注意力机制中归一化的局限性，特别是softmax归一化在token选择和训练过程中的问题，为未来注意力架构提供更鲁棒的归一化和选择策略。

Method: 建立理论框架分析模型的选择能力和几何分离，推导token向量在softmax缩放下的距离界限和分离标准，并通过预训练GPT-2模型进行实证验证。

Result: 随着选择token数量增加，模型区分信息性token的能力下降，趋向均匀选择模式；softmax归一化在低温度设置下存在梯度敏感性问题。
...
Result: 在真实世界金融事件序列、图像和时间序列基准测试中，PETRA显著减小模型规模（达75%）、降低延迟（达33%）、提升吞吐量（13%），且目标指标无明显下降。

Conclusion: PETRA框架有效解决了神经网络模型优化的关键挑战，实现了模型性能与可扩展性的显著提升，为资源受限环境下的模型部署提供了可行方案。
...
TL;DR: 提出了MODE（混合动态专家）框架，通过神经门控机制将复杂动态分解为稀疏可解释的组件，能够无监督发现行为机制并在机制转换中进行准确长期预测。


<details>
  <summary>Details</summary>
Motivation: 生命科学中的动态系统通常包含重叠行为机制的复杂混合，传统基于流的方法假设局部平滑动态，难以处理机制间的噪声和不规则转换。

Method: MODE使用图形建模框架和神经门控机制，将复杂动态分解为稀疏可解释组件，允许智能体跳跃到不同的控制规律来处理噪声转换。

Result: 在合成和真实生物数据上验证，MODE在无监督分类任务中表现优异，能成功预测细胞生物学中的关键循环和分支过程，并能区分增殖与分化动态，预测细胞命运承诺时间。

Conclusion: MODE框架特别适合处理生命科学中复杂动态系统的噪声机制转换，为计算生物学中的关键挑战提供了有效解决方案。
...
TL;DR: 提出BaNEL算法，仅使用失败样本进行后训练，在奖励信号稀疏且评估成本高的场景下提升生成模型性能
...
Method: 将学习失败规律建模为另一个生成建模问题，利用该模型判断新数据是否类似已知失败并引导生成远离这些失败

Result: 在多个稀疏奖励任务中，BaNEL无需观察成功样本即可提升模型性能，比现有新颖性奖励方法成功率高出数个数量级，且使用更少的奖励评估

Conclusion: BaNEL能够有效解决稀疏奖励环境下的模型后训练问题，通过仅利用失败样本实现性能提升
